...
func TestResolveQuoteURI_UsesExistingRowWhenTheKeyIsInFlight(t *testing.T) {
	...
	// fetcher が失敗するので nil。
	require.Nil(t, r.resolveQuoteURI(listed, 0, false), "前提: 取得 URI だけでは引けない")

	r.resolvingNotes.Store(noteGroupKey(listed, false, false), storedAs)
	got := r.resolveQuoteURI(listed, 0, false)
...
func TestResolveQuoteURI_UsesEphemeralRowWhenTheKeyIsInFlight(t *testing.T) {
	...
	require.Nil(t, r.resolveQuoteURI(listed, 0, true), "前提: 取得 URI だけでは引けない")

	r.resolvingNotes.Store(noteGroupKey(listed, false, true), storedAs)
	got := r.resolveQuoteURI(listed, 0, true)
