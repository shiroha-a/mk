...
//
// **1 枠あたりの最悪値は `imagedecode.DriveMaxRasterBytes` (512MiB)。**
// 以前はここに上限が無く、drive が渡す cap (`UpstreamMaxPixels` = 268MP) を
// バイトに読み替えた **1.07GB** が 1 枠の最悪値だった。16383x16383 の全画素
// ゼロ PNG は **764 KiB** で作れて 268MP にちょうど一致するので上限を通り抜け、
// 8 core / 32GB の実測で `processImage` の 4 段を通すと**ピーク RSS 2.23 GiB /
// 13.6 秒**になる。`GOMAXPROCS / 2 = 1` の 2GB VPS なら**その 1 本で落ちる**。
//
// #3038 で 1 枚あたりのバイト予算を入れたので、**「枠の本数 × 512MiB」が
// 実際に同時確保量の上限になる**。宣言寸法の cap を下げないのは、102MP の
// 実写真がサムネイルと webpublic を失うのを避けるため (#3037 でその回帰を
// 実際に出した)。予算を超えた入力は「webpublic を作れなかった」経路に入って
// アップロードごと拒否されるので、原本が公開側へ出る形にはならない。
//
// **値と枠の本数は media proxy の枠 (#3032) と同じ。** 同じデコーダで同じ
// 中間バッファを確保するので、あちらの実測 (8 core / AVIF / c=8 で peak RSS が
