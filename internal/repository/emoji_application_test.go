...
func TestEmojiApplicationRepository_CreateWithQuota_Boundary(t *testing.T) {
	...
	require.NoError(t, repo.CreateWithQuota(quotaApp("ea_q1a", "ea_q1", "a", now), windows))
	require.NoError(t, repo.CreateWithQuota(quotaApp("ea_q1b", "ea_q1", "b", now), windows))

	err := repo.CreateWithQuota(quotaApp("ea_q1c", "ea_q1", "c", now), windows)
...
func TestEmojiApplicationRepository_CreateWithQuota_WindowExpiry(t *testing.T) {
	...
	require.NoError(t, repo.CreateWithQuota(quotaApp("ea_q2b", "ea_q2", "b", now), windows))
}
...
func TestEmojiApplicationRepository_CreateWithQuota_CountsAllStatuses(t *testing.T) {
	...
	windows := []QuotaWindow{{Name: "week", Duration: 7 * 24 * time.Hour, Max: 3}}
	err := repo.CreateWithQuota(quotaApp("ea_q3z", "ea_q3", "z", now), windows)
	var qe *QuotaExceededError
	require.ErrorAs(t, err, &qe)
...
func TestEmojiApplicationRepository_CreateWithQuota_ScopedToUser(t *testing.T) {
	...
	windows := []QuotaWindow{{Name: "day", Duration: 24 * time.Hour, Max: 1}}
	require.NoError(t, repo.CreateWithQuota(quotaApp("ea_q4a", "ea_q4", "a", now), windows))
}
...
func TestEmojiApplicationRepository_CreateWithQuota_ZeroIsUnlimited(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		require.NoError(t, repo.CreateWithQuota(
			quotaApp("ea_q5"+string(rune('a'+i)), "ea_q5", string(rune('a'+i)), now), windows))
	}
	// windows 自体が空でも同じ
	require.NoError(t, repo.CreateWithQuota(quotaApp("ea_q5z", "ea_q5", "z", now), nil))
}
...
func TestEmojiApplicationRepository_CreateWithQuota_NarrowestWindowWins(t *testing.T) {
	...
	require.NoError(t, repo.CreateWithQuota(quotaApp("ea_q6a", "ea_q6", "a", now), windows))
	err := repo.CreateWithQuota(quotaApp("ea_q6b", "ea_q6", "b", now), windows)
	var qe *QuotaExceededError
	require.ErrorAs(t, err, &qe)
	require.Equal(t, "day", qe.Window.Name)
}
...
func TestEmojiApplicationRepository_CreateWithQuota_LatestFullWindowWins(t *testing.T) {
	...
	err := repo.CreateWithQuota(quotaApp("ea_q13z", "ea_q13", "z", now), windows)
	var qe *QuotaExceededError
	require.ErrorAs(t, err, &qe)
	...
	require.NoError(t, repo.CreateWithQuota(quotaApp("ea_q13z", "ea_q13", "z", qe.RetryAt), windows))
}
...
func TestEmojiApplicationRepository_CreateWithQuota_RetryAtIsInclusive(t *testing.T) {
	...
	windows := []QuotaWindow{{Name: "day", Duration: 24 * time.Hour, Max: 1}}
	err := repo.CreateWithQuota(quotaApp("ea_q14z", "ea_q14", "z", now), windows)
	var qe *QuotaExceededError
	require.ErrorAs(t, err, &qe)
	require.NoError(t, repo.CreateWithQuota(quotaApp("ea_q14z", "ea_q14", "z", qe.RetryAt), windows))
}
...
func TestEmojiApplicationRepository_CreateWithQuota_DuplicatePending(t *testing.T) {
	...
	require.NoError(t, repo.CreateWithQuota(quotaApp("ea_q7a", "ea_q7", "same", now), windows))
	require.ErrorIs(t,
		repo.CreateWithQuota(quotaApp("ea_q7b", "ea_q7", "same", now), windows),
...
func TestEmojiApplicationRepository_CreateWithQuota_Concurrent(t *testing.T) {
	...
	now := time.Now()
	windows := []QuotaWindow{{Name: "day", Duration: 24 * time.Hour, Max: 3}}
	...
	for i := 0; i < n; i++ {
		go func(i int) {
			...
			errs <- repos[i].CreateWithQuota(
				quotaApp("ea_q8"+string(rune('a'+i)), "ea_q8", string(rune('a'+i)), now), windows)
		}(i)
	}
	ready.Wait()
	close(start)
...
func TestEmojiApplicationRepository_CreateWithQuota_RetryAtAtLimit(t *testing.T) {
	...
	windows := []QuotaWindow{{Name: "day", Duration: 24 * time.Hour, Max: 2}}
	err := repo.CreateWithQuota(quotaApp("ea_q9z", "ea_q9", "z", now), windows)
	var qe *QuotaExceededError
	require.ErrorAs(t, err, &qe)
...
func TestEmojiApplicationRepository_CreateWithQuota_RetryAtOverLimit(t *testing.T) {
	...
	windows := []QuotaWindow{{Name: "day", Duration: 24 * time.Hour, Max: 2}}
	err := repo.CreateWithQuota(quotaApp("ea_q10z", "ea_q10", "z", now), windows)
	var qe *QuotaExceededError
	require.ErrorAs(t, err, &qe)
	...
	require.NoError(t, repo.CreateWithQuota(quotaApp("ea_q10z", "ea_q10", "z", later), windows))
}
...
func TestEmojiApplicationRepository_CreateWithQuota_CountsBothKinds(t *testing.T) {
	...
	err := repo.CreateWithQuota(quotaApp("ea_q11z", "ea_q11", "z", now), windows)
	var qe *QuotaExceededError
	require.ErrorAs(t, err, &qe)
	require.Equal(t, 2, qe.Used)
	...
	require.ErrorAs(t, repo.CreateWithQuota(remote, windows), &qe)
}
...
func TestEmojiApplicationRepository_CreateWithQuota_ZeroCreatedAtUsesNow(t *testing.T) {
	...
	app.UpdatedAt = time.Now()
	windows := []QuotaWindow{{Name: "day", Duration: 24 * time.Hour, Max: 1}}
	require.NoError(t, repo.CreateWithQuota(app, windows))
}
