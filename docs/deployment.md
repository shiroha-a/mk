...
```
  ...
  ok    database     接続 ok / migration version 92
  ok    redis        接続 ok
...
mk-goはTS版と同じPostgreSQL/Redisを共有できるため、バイナリの差し替えだけで移行可能。マイグレーションはTS版テーブルに対して原則追加のみだが、例外が 13 件ある ([TS版からの移行](migration-from-ts.md#破壊的なマイグレーション))。
...
| 分類 | 意味 | 対処 |
|---|---|---|
| `unrepairable` | 再実行しても直らない (申請ファイルが削除済み / リンク行で実体を持たない / `accessKey` が無く実体を辿れない / 実体がストレージから消えている / 複製の上限 32 MiB を超えている / MIME が絵文字として許可されない) | モデレーターが絵文字を削除して別の画像で登録し直す (下記。差し替えだと利用者所有のままになる) か、絵文字を削除する。**承認済みの申請は却下できない** |
| `needs-review` | 絵文字が申請ファイルを参照しておらず、参照先も system 所有ではない (モデレーターが `admin/emoji/update` で差し替えた形)。**まだ誰かの drive 操作で壊れうる** | 絵文字を削除して同じ名前で登録し直す (下記)。差し替え (`admin/emoji/update`) では直らない。バッチは差し替えを巻き戻さないので触らない |
...
**差し替え (`admin/emoji/update`) では直らない。** あちらは渡した drive ファイルの URL を
そのまま絵文字に入れるので、選んだファイルを消せば同じように壊れる。**画像を選ぶ画面
(`drive/files`) には自分の drive のファイルしか出ない**ので、「system 所有のファイルを
選ぶ」という逃げ道も無い。

**直すなら絵文字を削除して同じ名前で登録し直す** (`admin/emoji/add` は #2999 で system
所有へ複製する)。

1. **先に画像を手元へ保存する。** 参照先が申請者など他人の drive ファイルだと、画像を選ぶ
   画面 (`drive/files`) には出てこない。保存元は **`originalUrl`** — `/files/:accessKey` は
...
2. `admin/emoji/delete` で絵文字を消す (**絵文字の行しか消さない**ので元の drive ファイルは
   残る)
3. 保存した画像を自分の drive へ上げ、**同じ名前**で登録し直す

**入れ直しになるのはカテゴリ・エイリアス・ライセンス・センシティブ・ローカル限定・
リアクションに使えるロール。** とくに**センシティブとローカル限定は落とすと緩む向き**なので
先に控えておくこと。絵文字の id も変わるが、ノートやリアクションは名前で引くので表示は戻る。

**申請経由の絵文字では副作用がある。** `emoji_application.emojiId` は古い id を指したままに
なるので、申請者の画面 (`emoji-application/list-mine`) に**「承認後に絵文字が削除されました」**
と出る (絵文字自体は同じ名前で存在する)。次にこのバッチを流すと、その申請は `skipped` に落ちる。

`skipped` だけは exit code に効かない。入るのは「承認後にモデレーターが絵文字を
削除した」形で、守るものがもう無い (**上の手順で登録し直した行もここに落ちる** — 申請が
指している id の絵文字はもう無いため)。

**このバッチが直すのは申請経由の絵文字だけ。** `admin/emoji/add` は #2999 で system 所有へ
複製するようになったが、**それ以前に `add` で登録した絵文字は対象外のまま**。
**`admin/emoji/update` は今も差し替え先の URL をそのまま入れる**ので、この形は画像の
差し替え経由で増えうる (別 issue)。該当するかは以下で分かる (読み取りのみ)。
...
(絵文字を削除して同じ名前で登録し直す)。本番では 0 件 (2026-09-15 実測。このバッチが
