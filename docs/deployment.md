...
```
  ...
  ok    database     接続 ok / migration version 84
  ok    redis        接続 ok
...
mk-goはTS版と同じPostgreSQL/Redisを共有できるため、バイナリの差し替えだけで移行可能。マイグレーションはTS版テーブルに対して原則追加のみだが、例外が 12 件ある ([TS版からの移行](migration-from-ts.md#破壊的なマイグレーション))。
