...
> 混じりうるので、乖離を判断するときは対象の実装を現 pin (`2026.9.0-mk.16`) で確認すること。
...
| config キー | 20 前後 | 0 | — |
| fork frontend の独自変更 | 62 tag (`2026.7.0-mk.0` ～ `2026.9.0-mk.16`) | — | — |
...
**現在の pin は `2026.9.0-mk.16`。** tag 列は「その変更が最初に入った世代」で、
...
| `2026.9.0-mk.16` | リモート絵文字の一覧を media proxy 経由にする (#2957)。カスタム絵文字管理 (beta) のリモート一覧が**1 件も表示されていなかった**。グリッドの image セルは `<img :src="cell.value">` に値をそのまま入れる実装で、渡していたのが相手サーバーの `publicUrl` だったため、`img-src 'self' data: blob:` をenforce している構成で全部ブロックされる (実測: 上位 5 ホストで約 1.1 万件、すべて外部オリジン)。**media proxy は通る** — 外部 3 ホストで実測し、いずれも 200 (allowlist が `emoji.publicUrl` を通す)。**`MkDataCell` は変えない** — 汎用のグリッド部品で image セルは watermark エディタなどでも使うので、一律に proxy へ通すと影響が読めない。呼び出し側で変換する (#2935 と同じ形)。ローカル一覧と登録画面は影響を受けない (URL が自オリジンで `'self'` に入る)。編集ダイアログは #2903 で既に proxy 経由で、**一覧だけが取り残されていた**。**同じ型を 3 回踏んだ**ので (#2903 モーダル / #2935 審査画面 / 本件 一覧)、`TestRemoteMediaGoesThroughProxy` で新規流入を止める。**一般的な判定はできない** — `publicUrl` はローカルなら自オリジン、リモートなら相手のオリジンで静的には区別できないため、#2792 と同じ allowlist 方式にし、通す場合は理由を書く。**mk-go 固有の CSP への適応**であって、純正へ持ち込んでも意味が無い (あちらに CSP は無い)。 |
