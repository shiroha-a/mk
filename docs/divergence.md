...
| config キー | 20 前後 | 0 | — |
| fork frontend の独自変更 | 26 tag (`-mk.0` ～ `-mk.22c`) | — | — |
...
| `2026.7.0-mk.22c` | boot エラー画面の Reload を `addEventListener` にする (#2786)。**inline event handler は CSP の hash では通らない** (`'unsafe-hashes'` が要る) ので、`script-src` から `'unsafe-inline'` を外すと `onclick="location.reload(true);"` が block され、「Failed to initialize Misskey」画面の唯一の復旧手段が押しても反応しなくなる。SPA shell が読む `packages/frontend` 側の inline handler はここ 1 箇所だけ。`packages/frontend-embed/public/loader/boot.js` にも同じ形が残っているが、`/embed/` は frontend CSP の対象外なので今は壊れない — **embed に CSP を広げるならそちらも直す** |
