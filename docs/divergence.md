...
> 混じりうるので、乖離を判断するときは対象の実装を現 pin (`2026.9.0-mk.30`) で確認すること。
...
| config キー | 20 前後 | 0 | — |
| fork frontend の独自変更 | 96 tag (`2026.7.0-mk.0` ～ `2026.9.0-mk.30`) | — | — |
...
**現在の pin は `2026.9.0-mk.30` (`a534709c`)。** tag 列は「その変更が最初に入った世代」で、
...
| drive のメディア処理の同時実行 | **制限なし** (`DriveService` に semaphore が無い) | 画像のデコード / リサイズ / エンコードを同時 `GOMAXPROCS / 2` 本 (最低 1) に制限する。**mk-go 独自の硬化** (#3037)。media proxy (#3032) と同じ理由で、同時に走るデコードの本数がそのまま確保される中間バッファの本数になる。**ffmpeg は枠の外**で回す — 別プロセスなので Go ヒープを使わない一方、1 本で何十秒も回りうるため枠の内側だと動画 1 本で画像処理が全部止まる |
| ffmpeg の実行 | `execa` を **timeout 無し**で呼び、出力も全部ためる (`FileInfoService` / `VideoProcessingService`) | `exec.CommandContext` + 呼び出し元 context + **60 秒の deadline**、併合出力は 64KiB まで、生成されたフレームの読み込みは 32MiB まで。**mk-go 独自の硬化** (#3037)。細工した動画 (壊れた duration / 極端なフレームレート / 巨大な解像度) で ffmpeg は何分でも回り、利用者が接続を切っても止まらなかった |
...
| `drive_file` 行が無い実体の配信 | `FileServerService` が毎回 `resolveFileByAccessKey` を通し、行が無ければ 404 | **同じ (#3037)**。以前は primary が非ローカルのときだけ DB を引き、それ以外は行を確認せず実体を返していた。差が出るのは行を消したのに実体が残っているとき (削除時のオブジェクト削除が失敗した / バケットを他の用途と共有) で、**一度公開した URL を知っている人には削除が効かない**。ローカルストレージ構成でも毎回 1 回引くようになる (非ローカルでは元から引いていた) |
...
| 管理者ロールの付け外し | `assign.ts:69-76` は `canEditMembersByModerator` だけを見る | **管理者しか付け外しできない (#3037)**。`canEditMembersByModerator` は「モデレーターがメンバーを編集してよいか」しか見ないので、管理者ロールにそのチェックが立った瞬間に**モデレーターが自分自身へ付けて昇格できる**。ロールの作成・更新・削除は管理者専用なのに、付け外しだけが抜けていた。unassign も塞ぐ (管理者を降格させて実質の最上位になれるため) |
| 条件つきロールが配れる権限 | target と condFormula の組み合わせを検査しない | **自分で満たせる条件では管理者 / モデレーターを配れない (#3037)**。条件には `isCat` / `isBot` / `isLocked` / `isExplorable` / フォロー数 / 投稿数 といった**本人が切り替えられる・積み上げられる値**が使えるので、そこに `isAdministrator` を立てると「猫と名乗る」だけで誰でも取れるロールになる。管理画面は条件を並べるだけなので、作った側が気付きにくい。本人の操作では変えられない条件 (`isLocal` / `createdMoreThan` / `roleAssignedTo` / `isSuspended`) だけなら通る。既存のロールは作り直すまで動き続ける (黙って降格させない) |
| policy の値の型 | ajv の paramDef は `policies` を `type: 'object'` としか宣言せず、**値の型を見ない** | **既定値の型と突き合わせて弾く (#3037)**。consumer は `if limit, ok := PolicyNumber(v); ok` の形で読むので、数値の policy に文字列が入ると**上限違反で弾かれるのではなく上限そのものが消える**。既定 policy 側は全利用者でその上限が消える。未知のキーは通す (upstream は object lookup なので誰も読まない) |
...
| `Update` の著者照合 | `ApInboxService.update` は object の id host しか見ない | **一致を確認できたときだけ通す (#3037)**。`UpdateRemoteNote` / `UpdateRemoteQuestion` の attribution 検査は「lookup が成功し URI が非 nil で値が違うとき」だけ拒否する形で、**DB 障害 / 行の消失 / URI が NULL のどれでも検査が丸ごと消えて**いた。ここは他人のノート / poll を書き換えられるかを決める唯一の検査で、通れば `upsertEmojis` が使う host まで攻撃者の言い値になる |
...
| `username/available` / `email-address/available` のレート制限 | **無し** (`meta` に `limit` が無い) | **1 分 60 回** (#3037)。どちらも未認証で叩けて、返るのは「使われているか」という真偽値そのもの。止めたいのは 1 件ずつの確認ではなく総当たり — 辞書を回して実在する利用者名の一覧を作る、手持ちのメールアドレス一覧からこのサーバーの登録者を割り出す。**メールアドレスは公開情報ではない**ので後者の方が重い。同梱フロントは入力中に debounce 付きで叩くので 1 分に十数回、実使用は大きく下回る |
...
| drive アップロードの `maxFileSizeMb` | `ApiServerService.ts:52-57` の `limits:{fileSize, files:1}` でパーサ自身に打ち切らせる | **読み切る前に引く (#3037)**。以前は `Upload` の中で評価しており、そこへ届く時点で本体は全部メモリに載っていた。body limit は `config.maxFileSize` (既定 250MB) なので、**30MB しか保存できない利用者が 250MB を確保させられた**。`FileHeader.Size` が上限を超えていれば本体を開かず、実際に読むときも `上限 + 1` で打ち切る |
