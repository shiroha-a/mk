...
| chat room の `Group.id` の host | **対応物なし** (upstream に inbound の chat room 連合が無い。`ApInboxService` は `Invite` を扱わない) | **送信元の actor と突き合わせる**。管理していない host の room URI を名乗ってローカルの id 空間に room を作れる (`chat_room` に host 列が無いので、攻撃者が**自分の host で**任意 id を先取りする形は残る) |
...
| `specified` ノートの `Delete` / `Update(Question)` の配送先 | フォロワーへも配る (`Update(Question)` は加えて relay へも) | **宛先とメンション先だけに送る** (`Create` と同じ集合)。宛先には直接届くので届く相手は減らない。アンケートは投票のたびに `Update` が出るので、可視性を見ないと本文が全フォロワーへ渡る |
| nodeinfo discovery の `links[].href` | 検証なし | **取得元の host に縛り、host を跨ぐ redirect は追従しない**。href の検証だけだと 302 一回で任意の host へ抜けられる (最終 URL を見て本文を捨てるだけでは GET 自体は出てしまうので、`CheckRedirect` で止める)。返る JSON は content-type も見ずに instance 行へ書き戻るため |
