...
| `admin/emoji/add` の `fileId` が指す画像 | **操作者の drive ファイルをそのまま参照する。** `add.ts` は `driveFile.url` を `originalUrl` に入れるだけ | **system 所有の drive ファイルへ複製してから参照する** (#2999)。`fileId` に来るのはふつうモデレーター自身が直前に上げたファイルで、**本人が drive から消せば絵文字の画像が壊れる** (ロールの変更や退会でも同じ)。申請の承認は #2966、リモートの複製と zip の取り込みは #670 で既に system 所有にしてある。**`admin/emoji/update` の `fileId` は未対応** — 画像を差し替えると再び利用者所有のファイルを指すので、この形は `update` 経由で今も増える (別 issue)。**複製は `CopyToSystemFile`** — 自分の公開 URL を HTTP で叩くのは SSRF ガードと衝突し、非公開 URL の構成では取れない。**元のファイルは触らない** (所有権を移すとノートの添付やプロフィールで使っているファイルが利用者の drive から突然消える)。**複製は検証をすべて通した後** (先にやると重複名や MIME で弾く分まで実体を作って捨てる。upstream の `copy.ts` がその形で、`DUPLICATE_NAME` のたびに孤児を残す)。**複製した実体の MIME をもう一度見る** — `Upload` は `AnalyseFile` でバイト列から型を引き直すので、行の宣言と実体がずれていると allowlist 外が通る。**弾いたときと `Create` が失敗したときは複製を消す** (誰からも参照されない孤児になる)。**複製の失敗は種別ごとに分ける** (#2792) — 実体がもう無い (400 `NO_SUCH_FILE`) / 複製の上限 32 MiB を超える (400 `EMOJI_IMAGE_TOO_LARGE`) / ストレージ・DB の障害 (500) で、**どれも承認経路と同じ error id**。**行の `size` で先に断る** — 上限超過を読み切ってから 400 にするのは無駄なので、実体を読む前に弾く。**発火するのは role policy の `maxFileSizeMb` を 32 MiB より上へ設定した (ことがある) 構成だけ** — 既定は 30 MB で upload できた時点で必ず上限内だが、一度上げて入れた行は上限を戻しても残る。**一括登録の同時実行 (同梱 frontend は最大 100 件を一度に投げる) はこれでは軽くならない**。`size` は行に書いてあるだけで実体とずれうるので、**過小申告は読み出し側が捕まえ、過大申告はここで確定する**。**登録できなくなる入力が増える** — 上限超過 (drive が受け取る上限 = role policy の `maxFileSizeMb` は 32 MiB より上へ設定できる) と、実体を持たない行 (`isLink` / `accessKey` 無し / ストレージから消えた) は、**管理画面には見えているのに 400** になる。**元が既に system 所有 (`userId` / `userHost` 両方 NULL) なら複製しない** — 判定は孤児 cleanup の guard (`orphanWhere`) と同じ条件で、`admin/emoji/copy` や import-zip が作った画像を二重に持たないため。**利用者から見た削除の意味は変わる** — 他人のファイルを `fileId` に渡せる (upstream も所有者を見ない) ので、所有者が drive から消してもコピーは絵文字として残る。**`url` 直接指定の legacy 経路 (mk-go 独自) は取り込まない** — あちらの意味は「この URL を指す」で drive に行の無い外部 URL も指せる escape hatch、かつ取り込みには HTTP が要る。直している失敗形は「利用者が自分の drive ファイルを消す」で、`url` 経路には所有者の概念が無い。**配線しない構成 (`emojiImageFetcher` 未配線) では従来どおり drive の URL をそのまま使う** — 起動時の critical wiring チェックが知らせる |
