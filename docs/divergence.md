...
| media proxy の pack 時適用範囲 | `DriveFileEntityService.getPublicUrl` などが remote の drive file を proxy する一方、**ロール / アバターデコレーション / 通知 icon / feed / SSR OGP / system account は raw** を返す | **閲覧者のブラウザが remote origin を直接取りに行く経路を塞ぐ (#1529)**。ロール `iconUrl` (公開 `roles/*` と roleAssigned 通知。admin/roles/show は編集フォームが読んだ値をそのまま書き戻すため raw 据え置き)、アバターデコレーション `url` (pack 時 + 公開カタログ picker)、通知 `icon` / アクセストークン `iconUrl` (利用者由来は期限付き署名。上記行)、reversi の WebSocket stream payload の avatar (REST は proxy 済みで stream だけ漏れていた)、RSS/Atom/JSON feed の avatar、お知らせ SSR の og:image、ローカル system account の `meta.iconUrl` 由来 avatar。**意図的に raw のまま**にするのは広告 `imageUrl` (upstream も server/client で raw。ただし mk-go の CSP enforce では remote 画像の広告は表示されない)、admin 専用の avatar-decorations 作成/一覧/更新、`/api/meta` のブランディング URL、ActivityPub outbound。admin の応答を raw に保つのは、同梱 frontend の編集フォームが読んだ値をそのまま保存する経路があるため (`admin/roles/show` → `admin/roles/update`、`pages/admin/roles.edit.vue`) |
