...
| アンテナの `userListId` (`antennas/update` と照合) | **「新 src か既存 src が list」で、かつ `userListId` を送ったときだけ**所有者を検証する。`userListId` を送らずに src だけ `list` へ切り替えると、保存済みの値は検証されない (upstream は検証済みの値しか保存しないので、実際に他人の list が残るのは直接 DB を触ったときに限られる)。`userListId` を送らない更新では既存値をそのまま残す。照合 (`AntennaService`) は list の所有者を見ない | **更新後の src が `list` なら、更新後に参照する `userListId` (送られた値、無ければ既存値) を毎回検証する** (`NO_SUCH_USER_LIST`)。mk-go は以前 `src` が list 以外のときも送られた `userListId` を未検証のまま保存しており、src だけを後から list へ切り替えると他人の list をアンテナのソースにできた。**更新後の src が `list` 以外なら `userListId` は null にする** (送られた値も既存値も残さない)。**照合時にも list の所有者 == アンテナの所有者を確かめる** — 修正前に保存された他人の `userListId` が残っていても、その list のメンバー構成をアンテナ経由で覗けないようにする (既存データの救済) |
