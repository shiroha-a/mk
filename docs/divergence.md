...
| アニメーション画像の webpublic | **作らない** (= 他人に見せる側も原本のまま) — `encodeWebP` は 1 枚しか受けないので、作ると 1 コマの静止画になる。upstream も `isAnimated` なら作らない (`DriveService.ts`) が、**判定の仕方が違う**。upstream は `metadata.pages > 1` という中身の判定で、mk-go は MIME 判定 (`isAnimatedMime` は GIF と APNG だけ)。このため**乖離が 2 方向に出る**: (a) **メタデータを持つ APNG には作る** — アニメーションを保つために撮影情報を残すのは割に合わないので、upstream より厳しい側に倒している (GIF は `hasStrippableMetadata` の対象外なのでこの枝に入らない)。(b) **animated WebP / AVIF は拾えず静止画になる** — 前者は ANIM チャンクを読まないと分からず、後者は「AVIF は必ず webpublic を作る」枝があるため寸法にもメタデータにも関係なく潰れる。中身で判定するには形式ごとの走査が要る |
