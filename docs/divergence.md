...
| HEIC / HEIF の代替画像 | `sharp-convertible-image` に **HEIC が無い** (`is-mime-image.ts:10`) ので webpublic も thumbnail も作られず、原本が公開 URL になる | **画像として扱い、webpublic を必ず作る (#3038)**。mk-go は media proxy 側で既に `gen2brain/heic` を使っており (`isConvertibleImage` に入っている)、**drive だけが取り残されていた**。作られないと `GetPublicURL` が原本へ落ちるので **EXIF の GPS がそのまま公開される** — しかも HEIC は iPhone の既定形式で位置情報を持つのが普通なので、AVIF / WebP で塞いだものより条件が緩い (サイズの閾値すら無く全件が対象だった)。upstream と同じ状態だが厳しい側へ倒す。**原本を出せない画像は webpublic を作れなければアップロードごと拒否する** — 対象は「落とすメタデータを持つもの」と「原本がブラウザで開けないもの」(AVIF / HEIC / HEIF) の 2 つで、落とすものが無く原本をそのまま配ってよい画像は従来どおり通る。画像プロセッサを配線していない構成では見ない (そこは代替画像の生成自体を行わない運用なので、断ると画像が 1 枚も上げられなくなる) |
...
| drive のメディア処理の同時実行 | **制限なし** (`DriveService` に semaphore が無い) | 画像のデコード / リサイズ / エンコードを同時 `GOMAXPROCS / 2` 本 (最低 1) に制限する。**mk-go 独自の硬化** (#3037)。media proxy (#3032) と同じ理由で、同時に走るデコードの本数がそのまま確保される中間バッファの本数になる。**ffmpeg は枠の外**で回す — 別プロセスなので Go ヒープを使わない一方、1 本で何十秒も回りうるため枠の内側だと動画 1 本で画像処理が全部止まる。**1 枠あたりの最悪値を 512MiB に固定した (#3038)。** 以前は上限が無く、drive が渡す cap (268MP) をバイトに読み替えた **1.07GB** が 1 枠の最悪値だった — 16383x16383 の全画素ゼロ PNG は **764 KiB** で作れて 268MP にちょうど一致するため上限を通り抜け、8 core / 32GB の実測で `processImage` の 4 段を通すと**ピーク RSS 2.23 GiB / 13.6 秒**になる。`GOMAXPROCS / 2 = 1` の 2GB VPS はその 1 本で落ちる。宣言寸法の cap とは**別にバイト予算を持たせて** 512MiB (8bit で約 134MP) にしたので、「枠の本数 × 512MiB」が実際に同時確保量の上限になる。寸法の cap を下げないのは 102MP の実写真 (388MiB) がサムネイルと webpublic を失うのを避けるため。**グレースケールとパレットも 4 バイト/画素で見積もる** — デコード直後は 1 バイト/画素でも `resizeFit` / `imaging.Fill` が NRGBA へ変換するのでピークは 4 倍になる。予算を超えた入力は「webpublic を作れなかった」経路に入ってアップロードごと拒否される (原本は公開側へ出ない)。`GOMEMLIMIT` の設定はリポジトリに無い |
