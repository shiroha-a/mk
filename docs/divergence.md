...
- 基準: **mk-go 1.4.0** ⇔ Misskey TS `2026.9.1`
- 最終更新: 2026-09-23
...
|---|---|---|---|
| API endpoint | GET variant 23 + alias 4 + 分割アップロード 4 + 承認制 7 + 絵文字の申請 10 + exact assignment lookup 2 + admin 観測 6 + IP 検索 3 | chat 15 | **0** |
...
| IP からの関連アカウント検索 | 3 | `admin/ip/accounts` (#3104 / 親 #3066)。**upstream は逆向きしか持たない** — `admin/get-user-ips` は「この利用者が使った IP」で、荒らしの使い捨てアカウントを追うときに要る「この IP を使った利用者」は DB を直接引くしかなかった。正規化した IP と完全一致で引き、候補ごとに初回観測・最終観測・観測回数を返す。**権限は 3 段** — `RequireModerator` + ロールポリシー `canSearchIpHistory` (**既定 false**) + scope `read:admin:user-ips` (upstream の `admin/get-user-ips` と同じものを再利用する。misskey-js と完全一致させる契約なので新しい scope を足さない)。**既定でモデレーターに開かない**のは、upstream の `admin/get-user-ips` が `requireAdmin: true` で、同じ「利用者 ↔ IP の対応」を扱うため。既定で開くと同じ機密情報に既存より緩い経路を新設することになる。モデレーターへ開きたい運営者はロールで `canSearchIpHistory` を true にする。**`privilegedPolicyKeys` には載せていない** — あれは policy 単独で `/admin/*` を開ける key の一覧で、この route は `RequireModerator` を併用するので条件つきロールの自己付与では昇格できない (`checkConditionalPrivilege` が条件つきロールでの `isModerator` 付与を塞ぐ)。ただし**運営者が作った条件つきロールで既存のモデレーターに配ることはできる**ので、「既定は管理者のみ」は運営者が変えていない前提の話。**`perm-check` は検出しない** — あちらは upstream の golden に載る endpoint しか見ないので、mk-go 独自の endpoint の認証段は `wiring-check` (`TestIPAccountSearchRouteIsRegistered` / `TestIPRelatedAccountsRouteIsRegistered`) でしか固定されていない。**`loggingEnabled` と `hasAnyHistory` を別々に返す** — 記録を無効にした後も残っている記録は引けるので (#3066 §7)、単一の状態に潰すと「一致なし」と「そもそも記録が無い」を言い分けられない。**DB 障害はすべて 500** で、空の結果に倒さない (#2792) — 「その IP を使ったアカウントは無い」という誤った事実になり、調査の結論を反転させる。`retentionDays` (90) も返す — 画面が保持期間を決め打ちすると、サーバーが変えたときに黙って嘘になる。定義は `internal/core/iplog` の `Retention` 1 箇所で、日次の掃除もそこを見る。**利用者の行が引けない観測は候補にしない** (`user_ip.userId` には **FK が無い** — upstream の `MiUserIp` にも relation が無く migration も制約を作らないので、アカウントを完全削除しても行が残り、消えるのは 90 日の掃除が来たとき)。**そのぶん `accounts` は `limit` より短くなり、1 ページが丸ごと落ちると空になる。** **`hasMore` はこのとき true とは限らない** — あれは「引けた行が `limit` を超えたか」で決まるので、行数が `limit` 以下で全件落ちると `accounts: []` + `hasMore: false` になり、**「一致する行が 1 件も無い」ときとバイト単位で同じ応答**になる。そこで**落とした件数を `droppedCount` で返す。** これが無いと画面は「0 行ヒット」と「N 行ヒットして全部落とした」を区別できず、**記録が残っているのに「この IP からの接続は記録されていません」と断定する** — しかもそれは荒らしの使い捨てアカウントが消された後 = この機能が要る場面そのもので、件数はたいてい `limit` 未満なので `hasMore` では分けられない。読む側は `accounts` が空でも `droppedCount > 0` なら「記録はあるが候補が消えている」と読み、`hasMore` が立っていれば `offset += limit` で次を引くこと。**`droppedCount` はページごとの値**なので、複数ページを 1 つの一覧として見せる画面は累積して持つこと (最新ページの値だけを出すと、落ちの無いページを引いた時点で「落としたものは無い」という新しい嘘になる)。**ページングは offset なので取りこぼしうる** — 並びの先頭が最終観測なので、ページを送っている間にその IP から新しい接続があると当該行が前のページ側へ移り、既に読んだ範囲に入って二度と出てこない (逆向きの重複は同梱フロントエンドが `userId` で落とす)。**落ちる向きに偏りがある** (今まさに接続している候補ほど落ちる) ので、画面は注釈でそれを明示する。keyset ページングにするなら #3105 の並び替えと一緒に。**同じ IP を使ったことは同一人物であることを意味しない** (家庭・職場・学校・公衆 Wi-Fi・携帯回線の CGNAT・VPN) ので、返すのは調査の候補とその根拠であって判定ではなく、自動判定にも自動処分にも使わない。フロントは注意書きを結果の有無に関わらず常設する。`tests/diff/test_endpoints.py` の ignore-list にも登録済み (policy は `/api/meta` と `users/show` の両方に出る)。**`admin/ip/related-accounts` は逆に利用者を起点にする (#3105)** — 対象が窓の中で使った IP を起点に、同じ IP を使った**他の**ローカルアカウントを候補として返す。権限は `accounts` と同じ 3 段で、**同じ policy と scope を再利用する** (扱うのは同じ「利用者 ↔ IP の対応」なので、別 policy にすると片方だけ開けてしまう)。順位は**時間減衰付きのスコア**で決める — 半減期 30 日、`t(p) = max(0, now - min(対象の最終観測, 候補の最終観測))`、`w(p) = 2^(-t/H)`、`score = Σ w(p)` (重複を除いた共有 IP ごと)。**古い側からの経過時間を使う**のは、一方が最近その IP を使っただけで、もう一方の古い一致まで新しい一致として扱わないため。**同じ IP は 2 回数えない** (観測回数では加点しない) し、**共有 IP 数で乗算もしない** (数は和に既に効いているので二重になる)。**`score` は確率ではない** — 重みの単純和なので上限は 1 ではなく、今日使った IP を 3 つ共有していれば 3.0 になる。順位を決めるためだけの値で、パーセントとして出さないこと。`now` は 1 回の検索で固定する (呼ぶたびに取り直すと、同じ検索の中で先に計算した候補ほど減衰が浅くなる)。同点は「共有 IP 数 → **候補側の**最新の観測 → `userId` 昇順」で決定的に並べる (対象側の観測を見ると、対象がその IP を最近使ったという理由だけで候補の順位が上がり、排除したはずの形を tiebreak の粒度で再導入する) (浮動小数の和なので厳密な同点は普通に起きる)。候補ごとに**共有した各 IP と双方の最終観測**を返すが、**「同時に使っていた」とは書かない** (一致していなければ両方の時刻が並ぶだけ)。各 IP について**窓の中でその IP を使ったアカウント数**も返す — 共有回線かを読む側が判断する材料で、数が大きいほど一致は弱い。**走査そのものを有界にしてある** — 起点の IP は最終観測の新しい順に 50 本、1 つの IP から取る候補は 200 件まで (+1 件は「まだ他にも居る」の判定用)。CGNAT や公衆 Wi-Fi の IP は 1 つで数千アカウントが載りうるので、上限が無いと利用者 1 人を指定するだけで無制限の走査を外から回せる。**「読んでから絞る」形では有界にならない** — `COUNT(*) OVER (PARTITION BY ip)` で数える形は `ROW_NUMBER` の絞りより前にその IP の全行を読んでソートするので、分布に関係なく全行読む (実測 200 万行の構成で 6,020 ms、temp に 176 MB)。LATERAL + per-IP LIMIT を `(ip, lastSeenAt DESC, userId)` の index に完全に乗せると 0.1-0.2 ms。**そのぶん「その IP を使ったアカウント数」は正確な数ではない** — 数えるには全行読む必要があるので、上限まで見えたときは `ipAccountCountIsLowerBound` を立てて下限として返す (共有回線かの判断にはそれで足りる)。**打ち切ったら `truncated` を返し、起点を切ったのか候補を切ったのかを `targetIpsTruncated` / `candidatesTruncated` で分ける** (前者は期間を絞れば絞り込めるが後者はどうにもならないので、1 つに潰すと画面が原因を誤って説明する。**両方立つことがある**ので、画面は片方に潰さず独立に出すこと)。**打ち切った検索からは何も断定しない** — 「一致なし」も「候補はすべて削除済み」も言えない (見たのは全体の一部)。**重みそのものは wire に出さない** — [0,1] に収まるのでパーセントと見分けが付かない。代わりに減衰に使った経過日数 (`elapsedDays`) を返し、`halfLifeDays` と合わせて再現させる。**リモート利用者は引く前に 400** — `user_ip` は自インスタンスで認証を通した利用者しか書かないので必ず「候補なし」になり、それを「関連が無い」と読ませない。**`accounts` と同じ落とし穴がそのままある** — `hasMore` は利用者を解決する前の件数で決まり、`droppedCount` はページごとの値 (複数ページを 1 つの一覧として見せる画面は累積して持つこと)、offset ページングなのでページを送る間に順位が動くと候補を取りこぼしうる。**`totalCount` は返さない** — 表示できない候補 (アカウントが完全削除されたもの) を含む数になり、最後までページを送っても届かない数字を画面に出すことになる。**`admin/ip/lookup-log` は照会そのものの監査記録を返す (#3106)。** 誰が・いつ・何を・どの期間で引いて・何件返したかだけを持ち、**結果は残さない** — 候補のアカウントや一致した IP を書くと、この表が第 2 の「IP とアカウントの対応」になる。**`moderation_log` には入れない** — あちらは保持期間を持たず永久に残るのに、ここに書くのは照会に使った IP そのもので、`moderation_log` 全体に保持期間を入れると無関係な記録まで消える。専用テーブル `ip_lookup_log` (migration 000096) にして日次の掃除で 90 日で刈る。**読み取りも照会と同じ 3 段で守る** — この応答自体が「誰がどの IP を引いたか」の一覧そのものなので、ここが緩むと「IP を引く権限は無いが、誰がどの IP を引いたかは読める」状態になる。**記録に失敗しても照会は成立させる** (監査の障害が調査そのものを止めないため。代わりに Error でログに残す)。**弾いた照会は記録しない** (引いていないので残す事実が無く、残すと「照会した」件数が実態と合わなくなる)。`ip_lookup_log.userId` に FK は無く、**退会しても記録は残る** (監査の目的からしてそれが正しい。読み取り側は利用者を null で返し、id は残す)。**3 本とも `Cache-Control: no-store` を付ける** (#3066 §8) — POST + Authorization なので既定でも共有キャッシュは保存しないが、明示しないと検証できず、中間にキャッシュを置く構成や将来 GET 版を足したときに黙って漏れる。IP は body に載るので URL には元から残らない。**upstream の `admin/get-user-ips` も監査と `no-store` の対象にしてある** — 返すのは同じ「利用者 ↔ IP の対応」で管理者なら誰でも叩けるので、外すと**監査を迂回して同じものを引ける**。(`kind` は `userIps`、窓を取らない口なので `sinceDays` は 0)。**`admin/show-user` が返す `signins` も同じ policy で守る** (#3114) — あちらは `signin` テーブル (`user_ip` とは別) の**ログインのたびの IP** で、upstream は policy を見ずに全件返す。mk-go は `canSearchIpHistory` を持つ相手にだけ返し、返したときだけ記録する (`kind` は `signins`、`sinceDays` は 0)。**伏せるときは `ip` だけでなく `headers` も空にする** — 本番構成の nginx が `X-Real-IP` / `X-Forwarded-For` を必ず付けるので、`ip` だけ潰してもキー 1 つ隣で同じ IP が読める。**揃っているのは policy 段だけ** — scope は `read:admin:show-user` のままでレート制限も件数の上限も無く、`signin` には保持期間も無いので、`user_ip` が 90 日で消えた後も同じ時期のログイン IP は残る (docs/deployment.md に表で併記) (`kind` は `userIps`、窓を取らない口なので `sinceDays` は 0)。**監査の一覧を読んだことは記録しない** — 監査ログの閲覧を監査し続けると際限が無いため。**`admin/drive/show-file` が返す `requestIp` / `requestHeaders` も同じ policy で守る** — upstream はモデレーターなら返すが、mk-go はそこだけ素通しだと `admin/ip/*` の 3 段 gate を迂回して同じ「利用者 ↔ IP の対応」を引ける (ファイル 1 つにつき 1 件ずつだが、一覧から辿れば同じこと)。**伏せるときは `requestHeaders` も空にする** — 本番構成の nginx が `X-Real-IP` / `X-Forwarded-For` を必ず付けるので、`requestIp` だけ潰してもキー 1 つ隣で同じ IP が読める。**こちらは監査に残さない** (`admin/ip/*` と違い「IP を引く」ことが目的の口ではなく、ファイルの詳細を開いた副産物なので、記録すると監査が実質的に全ドライブ操作のログになる)。**レート制限は 4 本とも 1 時間 120 回で、利用者単位だけで数える** (`UserBucketOnly`) — upstream に対応する制限は無い。**IP でも数えない**のは、レート制限が route の権限検査より前に走るため: 両方見ると 403 になる相手でも枠を消費し、同じ出口 IP から未認証で叩き続けるだけで正当なモデレーターを窓のあいだ締め出せる (しかもこの 429 は `ip_lookup_log` にもサーバーログにも残らない)。守りたいのは認証済みの利用者による濫用で、そちらは利用者単位の枠が押さえる。`DefaultEndpointLimits` に無い endpoint は素通りするので、route を rename すると**上限が黙って消える** (`TestIPLookupRoutesHaveRateLimits` が router.go の path と表のキーを突き合わせて固定している)。調査中は続けて叩くので短い窓では止めず、時間あたりで抑える。**`admin/ip/*` の 3 本には DB 側にも 10 秒の `statement_timeout` を掛ける** (`admin/get-user-ips` は 1 利用者の最新 30 件を index で引くだけなので対象外) — 走査の上限 (起点 50 本 × 候補 200 件) は「返す量」を決めるもので、こちらは「その上限の中でも遅いときに何秒で諦めるか」。index が効かない構成・想定外の分布・DB 側の競合では有界な問い合わせでも待たされる。**当たったら 500** で、途中までの結果は返さない (「これで全部」と読まれる)。`SET LOCAL` なのでトランザクション限りで、接続プールを通じて他のクエリには漏れない |
...
| `/api/roles/*` / `admin/roles/*` / `/api/meta` / `users/show` の `policies` | `canSearchIpHistory` | IP アドレスから関連するローカルアカウントを引けるか (#3104)。upstream の `admin/get-user-ips` は `requireAdmin: true` で policy を持たないため TS 側に対応キーが無い。**既定は false = 管理者のみ。** 同じ「利用者 ↔ IP の対応」に、既存 (`admin/get-user-ips`) より緩い経路を既定で開かないため。モデレーターに許可するかは運営者がロールで決める。**`HasRolePolicy` は管理者を短絡するが `GetUserPolicies` はしない**ので、`policies.canSearchIpHistory` は管理者でも false のまま返る — 画面の出し分けは `$i.isAdmin \|\| policies.canSearchIpHistory` で書くこと (policy だけで絞ると既定の構成でメニューが誰にも出ない)。**モデレーターは短絡しない** — route 側の `RequireModerator` を通っても policy が false なら 403。ロール編集画面で列挙が要るのは 3 箇所 (`roles.editor.vue` のキー一覧、`roles.policy-editor.vue` のキー一覧と編集フォーム) で、`TestMkGoRolePolicyKeysAreListedInFrontend` が固定する。`tests/diff/test_endpoints.py` の ignore-list にも登録済み |
...
| `admin/queue/jobs` | `limit` / `page` | 1 ページの件数 (既定 30、上限 100) とページ番号。**指定したときだけ効く** (`limit: 0` は省略と区別できないので省略扱い) — 省略時は upstream と同じく state ごとに新しい順で最大 101 件 (`getJobs(types, 0, 100)` は終端を含む) を state の指定順に連結して返す (#3167)。指定時は**全 state 合計**で `limit` 件に切る (state を順に詰めるので、後ろの state は前が埋めると出ない)。同梱フロントは指定しない。以前は省略時にも既定の 30 件が合計に効き、しかも古い順だったので、「すべて」タブが古い完了済みだけで埋まって失敗が 1 件も見えなかった |
...
| 転送 activity の LD-Signature 検証と compact | `core/federation/ld_signature_verifier.go` / `queue/processors/inbox.go` (`authorizeActor`) | **upstream と同等** (`InboxProcessorService.process`)。HTTP 署名者と `actor` が食い違う転送経路では、`signature` を外して upstream と同じ `CONTEXT` (`ld.InboxCompactContext`) へ compact し、**compact 後の文書で検証して、その文書を handler へ渡す**。以前の mk-go は compact せず生 body を検証・処理していたため、**署名が覆っていないキーを転送者が足せた** — AS2 context は `"@vocab": "_:"` なので、context で定義されていない語 (Mastodon の文書に足した `_misskey_content` / `_misskey_summary` / `quoteUrl` など) は URDNA2015 で blank node 述語になって捨てられ、署名に含まれない。handler は生の JSON キーを読むので、Mastodon 利用者の署名付き Create / Update に `_misskey_content` を足して自分の HTTP 署名で転送するだけで、**その利用者名義のノート本文を偽造・書き換えできた**。compact 後の文書では署名外の述語は `_:<name>` というキーで残り、handler からは見えない。compact は RDF として同値な変形なので、検証を compact 後の文書に掛けることで「渡す文書 = 署名された内容」を検証そのものが保証する。**upstream と同じく、CONTEXT に無い語は完全 IRI で残って読めなくなる** (Mastodon の `toot:blurhash` / `toot:focalPoint` など。mk-go の送信用 context が `misskey:quoteUrl` に割り当てている `quoteUrl` も、upstream の `as:quoteUrl` とは別 IRI なので残らない — 引用は `_misskey_quote` でも届く)。**通常経路 (署名者 == actor) は生 body のまま** — HTTP 署名の digest が body 全体を覆うので不要で、upstream もこの経路では compact しない。**forbidden directive の検査は compact 後の文書にも掛ける** — 生の文書の検査はキー名しか見ないので、inline context で `"g": "@graph"` / `"inc": "@included"` と別名を付けると素通りするが、compact がそれを `@graph` / `@included` に戻す (upstream も compact 後に検査する)。**HTTP 署名ヘッダの無い legacy 経路も、LD-Signature を検証したら compact 後の文書を渡す** (mk-go 独自の経路で upstream に対応物は無い。本番の inbox handler は Signature ヘッダを必須にして Headers を詰めるので到達しないが、検証済みとして扱う以上は生 body を渡さない)。**残る差 (#2106 L49)**: mk-go の loader は preload 済みの 3 context (AS2 / security v1 / identity v1) しか解決せず fetch しないので、それ以外の remote context を参照する転送 activity は compact 段で `ErrCacheFrozen` になり拒否される (upstream は fetch して受理する)。あわせて freeze を compact の**前**に置いている (upstream は compact の後。fetch しない以上結果は同じで、将来 fetch を足したときに compact が無防備に fetch しないようにするため) |
...
| `2026.9.0-mk.33` | IP アドレスから関連アカウントを探せるようにする (#3104、親 #3066)。管理画面に「IP アドレスから探す」(`/admin/ip-search`) を足し、mk-go 独自の `admin/ip/accounts` を叩く。**出るのは調査の候補であって判定ではない** ので、「同じ IP を使ったことは同一人物であることを意味しない」という注意書きを結果の有無に関わらず常設する。メニューの出し分けは `$i.isAdmin \|\| $i.policies.canSearchIpHistory` — **`HasRolePolicy` は管理者を短絡するが `GetUserPolicies` はしない**ので、policy だけで絞ると既定 (= 管理者のみ) の構成でメニューが誰にも出ない。ロールポリシー編集画面の `canSearchIpHistory` は mk-go 固有 policy なので**列挙が要るのは 3 箇所** (`roles.editor.vue` のキー一覧、`roles.policy-editor.vue` のキー一覧と編集フォーム)。**「記録が無効」「記録が 1 件も無い」「この期間に一致が無い」を言い分ける** (#3066 §7) — 記録を無効にした後も残っている記録は引けるので、単一の状態に潰さない。**古い応答は世代で捨てる** — 「さらに表示」の最中もフォームは生きているので、捨てないと「検索したアドレス」と一覧の中身が食い違い、次のページ送りが別の結果集合から採った offset を渡して以降の候補が出てこなくなる (`admin-user.emoji-applications.vue` が mk.29a で同じ形を解いている)。**次の offset は `offset + limit`** — サーバーは `limit` 行引いてから利用者を解決できない観測を落とすので、表示件数を offset にすると落とした分を読み飛ばす。**`hasMore` が立っているときは「記録されていません」と断定しない** (1 ページが丸ごと削除済みアカウントだと `accounts` が空になる) — ただし**これは半分しか塞げておらず、`2026.9.0-mk.33a` で `droppedCount` による判定へ改めた** (下の行)。**権限不足 (403) を「サーバーのログを確認してください」にしない** — ポリシーを持たないモデレーターは URL 直打ちでこの画面を開けるが、その 403 はサーバーのログに何も残さないので本人に手の打ちようがない案内になる。**保持期間はサーバーが返す値を使う** (画面で決め打ちすると、サーバーが変えたときに黙って嘘になる)。**locale は `ja-JP.yml` だけ** — `locales/README.md` と `scripts/check-shipping.mjs` が他の locale の手編集を禁じており、欠けたキーは ja-JP にフォールバックする。**純正へは還元できない行** (純正 backend にこの endpoint と policy が無い)。 |
| `2026.9.0-mk.33a` | 候補が全員消えているのを「記録がない」と言わない (#3104 のレビュー 2 周目)。`mk.33` は「`hasMore` が立っているときは断定しない」形だったが、**`hasMore` は「引けた行が `limit` を超えたか」で決まる**ので、行数が `limit` 以下で全件が解決不能だと `accounts` が空・`hasMore` が false になり、「一致する行が 1 件も無い」ときと区別が付かず「このIPアドレスからの接続は記録されていません」と断定していた。**荒らしの使い捨てアカウントを消した後にその IP を引く**という、この機能が要る場面そのもので踏む(`user_ip` に FK が無いので行だけ 90 日残り、件数はたいてい `limit` 未満)。サーバーが `droppedCount` を返すようにして、判定をそちらへ移した。**候補が出ているときも落とした件数を添える** (黙って減らすと「これで全部」と読まれる)。あわせて、送信ボタンの `disabled` から `loadingMore` を外し (`misskeyApi` に中断もタイムアウトも無いので、ページングが滞留している間に新しい検索へ逃げる手段が無かった。競合は世代ガードだけで処理できる)、代わりに入力が空のあいだ押せなくした。通信失敗と `offset` の打ち止めを「サーバーのログを確認してください」から分けた (どちらもサーバーのログには何も残らない)。期間セレクトに「最終観測がこの期間に入る利用者を出します」を常設 (SQL の条件は `lastSeenAt` なので「直近 7 日」の結果に「最初の観測: 100 日前」が並ぶ)。読み上げ用の live region を常設にした (中身と一緒に `v-if` で挿入すると初回の結果は読み上げられない)。区切り線を `.row + .row` に (`:last-child` はこの div の最後の子 = 注釈を指すので当たらない)。`search` / `loadMore` / `lastActiveDate` はルートの既存キーを使う。**純正へは還元できない行** (純正 backend にこの endpoint が無い)。 |
| `2026.9.0-mk.33b` | 落とした件数を累積で持ち、判定をテストできる形に出す (#3104 のレビュー 3 周目)。`mk.33a` で足した「落とした件数」の注釈が**最新ページの値しか見ていなかった** — 一覧は全ページの累積なので、落ちの無いページを引いた時点で注釈が消え、「落としたものは無い」という新しい嘘になる (page1 で 3 件落ちたまま page2 が 0 件なら、3 件は一覧に無いのに注釈だけ消える)。過少申告も同じ。`droppedCount` と `hasAnyHistory` を累積で持つようにした。**判定は `@/utility/ip-search-result.js` へ出して `test/unit/` で表にした** — この分岐は 2 度続けて間違えており (`mk.33` は `hasMore` で分けて最後のページに嘘が残り、`mk.33a` は per-page の `droppedCount` を見て累積の一覧と食い違った)、どちらも読解でしか見つかっていない。分岐そのものが機能の本体なので、純粋関数にして 8 形の変異で検出を実測した。あわせて読み上げ用の live region が画面と同じことを言うようにし (「候補は見つかりませんでした」だけだと、「記録はあるが候補が全員消えている」というこの機能でいちばん重要な区別が視覚表示にしか無い)、サインイン切れ (`CREDENTIAL_REQUIRED`) も「サーバーのログを確認してください」から分けた (あれもサーバーには何も残らない)。**純正へは還元できない行** (純正 backend にこの endpoint が無い)。 |
| `2026.9.0-mk.34` | 関連アカウント候補をユーザーモデレーション画面に出す (#3105、親 #3066)。対象ユーザーが期間内に使った IP を起点に、同じ IP を使った**他の**ローカルアカウントを候補として並べる (mk-go 独自の `admin/ip/related-accounts`)。**自動では引かない** — 機微な照会なので、押されたときだけ実行する。**スコアは出さない** — 重みの単純和なので上限は 1 ではなくパーセントとして読めない。数値を出すと「関連度 87%」のように読まれる。出すのは順位と根拠 (共有した各 IP、双方の最終観測、その IP を使ったアカウント数) のほうで、**「同時に使っていた」とは書かない** (双方の最終観測を並べて出すだけ)。タブの出し分けは `iAmAdmin \|\| $i.policies.canSearchIpHistory` (`admin/index.vue` と同じ理由)。リモートとシステムアカウントは `user_ip` に行を持たないので、そもそも出さない。**打ち切りは黙らない** — 立っているときは順位もスコアも下限で、続きを読むと順位が入れ替わりうる。「候補なし」「記録が無い」「候補が全員消えている」の言い分けは `@/utility/ip-search-result.js` を #3104 と共有する (同じ構造なので 2 箇所に書くと片方だけ間違える)。**純正へは還元できない行** (純正 backend にこの endpoint が無い)。 |
| `2026.9.0-mk.34a` | 照合していないものを「一致が無い」と言わない (#3105 のレビュー 1 周目)。対象の IP 記録が窓の中に 1 件も無いと候補側のクエリは 1 回も飛ばないのに「記録されていません」と断定していた (`hasAnyHistory` は**テーブル全体**を見る値なので、そこだけでは「照合していない」と「照合したが一致が無い」を区別できない)。起点の IP を上限で切ったときも同じで、しかも打ち切りの文面が「候補が多い」としか言わず**原因を誤って説明**していた (IPv6 の privacy extensions は日単位でアドレスが変わるので、90 日窓で 50 本超えはむしろ通常)。判定に `targetIpCount` と `truncated` を渡し、「比較していない」と「調べた範囲では見つからなかった」を足した (判定は #3104 と共有しているので `test/unit/` の表も広げた)。打ち切りは原因ごとに文面を分け、「さらに表示」で解消できると読ませない。「その IP を使ったアカウント数」が上限まで見えた数のときは「N 件以上」と出す。重みの代わりに経過日数を出す (重みは [0,1] に収まるのでパーセントと見分けが付かない)。**「両者の観測日時が違う場合、同時に使っていたことは意味しません」をやめた** — 「日時が同じなら同時に使っていた」という逆の推論を許すので、「出しているのはそれぞれの最終観測で、両者が同じ時間帯に接続していたことを示すものではありません」と無条件に書く。期間が対象側と候補側の両方に掛かること、番号が「表示順」であって順位そのものではないことも明記した。`ip-search.vue` が結末の enum を直接 index していたのもやめた (共有モジュールに結末が増えると、この画面に無いキーを引いて undefined を読み上げる)。**純正へは還元できない行** (純正 backend にこの endpoint が無い)。 |
| `2026.9.0-mk.34b` | 打ち切った検索から断定せず、両方の打ち切りを伝える (#3105 のレビュー 2 周目)。`mk.34a` が 2 つの回帰を作っていた。(1) 打ち切りの原因を分けたときに `v-else-if` にしたため、**起点と候補の両方を切ると候補側の説明が丸ごと落ちる** (「集めていない候補のほうが強い可能性がある」「アカウント数も『以上』になる」「ページを送っても解消しない」が消え、「期間を絞ると起点を絞り込めます」だけが残って候補側への手当てとして読まれる)。0 周目は単一メッセージで両方伝わっていたので**悪化**していた。IPv6 の privacy extensions による起点 50 本超と CGNAT の IP は同時に起きやすい。(2) **打ち切った検索でも「すべて削除済み」と断定していた** — 言えるのは「集めた範囲の候補は全員消えている」までで、到達条件は使い捨てアカウントが一斉に消された後というこの機能が要る場面そのもの。しかも `mk.34a` はその挙動を `test/unit` で固定してしまっていた。サーバーが `targetIpsTruncated` / `candidatesTruncated` を別々に返すようにして画面は独立に出し、`noneResolvablePartial` を足した。**並び順はアカウント数を考慮していない**ので読み手にその数も見るよう促すが、上限で頭打ちになると比較に使えないことも書いた。**純正へは還元できない行** (純正 backend にこの endpoint が無い)。 |
| `2026.9.0-mk.34c` | 判定表を網羅して固定する (#3105 のレビュー 3 周目)。`mk.34b` が足した結末が**合算した打ち切りフラグ**を見ており、しかも `hasMore` より先に返していたため、(a) 起点だけを切ったときに「候補が多く」と説明し (`mk.34b` が banner から取り除いた誤説明そのもの)、(b) 続きのページがあるのに「集めた範囲の候補はすべて削除済み」と断定して次ページへの案内が消えていた (ボタンは出たままなので画面内で矛盾)。あわせて並び順の説明に足した上限の但し書きが、打ち切りの無い通常の検索で「『0件以上』と出ている場合は…」という存在しない表示を指していた。**この判定は 3 周続けて間違えており、毎周 1 つ直して隣の枝で 1 つ作っていた。** 原因は組み合わせを網羅していなかったことで、以前の表は 16 通りしか回さず候補の有無・起点・打ち切りの 3 軸を振っていなかった。**全 576 通りを回して「言ってよいこと」を枝ごとに固定した** (調べきっていないときに「一致なし」と断定しない / 打ち切りや続きがあるときに「すべて削除済み」と断定しない / 続きが無いのに「このページには」と言わない など)。3 周目の回帰そのものを含む 5 形の変異でこの表が落ちることを実測。**純正へは還元できない行** (純正 backend にこの endpoint が無い)。 |
| `2026.9.0-mk.35` | IP 照会の監査記録を読む画面 (`/admin/ip-lookup-log`、#3106)。誰がいつどの IP / 利用者を引いて何件返したかを新しい順に並べる。**記録されるのは照会の事実だけで、結果は残っていない** ので、画面にもその旨を出す。**空の一覧を「照会されていない」と断定しない** — この機能を入れる前の照会・保持期間を過ぎて消えたもの・まだ一度も引かれていないものが同じ表示になる。メニューの出し分けは IP 検索と同じ条件 (記録に入るのは照会に使った IP そのものなので、照会より緩い経路にしない)。あわせて `ipSearchErrorKind` に 429 を足した — `RATE_LIMIT_EXCEEDED` を `failed` (「サーバーのログを確認してください」) に倒していたが、レート制限はログを 1 行も残さないので確認しても何も無く、正しい対処は「待つ」。#3106 でこの endpoint 群にレート制限を入れたことで初めて到達しうる経路になった。**純正へは還元できない行** (純正 backend にこの endpoint が無い)。 |
| `2026.9.0-mk.35a` | 入力欄の無い画面で「IPアドレスとして読めません」と言わない。`admin-user.related-accounts.vue` が初回の `INVALID_PARAM` を共有の分類にそのまま渡しており、IP の入力欄も自由入力の期間も無い画面で「IPv4またはIPv6のアドレスを入力してください」を出しうる状態だった (初回に 400 が返る現実的な原因は**対象がリモート利用者**であること)。`mk.35` の `ip-lookup-log.vue` が同じ理由で `pagingLimit` の文面を分けているのに、こちらだけ対処されていなかった (#3106 のレビュー 2 周目)。**純正へは還元できない行**。 |
...
| `2026.9.0-mk.39` | 再接続したときに、切断していた間の通知とノートを取りに行く (#3132)。realtimeMode の一覧はストリーミングのイベントだけで中身を更新するが、pub/sub なので切断中に発行された分は再送されない。`boot/main-boot.ts` は `_connected_` で**未読バッジの件数だけ**再同期しており、一覧の中身は誰も取りに行っていなかった (バッジだけ増えて中身が増えず、リロードするまで直らない)。非 realtimeMode は `useInterval` の `fetchNewer` が同じ役割を果たしているので、この穴は realtimeMode にだけある。**起点は `_connected_` を受けた瞬間に読む** — `Stream.onOpen` はイベントを emit した後にチャンネルを張り直すので、この時点ではまだ 1 件も届いておらず、手持ちの最新がそのまま「切断前の最新」になる (ジッターを待ってから読むと、その間に流れ込んだぶんだけ起点が進んで穴を飛び越える)。**`_disconnected_` は使えない** — `Stream.reconnect()` 経由の張り直し (`stream.ts` がバックグラウンド 20 秒超の復帰で呼ぶ = モバイルでいちばん多い経路) は `intentionalReconnect` で抑止されるので emit されない (`boot/main-boot.ts` が同じ理由でこのイベントを判定に使っていない)。**通知一覧の穴埋めは `markAsRead: false` で撃つ** — `i/notifications` は取得そのものが既読化を伴い (既定 true → `readAllNotifications` を publish)、それは「利用者が一覧を開いた/更新した」ことを既読の合図とする設計なので、背景の穴埋めでそのまま撃つと見ていない通知までバッジごと消える (`fetchNewer` と `init` / `reload` の両方に受け口を足してあり、一覧が空で `reload()` の枝を通るときも効く)。**控えは世代とセットで持つ** — `Paginator.generation` を足して `init()` で進め、控えてから一覧が総入れ替えされたら起点を捨てる (でないと reload 後に古い起点で取った過去のページが最新の上に積まれる)。**「いま決められない」と「本当に空」を分ける** — `init()` / `reload()` の最中も一覧は空だが、そこで控えると誤った起点が入れ替えを跨いで生き残るので控えず、本当に空のときだけ `reload()` に倒す (`fetchNewer` を起点なしで撃つとサーバーが降順で返して並びが壊れ、`sinceId: '0'` だと最古のページを取ってしまう)。あわせて `Paginator` の `enqueue` / `fetchNewer({toQueue:true})` に queue 内の重複排除を足し (`unshiftItems` / `prepend` は元から弾いている)、queue への差し込みを id で並べ直し (間引きで中間が欠けると順序が壊れる。切り詰めは並べ直した後)、`enqueue` の溢れ処理を「最古を捨てる」に直し (`pop()` は末尾が最古という前提で、並べ直しが入った以上それは成り立たない)、items に入れたものをqueue から引き取り (残すと「N 件の新しいノート」が実際より多く出る)、`pushItems` に重複排除を足した (**offsetMode では落とさない** — あちらは `offset: items.length` で位置を決めるので、items が伸びないと同じページを取り続ける)。追加パラメータは「足すだけ」の位置に置く (後ろにすると `limit` / `sinceId` / 静的 params / `computedParams` / 検索語まで差し替えられる)。判断は `paginatorResyncOptions()` に切り出し、SFC に残る組み立て 1 行と on/off は mk-go 側の静的ゲート (`internal/server/stream_resync_wiring_gate_test.go`) が見る。**既知の制約**: 埋められるのは 1 ページ (30 件) まで / こちらのリクエストが飛行中に `reload()` が始まると取り直した最新ページが穴埋め分の下に入る / レート制限に当たったら通常の取得と同じく追い読みも止まる (背景だからと握り潰すと、制限されている事実が見えなくなるほうが害が大きい)。**upstream 追従で落とす対象ではない** — 純正も同じ穴を持つが、これは pub/sub の性質から来る設計上の欠落で、純正側の修正で解消することがない。 |
| `2026.9.0-mk.40` | プロフィールページの pull to refresh が何も再取得しない問題を直す (#3150)。`pages/user/home.vue` の `reload()` が `// TODO` の空関数で、ジェスチャーもスピナーも動くのに中身が更新されなかった。`enablePullToRefresh` の既定は true なので設定を触っていない利用者は全員踏む。導入は `8c2ab25e5f` (#15851) で、pull to refresh 機能と同時に入ったまま実装されていない。**埋めるだけでは直らない** — 親の `fetchUser()` は先に `user.value = null` を置くので `v-if="user"` の下にあるタブごと unmount され、そこから呼ぶと実行中の `MkPullToRefresh` が破棄されて `refresher` の解決を待っている表示が戻らない。null を挟まず差し替える `refreshUser()` を別に用意して子へ渡す (SSR コンテキストへの短絡も通さない。あれは初回ハイドレーション用で、未ログインの訪問者に埋め込み済みの古いオブジェクトを返す)。**子がユーザーをローカルにコピーしていた** — `const user = ref(props.user)` は一度も再代入されない読み取り専用の複製で、親が再取得しても古いオブジェクトを持ち続けるので `computed` に替える。**memo の同期も効いていなかった** — `watch([props.user], ...)` は plain object を watch source にしており発火しない (getter で渡す。ただし編集中は上書きしない)。**タイムラインも更新する** — `index.timeline.vue` は `MkNotesTimeline` へ `pullToRefresh: false` を渡しており引っ張る操作は外側のページが持つ設計なので、表示中のタブの paginator を reload する口を expose する。**reject させない** — `MkPullToRefresh` は `refresher()` の解決だけを見ていて catch が無く、reject すると引っ張った表示が戻らなくなる。`Paginator` もネットワーク断は握り潰す方針なので `Promise.allSettled` で待つ。**純正へ還元しうる行** (上流の TODO を埋めたもの)。 |
...
| URL preview で取得元が決める文字列の長さ | upstream (summaly) が切るのは `title` 100 / `description` 300 だけで、`site_name` と URL 系 (`url` / `thumbnail` / `icon` / `activityPub`) は無制限。mk-go は **`site_name` も 100 で切り、URL 系は 2048 バイトを超えたら落とす** (切らずに落とす — 途中で切った URL は別の場所を指しうる)。**キャッシュの前提が違う** — upstream は 1 時間 / 100 件のメモリキャッシュだが、mk-go は 24 時間 Redis に載せ、クエリ文字列を変えれば URL は無限に作れる。上限が無いと 1 本あたり `urlPreviewMaximumContentLength` (既定 10MiB) までリニアに伸びる (実測) |
...
倒した後の行は **mk-go が普段作る link 行と同じ形**にする (`isLink=true` / `url=uri` /
...
「mk-go が作ったファイルに削除対象はありません。Misskey から引き継いだデータベースにだけ
...
| 項目 | upstream | mk-go |
|---|---|---|
| IP 履歴の記録契機 | 認証済み API 呼び出しのたび (`ApiCallService.logIp`。プロセス内の集合で重複排除し 1 時間ごとに clear) | **#3103 で upstream に揃えた。** それ以前はサインイン成功時しか記録しておらず、再ログインしない利用者は IP が 1 つのまま固定されていた。`meta.enableIpLogging` は**呼び出しのたびに読む**ので、管理画面での切り替えが再起動なしで効く (#3107。upstream も毎回 meta を見る)。**凍結アカウントは記録しない** — mk-go の `Authenticate` は `isSuspended` のとき `UserContextKey` に user を積まないため middleware が素通りする。upstream は `logIp` が 凍結判定より前にあるので記録する。moderation の用途では凍結後の IP こそ見たいことがあるが、**#3104 では変えていない** — 記録側の挙動なので、検索を足す PR で一緒に変えると「何が効いたか」が分からなくなる。検索結果では凍結済みの候補に印を付けて出す (行は残っているので引ける)。**記録されるのは `RealIP()` が返す値**。echo は `X-Forwarded-For` の末尾に接続元アドレスを足した列を**右から左**へ走査して最初の untrusted を採るので、全エントリが trusted なら先頭 (最左) を返す。**走査の途中で解析できない値に当たると、それより左は信用できないとして直アドレス (= 接続してきた proxy) を返す** (echo v4.15.3 の `ExtractIPFromXFFHeader`。UNIX ソケットの fallback だけは解析できない値を読み飛ばす)。**直アドレス (= proxy 自身) が入るのは、XFF ヘッダが無いとき・接続元が `trustProxy` の外にあるとき・trusted な hop の手前に解析できない値があるとき**で、信頼するのは `trustProxy` に書いた範囲だけ (以前は echo の既定で loopback / link-local / private が常に trusted で、`trustProxy` は trust を広げることしかできなかった。docs/configuration.md「前段 proxy の信頼」)。**#3104 では除いていない** — mk-go は自分の前段 proxy のアドレスを知らないので、除くには設定項目が要る。XFF が付かない構成では**全利用者が proxy の 1 アドレスに集まる**ので、そのアドレスで検索すると事実上の全件が候補として返る。これは記録側の構成ミスで、検索側で隠すと原因が見えなくなる (画面の注意書きが「同じ IP = 同一人物ではない」と常設で言っている)。**rate limit で 429 になったリクエストは記録されない** — middleware を rate limiter の後ろに置いているため。upstream は `call()` の起動直後に記録するので rate limit とは独立。重複排除の窓があるので実際の差はほぼ出ない |
...
| `signin` テーブルの IP | `enableIpLogging` に関係なく記録し、刈らない | **同じ。** gate が掛かるのは `user_ip` と drive の `requestIp` だけで、`signin` は成功・失敗ともに IP を持つ。`user_ip` は日次の clean cron が 90 日で刈るが `signin` は刈られない (upstream も刈らない)。**IP から関連アカウントを引く機能 (#3066 / #3104) はここを読まない** — 読むと「IP を記録しない」設定を実質的に迂回することになるため。`admin/ip/accounts` が引くのは `user_ip` だけ |
...
| `trustProxy` の解釈 | Fastify の `trustProxy` にそのまま渡す。`true` (全 hop を信頼)・hop 数・IP / CIDR / proxy-addr の名前のリストやカンマ区切り文字列を受ける。未設定なら private + loopback の 6 範囲 | **リスト・カンマ区切り・素の IP・名前 (`loopback` / `linklocal` / `uniquelocal`)・`false` は同じ意味。既定値も同じ 6 範囲。** `true` と hop 数は**起動エラー** — `true` は `X-Forwarded-For` の最左をそのまま採る設定で、前段を経由しないリクエストが IP を詐称でき、rate limit / サインイン履歴 / IP 履歴がすべて崩れるため、列挙させる。解釈できない要素も起動エラー (以前は捨てており、全部捨てると echo の既定 = XFF の最左を無条件に信頼、に落ちていた)。**以前の mk-go は echo の既定で loopback / private を常に信頼していた**ので、`trustProxy` に CDN の範囲だけを書いて nginx を同一ホストや Docker network に置く構成が動いていたが、今は nginx を書き足さないと全利用者の IP が nginx のアドレスになる。気付けるよう、明示した `trustProxy` が loopback / private のどれも含まないと起動時に、信頼しない loopback / private から `X-Forwarded-For` が届くと実行中に (プロセスごとに 1 回) Warn を出す (docs/configuration.md「前段 proxy の信頼」) |
| DB 接続の TLS (`db.extra.ssl`) | `db.extra` を node-postgres の pool へ素通しする。`ssl: true` は Node の既定 (`rejectUnauthorized: true`) で証明書とホスト名を検証し、`ssl` の object には `ca` (PEM の中身) や `cert` / `key` など任意の TLS オプションを書ける | **`ssl: true` の意味は同じ (検証あり、`sslmode=verify-full`)**、`ssl: { rejectUnauthorized: false }` と `ssl: no-verify` (node-postgres が同じ意味で受ける文字列) も同じ (検証なし TLS)。以前の mk-go は YAML の bool の `ssl: true` を平文、文字列の `'true'` を**検証しない TLS** として扱っており、どちらも upstream より弱かった。**object で解釈するのは `rejectUnauthorized` だけ** — pgx は CA をファイルパスでしか受けないので `ca` は起動エラーにして `db.extra.sslrootcert` (パス) へ誘導し、他のキーも黙って捨てずに起動エラーにする (捨てると検証先や送る証明書が運営者の意図と変わる)。`ssl` 以外の `extra` のキー (pool のオプション) は従来どおり無視する |
...
| WebAuthn の User Verification (UV) と署名カウンタ | 登録・2 要素目・パスキーの**全経路**で `requireUserVerification: true` で検証する (`WebAuthnService.ts`)。ブラウザへ渡す options はどこも `userVerification: 'preferred'`。署名カウンタが進まない assertion は `@simplewebauthn/server` が `(newCounter > 0 \|\| counter > 0) && newCounter <= counter` で throw するのでログインが失敗する | **upstream と同じ。** 登録・2 要素目・パスキーの全経路で UV を必須にして検証し、ブラウザへの options は `preferred` で出す。go-webauthn は session の `UserVerification` が `required` のときしか UV フラグを見ないので、Finish の時点で `required` に上書きしている (options が `preferred` なので、保存した値のままだと検査されない)。署名カウンタの巻き戻しも upstream と同じく拒否する (go-webauthn は `CloneWarning` を立てるだけで成功を返すので明示的に見ている)。counter を持たない認証器 (常に 0) は誤検知しない。**以前の mk-go は 2 要素目と登録で UV を要求していなかった**ので、その間に UV 無しで登録された鍵 (PIN も生体認証も無いセキュリティキー) は 2 要素目として使えなくなる。鍵の登録は TOTP が有効であることが前提なので (`i/2fa/key-done`)、該当する利用者はパスワード + TOTP でログインし、UV に対応した鍵で登録し直す。**mk-go 生まれの DB を TS に渡したときの非互換も解消する** — 以前は mk-go で UV 無しで登録した鍵が、TS へ移った時点で使えなくなっていた |
...
| `admin/queue/clear` / `promote-jobs` の moderation log | **記録しない** (`clear.ts` / `promote-jobs.ts` は `log()` を呼ばない。型 (`types.ts`) には `clearQueue` / `promoteQueue` が `Record<string, never>` で宣言されているが未使用) | **記録する。** 連合の配送を丸ごと落とせる操作が監査に残らないのは、同じ画面の他の操作 (`suspend` / `deleteAccount`) と揃わない。型名は upstream の宣言 (`clearQueue` / `promoteQueue`) に合わせ、**info に `{queue, state}` / `{queue, promoted}` を載せる** (upstream の宣言は空オブジェクトだが、どのキューをどの状態で消したかが残らないと記録の意味が薄い) |
...
| `WWW-Authenticate` の `error_description` | テンプレートに**無加工**で挿入する (`error_description="${err.message}"`)。`"` が 1 つ入れば quoted-string がそこで閉じ、以降が別の auth-param として読まれる | **quoted-string として閉じてから出す** (`"` / `\` は quoted-pair、制御文字は空白へ)。実測では mk-go にも upstream にも「利用者が制御する文字列がここへ届く経路」は無く (message は全てソース中のリテラル)、**注入は実証されていない**。それでもエラー文言に変数を混ぜた瞬間に注入になる構造なので、出す側で閉じてある。普通の文言は素通しするので、正常系の値は upstream と一致する |
| 資格情報を失効させた後の既存 WebSocket (`/streaming`) | **閉じない。** `StreamingApiServerService` は接続時に 1 度だけ `authenticate` し、以後は切断まで user / token を保持する。`i/regenerate-token` が発行する内部イベント `userTokenRegenerated` は `CacheService` の認証キャッシュを更新するだけで、`myTokenRegenerated` を main stream へ送るほかは接続に触らない。凍結 (`UserSuspendService`)・削除・`i/revoke-token` も同じ | **その資格情報で張られた接続をサーバー側から閉じる。mk-go 独自の硬化。** 対象は (a) `i/regenerate-token` の旧 native token、(b) `i/revoke-token` (アプリ連携の解除も含む) と OAuth の authorization code 再利用で失効させた access token、(c) `admin/suspend-user` / `admin/accounts/delete` / `admin/delete-account` / `i/delete-account` の対象利用者の全接続。閉じないと、漏洩したトークンで張られた接続が失効後も通知・DM・フォロワー限定投稿を受け取り続ける (HTTP 側の tokenCache を落としても WebSocket は再認証しない)。**接続は張られたプロセスにしか居ないので pubsub (`credential:revoke`) で全プロセスへ配る** (wordmute reload #791 と同じ形)。照合の鍵は native token なら **DB に保存された** token の SHA-256 (リクエストの文字列からは作らない。下の「native token の末尾空白」も参照)、access token なら行 id (`internal/misc/credkey`)。**生 token は pubsub に流さない。**閉じる前に**送信キューを吐き切ってから** close code 1008 (`credential revoked`) を送る。受信してから閉じるまで **1 秒の猶予**を置く — `myTokenRegenerated` は main stream の topic、失効は別 topic で、topic ごとに別の購読で配るので到着順が保証されず、先に閉じると通知が落ちるため。**閉じると決めた後に届いたクライアントのメッセージは処理しない** (吐き切りの間に `connect` 等で新しい購読を張らせない)。**失効を処理したプロセスは pubsub を経由せず自分の接続も直接処理する** — Redis が落ちて publish が失敗すると、pubsub 経由では自分にも届かないため。**同じ event で各プロセスの HTTP 側 tokenCache (30 秒、プロセス内) からも対象利用者の entry を落とす** — 落とさないと、Web ノードが複数ある構成で閉じた接続が別ノードへ再接続し、そのノードに残った旧 token / 凍結前の entry で認証されて無期限に残る。さらに安全網として **tokenCache の TTL が切れた後 (受信から 35 秒後) にもう一度同じ条件で閉じる** (失効の直前に DB を引いたリクエストが無効化を追い越して cache に積む競合は避けられないため)。クライアントは再接続し、失効済みの token なら upgrade で 401、**凍結・削除済みの利用者の token なら 403 (本文なし)** になる。凍結の 403 は upstream と同じ (`StreamingApiServerService` の `user?.isSuspended`)。**削除済みの 403 は mk-go 独自** — upstream は論理削除の時点では拒否せず (物理削除の後は token が引けず 401)、mk-go は認証 middleware が削除済みの利用者を匿名に落とすので、見ないと匿名接続として張れてしまう。**パスワード変更では閉じない** (native token が変わらないので upstream と同じ)。**モデレーター権限の剥奪でも閉じない** — `admin` チャンネルが受け取る `adminStream:<userId>` へは、通報のたびに**その時点の**モデレーター一覧 (`GetModerators`) へだけ publish するので、権限を失った後の接続には新しい event が届かない。ロールの変更経路 (付け外し / ロール自体の編集・削除 / 期限切れ / 条件つきロール) は多く、閉じる契機を網羅できないのに対し、publish 側で絞れば契機を問わず効く |
| native token の末尾空白 (`user.token` は `char(16)`) | `AuthenticateService` の `findOneBy({ token })` は PostgreSQL の char 比較なので、**末尾に空白を足した token も同じ利用者に解決される** | **保存値と完全一致しなければ 401 AUTHENTICATION_FAILED にする。** 空白付きの文字列は tokenCache の別キーになり (`InvalidateToken` が消すのは空白なしのキーだけ)、WebSocket の失効鍵もずれて `i/regenerate-token` で閉じなくなっていた (実 DB で再現)。正当なクライアントが空白を足すことは無いので安全側へ倒す。16 文字に満たない保存値は char の埋め草付きで読み出されるので、埋め草だけ落として比べる。access token (`varchar`) は元から完全一致 |
...
| 項目 | upstream | mk-go |
|---|---|---|
| MFM パーサの計算量 | mfm-js。仕事量・メモリの上限は持たない (病的な入力に対する計算量は未計測) | **失敗した試行を位置・深さごとにメモ化し、1 回の Parse の仕事量 (入力長に比例) とメモ表の確保量 (16MB) に上限を置く** (`internal/activitypub/mfm/parser.go`)。修正前は `<b>` を 24 個並べた 72 バイトで 30 秒かかり、リモートノート 1 件で inbox worker を占有できた。上限に届くのは病的な入力だけで、届いたら残りを構文を試さずテキストとして読む (通常の投稿の出力は変わらない。旧パーサとの差分比較で確認) |
...
| パスワードを照合する `i/*` (`i/change-password` / `i/delete-account` / `i/regenerate-token` / `i/update-email` / `i/move` / `i/2fa/register` / `unregister` / `register-key` / `key-done` / `remove-key`) | **照合失敗を数えない。** `i/update-email` (1h 3) と `i/move` (1 日 5) の route 上限を除いて各 endpoint の `meta` に `limit` が無く、token を持つ相手は `INCORRECT_PASSWORD` を見ながら現在のパスワードを無制限に総当たりできる | **照合に失敗した回数をアカウント単位で数え、1 時間に 10 回を超えたら照合せずに 429 `RATE_LIMIT_EXCEEDED` を返す** (`internal/core/passwordguard`。key は全 endpoint で共通なので、1 アカウントあたりの試行は endpoint の数によらずこの回数で止まる)。native token を盗まれただけの段階でパスワードまで割られると、token を失効させても signin し直せるため。**route ごとのレート制限にはしない** — limiter は `RequireAuth` / `RequireSecure` より前に走り、成否に関係なく user bucket を消費するので、被害者の token を持つだけの第三者 (scope 不問) が `i/regenerate-token` を使い切れる。token 漏洩時の唯一の対処を攻撃者が止められてしまう。正しいパスワードと、照合しなかったリクエスト (検証枠が取れなかった 503 など) は数えない。予約してから照合するので、並行で投げても照合に進めるのは上限まで。**native token を持つ攻撃者による妨害は残る** — わざと間違え続ける限り、被害者自身のパスワード操作も 429 になり続ける。利用者自身が native token を失効させる手段は `i/regenerate-token` しか無いので、その間は被害者が token を作り直せない。総当たりを止めることと引き換えで、仕組み上避けられない。`i/change-password` の route 上限 (1h 10 + 1s) は以前からある mk-go 独自のもので残してあり、こちらは token を持つ第三者に使い切られうる。一覧は `TestPasswordChecksAreFailureLimited` が handler の AST から見る (照合より前に guard を呼ぶこと) |
...
| 転送 activity の LD-Signature の `signature.created` | 見ない (`created` は署名対象の options に入るが、鮮度の判定には使わない) | **過去 7 日より古いか、1 時間より先なら拒否する** (欠落・複数・読めない形式も拒否)。**値は JSON の `created` キーではなく、署名された options の正規形 (URDNA2015 の n-quads) にある `dc:created` の literal から読む** (`ld.Processor.SignedCreated`)。options は identity/v1 で正規化されるので、転送者が `created` を消して `dc:created` (`{"@value": ..., "@type": "xsd:dateTime"}`) や完全 IRI `http://purl.org/dc/terms/created` に同じ値を書いても RDF は変わらず署名は通る。以前はキー `created` だけを見て欠落を受理していたため、**そう書き換えるだけで 400 日前の署名でも窓を外せた**。キーの許可リストで塞ぐ案もあるが、署名が覆うのは RDF で JSON のキーではないので、別名の列挙に依存しない正規形から読むほうを採った (Mastodon の `LinkedDataSignature#verify_actor!` も options を identity/v1 で compact してから読む)。**欠落を拒否しても正当な署名は落ちない** — RsaSignature2017 を付ける実装は署名時刻を必ず入れる (upstream `JsonLdService.signRsaSignature2017` は `created: new Date().toISOString()`、Mastodon `LinkedDataSignature#sign!` は `created` と `expires`。どちらもソースで確認した)。それ以外の実装 (Pleroma / GoToSocial 等) が created の無い RsaSignature2017 を付けるかは未確認で、付けていればその転送 activity は落ちる。転送経路の HTTP 署名は転送者 (攻撃者でもありうる) のもので、Date の窓は転送者がいくらでも作り直せる。LD-Signature は鮮度を持たないので、一度受け取った署名付き activity は**何年後でも投げ直せた** (削除済みノートの Create を再送して復活させる、古い Update で本文を巻き戻す等)。replay guard は 15 分しか覚えない。7 日の根拠は、`created` が署名 (= 配送用に render) した時刻で、転送者は元の body をそのまま流すため届くまでの遅延が元サーバーと転送者の配送 retry の和になること — upstream の deliver は既定 12 回・間隔は倍々で最大 8 時間なので、retry を使い切るまでおよそ 1.4 日 (jitter の上限を足して 1.6 日。`QueueProcessorService.ts` の `httpRelatedBackoff`)。**Mastodon の配送 retry はこれより長く続きうる**ので、受信側が 1 週間以上落ちていた場合の転送 activity は落ちる (直接配送は HTTP 署名の経路なので影響しない)。**窓の内側の replay は塞いでいない** (replay guard の TTL を 7 日にすると転送量ぶん Redis のキーが増えるため見送った) |
...
| `blockedHosts` / `silencedHosts` / `mediaSilencedHosts` と非既定ポート | `user.host` に非既定ポートを残したまま `` `.${host}`.endsWith(`.${x}`) `` で照合するので、**`https://evil.example:8443/` で actor を公開するだけで `evil.example` のブロック / サイレンスを回避できる** (末尾ドットの `evil.example.` も同じ) | **拒否側の一覧はポートと末尾ドットを落とした形でも照合する** (`idnhost.MatchesBlockList`)。ポートは相手が自由に選べる値で、同じホスト名の別ポートは同じ運営者の管理下にあるので、ブロックの意図から外す理由が無い。ポート付きで書いた指定 (`evil.example:8443`) はそのポートだけに当たる。一覧の Unicode 表記 (`パイ.example`) も punycode の `user.host` に当てる。host の IDN 表記は upstream の `new URL().host` と同じく UTS#46 の文字対応付けで畳む (全角英字・U+3002・soft hyphen を含む `https://ｅｖｉｌ.example/` も Go の HTTP client の接続先と同じ `evil.example` になる。以前の mk-go はここを畳まず、別綴りで保存・比較してブロックを素通りさせていた。既存行は `backfill-remote-host` が畳み直す)。**`federationHosts` (`federation: specified` の許可リスト) はポートを落とさない** — 落とすと許可が広がるので upstream と同じ厳密一致のまま (`good.example` は既定ポートだけを許し、`good.example:8443` は明記が要る)。拒否が許可に勝つので、許可リストにポート付きで載せても `blockedHosts` の `evil.example` は効く。`admin/federation/instances` の `blocked` / `silenced` 絞り込み (`host IN (...)` の完全一致) は表示用で対象外 |
| インスタンスの配送停止 (`suspensionState`) と非既定ポート | 配送時に `new URL(job.data.to).host` で instance 行を完全一致で引くので、**モデレーターが `evil.example` を停止しても、actor を `https://evil.example:8443/` で公開し直すだけで別の行 (`evil.example:8443`) になり配送が再開する** | **モデレーターによる停止 (`manuallySuspended`) は、ポートと末尾ドットを落とした host の行も見る** (`instance.Service.lookupSuspended`)。blockedHosts と同じ理屈で、ポートは相手が自由に選べる値で同じホスト名の別ポートは同じ運営者の管理下にある。**自動停止 (`autoSuspendedForNotResponding` / `goneSuspended`) は広げない** — 「その authority のサーバーが応答しない」という観測で、別ポートの別プロセスには当てはまらない。サブドメインには広げない (停止は元から完全一致)。利用者の `mutedInstances` は upstream と同じ完全一致のまま |
...
| 非公開 (`visibility: private`) の Flash | **可視性を見ない。** `flash/show` は id が分かれば誰にでも返し、`flash/like` も可視性を見ずに like させ、`flash/my-likes` (`FlashService.myLikes`) は like 済みの Flash を script ごと返す。SSR の `/play/:id` も可視性を見ずに title / summary / 作者を OGP に出す | **所有者以外には存在しない扱い** (`flash/show` / `flash/like` とも `NO_SUCH_FLASH`)。**SSR の `/play/:id` は非公開なら OGP を持たない素の shell を返す** — SSR はセッションを見ないので所有者かどうかを判定できず、所有者が開いても同じ (中身は SPA が `flash/show` で取り直すので表示には影響しない)。`flash/show` だけ塞いでも、like が通ると `flash/my-likes` (`i/flashs/likes`) が script 全文を返すので閲覧の抜け道になっていた。**like した後に非公開化されたものも `my-likes` には出さない** (repository の JOIN で `visibility = 'public'` か所有者のものに絞る)。**`flash/unlike` は自分の like 行があれば非公開でも外せる** — 外せないと、再公開されるまで自分の like を取り消す手段が無くなるため。like していない非公開のものに `NOT_LIKED` を返すと存在が分かるので、そのときは `NO_SUCH_FLASH` |
| アンテナの `userListId` (`antennas/update` と照合) | **「新 src か既存 src が list」で、かつ `userListId` を送ったときだけ**所有者を検証する。`userListId` を送らずに src だけ `list` へ切り替えると、保存済みの値は検証されない (upstream は検証済みの値しか保存しないので、実際に他人の list が残るのは直接 DB を触ったときに限られる)。`userListId` を送らない更新では既存値をそのまま残す。照合 (`AntennaService`) は list の所有者を見ない。import (`ImportAntennasProcessorService`) は `userListId` を常に null で作る。export は保存済みの `userListId` の list のメンバーを所有者を問わず `userListAccts` に出す | **更新後の src が `list` なら、更新後に参照する `userListId` (送られた値、無ければ既存値) を毎回検証する** (`NO_SUCH_USER_LIST`)。mk-go は以前 `src` が list 以外のときも送られた `userListId` を未検証のまま保存しており、src だけを後から list へ切り替えると他人の list をアンテナのソースにできた。**更新後の src が `list` 以外なら `userListId` は null にする** (送られた値も既存値も残さない)。**照合時にも list の所有者 == アンテナの所有者を確かめる** — 修正前に保存された (create / update、または import で持ち込まれた) 他人の `userListId` が行に残っていても、その list のメンバー構成をアンテナ経由で覗けないようにする。**既存の行は書き換えない** (migration で null 化はしておらず、照合と export が無視するだけ)。**export も同じ条件で、自分の list のときだけ `userListId` / `userListAccts` を出す**。**import は upstream と同じく `userListId` を常に null で作る** — mk-go は以前ファイルの `userListId` をそのまま保存しており、create / update の所有者検証を import で迂回できた |
| 通知の受信設定 `type: 'list'` の `userListId` (`notificationRecieveConfig`) | 保存時 (`i/update`) も照合時 (`NotificationService.createNotification`) も list の所有者を見ない。他人の list を指定すると、そのメンバーからの通知だけが届く | **照合時に list の所有者 == 通知の受信者を確かめ、他人の list なら誰も通さない** (存在しない list と同じ扱い)。通すと通知が届くかどうかで他人の list のメンバー構成を 1 人ずつ確かめられるため。保存時の検証は upstream に無いので入れていない (アンテナの照合と同じ多層防御) |
...
| `proxy` 設定時の宛先検査 | **検査しない。** private IP の拒否は `HttpRequestServiceAgent.createConnection` が接続した socket の `remoteAddress` を見る形で、`config.proxy` があると `getAgentByUrl` は `hpagent` の `HttpProxyAgent` / `HttpsProxyAgent` を返す。そちらの socket の相手は proxy なので、宛先 (`http://169.254.169.254/` など) は無検査のまま proxy へ渡り、`allowedPrivateNetworks` も効かない (加えて検査自体が `NODE_ENV === 'production'` のときだけ) | **proxy へ渡す前に宛先を検査する。** `safehttp` の `Transport.Proxy` callback で、リテラル IP はそのまま、ホスト名は手元で解決した全アドレスを dial 時と同じ判定 (`allowedPrivateNetworks` 込み) にかけ、1 つでも禁止範囲なら `ErrSSRFBlocked`。redirect 後の各リクエストも同じく通る。**手元で解決できない宛先は proxy に渡さない** (proxy だけが名前解決できる閉域構成では upstream と挙動が変わる)。`proxyBypassHosts` の宛先は従来どおり dial 時に検査する。宛先 host は net/http が dial する形 (小文字化 + `idna.Lookup`) に揃えてから検査と bypass の照合を行う (Unicode の IDN を生のまま resolver に渡すと必ず失敗するため)。**厳密な dotted-quad でない数値表記 (`0x7f.1` / `2130706433` / `127.1` / `0177.0.0.1`) は proxy に渡さない** — WHATWG URL と proxy 側の `getaddrinfo` (inet_aton) はこれを IPv4 として解釈するが、手元の resolver は名前として扱うので、search domain などで外部アドレスが返ると検査を通ってしまう。通過した宛先は 30 秒間キャッシュする (keep-alive でも `Transport.Proxy` はリクエストごとに呼ばれるため。拒否は覚えない)。**proxy 側の再解決による DNS rebinding の窓は残る**ので、proxy 側でも内部宛てを拒否する設定を求めている ([configuration.md](configuration.md)) |
...
| `following/requests/accept` の block 検査 | **対応物なし。** `acceptFollowRequest()` は block 関係を検査しない (`block()` が `cancelRequest` を双方向で呼び、pending request を消すことだけで守っている) | **`AcceptRequest()` の先頭で block 双方向を検査し、block されていれば `BLOCKING` / `BLOCKED` (400) で拒否する。** mk-go の `Block()` は follow request の取り消しを best-effort にしている (取り消しに失敗しても block 自体は成立させる、`removeFollowing` と同じ判断) ため、取り消しが失敗すると「block は成立、申請は残る」状態になりうる。following 行と違い、`follow_request` は承認すると静かに following へ変わるので、後始末の成否に依らず不変条件を保つための多層防御として追加した (#3111)。AP inbound の `Accept` (`core/federation/processor.go` の `handleAccept`) では、この検査が返す `ErrBlocking` / `ErrBlocked` も retry しても変わらないため ack する (`ErrRequestNotFound` と同様) |
