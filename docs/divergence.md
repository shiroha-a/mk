...
| リモート actor の `uri` / `host` の長さ | 検証なし | **収まらなければ actor ごと拒否する** (`uri` は varchar(512)、`host` は varchar(128))。身元そのものなので切ると別人になり、捨てると lookup の鍵が無くなる。upstream は `uri` に `person.id` をそのまま入れるので同じ 22001 で失敗しうる。**gate は create 経路にしかない** (`refreshActor` は既存行専用で `uri` / `host` を書かない)。拒否したあとの扱いは**経路で違う**: Like / Announce / Undo は `isPermanentSkipError` で ack するが、Create / Follow / Delete は error を surface するので inbox job が retry を使い切って dead になる。いずれにせよ**その actor から activity が来るたびに 1 回 fetch する状態は続く** (行が作られない以上 `lastFetchedAt` を進める先が無い) (#2723) |
...
| リモート Note / Announce の `id` の長さ | 検証なし (`uri` に生値を入れる) | **収まらなければ document ごと拒否する** (`note.uri` は varchar(512))。切ると別の note を指す URI になり、dedup (`FindByURI`) と Undo(Announce) の逆引きの鍵も壊れる。**job の結末は変わらない** — `handleCreate` / `dispatchActivity` はこの error を surface するので、その inbox job は retry を使い切って dead になる (`isPermanentSkipError` はハンドラが**下位の**失敗を握るためのもので、ハンドラ自身の戻り値には効かない)。変わるのは 1 回あたりのコスト (DB を叩かない) と、原因が 22001 ではなく明示的な拒否として残ること (#2723) |
...
**まだ規則を適用していない列がある。** #2723 のレビューで横断調査して見つかった分
(= 全量ではない) は #2726 に集約してある: `user_publickey.keyId` /
`user_publickey_extra.keyId` (各 256) / `note_reaction.reaction` (260) /
`poll.choices` (varchar(256)[]) / chat 系 (`chat_room.name` 256 /
`chat_room.description` 2048 / `chat_room.id` 32 / `chat_message.text` 4096) /
`abuse_user_report.comment` (2048) / AP tag 由来の emoji 列
(`emoji.originalUrl` / `publicUrl` / `uri` 512、`license` 1024)。
