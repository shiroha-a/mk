...
| パスワードを照合する `i/*` (`i/change-password` / `i/delete-account` / `i/regenerate-token` / `i/2fa/register` / `unregister` / `register-key` / `key-done` / `remove-key`) | **レート制限が無い** (各 endpoint の `meta` に `limit` が無い)。token を持つ相手は `INCORRECT_PASSWORD` を見ながら現在のパスワードを無制限に総当たりできる | **1 時間 10 回 + 1 秒間隔** (`DefaultEndpointLimits`)。認証済みのリクエストは user bucket を必ず消費するので、IP をローテートしても 1 アカウントあたりの試行はここで止まる。native token を盗まれただけの段階でパスワードまで割られると、token を失効させても signin し直せる。一覧は `TestPasswordCheckingRoutesHaveRateLimits` が handler の AST から導出して突き合わせる。**`i/update-email` (1h 3) / `i/move` (1 日 5) は upstream にも上限があり、そちらに揃えたまま** |
