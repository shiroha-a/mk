...
| `trustProxy` の解釈 | Fastify の `trustProxy` にそのまま渡す。`true` (全 hop を信頼)・hop 数・IP / CIDR / proxy-addr の名前のリストやカンマ区切り文字列を受ける。未設定なら private + loopback の 6 範囲 | **リスト・カンマ区切り・素の IP・名前 (`loopback` / `linklocal` / `uniquelocal`)・`false` は同じ意味。既定値も同じ 6 範囲。** `true` と hop 数は**起動エラー** — `true` は `X-Forwarded-For` の最左をそのまま採る設定で、前段を経由しないリクエストが IP を詐称でき、rate limit / サインイン履歴 / IP 履歴がすべて崩れるため、列挙させる。解釈できない要素も起動エラー (以前は捨てており、全部捨てると echo の既定 = XFF の最左を無条件に信頼、に落ちていた) |
