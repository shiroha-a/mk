...
| WebAuthn の User Verification (UV) と署名カウンタ | 登録・2 要素目・パスキーの**全経路**で `requireUserVerification: true` で検証する (`WebAuthnService.ts`)。ブラウザへ渡す options はどこも `userVerification: 'preferred'`。署名カウンタが進まない assertion は `@simplewebauthn/server` が `(newCounter > 0 \|\| counter > 0) && newCounter <= counter` で throw するのでログインが失敗する | **鍵が唯一の要素になるログインだけ UV を必須にする**。対象は `signin-with-passkey` と、`signin-flow` で `usePasswordLessLogin` の利用者がパスワード不一致のまま credential を出す経路。options も `required` で出す (upstream は `preferred`。UV できない認証器はブラウザの段階で断られるだけで、サーバー側の結果は同じ)。**要否は Finish の時点で決め直す** — challenge は user 単位で 1 件しか無いので、正しいパスワード付きで始めた challenge にパスワード無しの credential を載せられる。 **パスワード + 2 要素目の鍵と登録は UV を要求しない (upstream より緩い、既存の差分)**。PIN の無いセキュリティキーを 2 要素目として 使えるようにしている。署名カウンタの巻き戻しは upstream と同じく拒否する (go-webauthn は `CloneWarning` を立てるだけで成功を 返すので明示的に見ている)。counter を持たない認証器 (常に 0) は誤検知しない |
