...
| media proxy の pack 時適用範囲 | `DriveFileEntityService.getPublicUrl` などが remote の drive file を proxy する一方、**ロール / アバターデコレーション / 通知 icon / feed / SSR OGP / system account は raw** を返す | **閲覧者のブラウザが remote origin を直接取りに行く経路を塞ぐ (#1529)**。ロール `iconUrl` — proxy 化するのは公開 `roles/*`・roleAssigned 通知・`/api/i` の `rolePayload`・`admin/show-user` の `roles[]`・`admin/roles/list` (#3130 review で追加。消費者 (`MkRolePreview` / `rolesCache` / `MkRoleSelectDialog` / `admin-user.vue` / `emoji-edit-dialog.vue`) はどれも書き戻さない)。**raw のまま**にするのは `admin/roles/show` とその応答を経由する create/update だけ — 同梱 frontend の編集フォーム (`pages/admin/roles.edit.vue`) が読んだ値をそのまま `admin/roles/update` へ書き戻すため、proxy 済み URL を返すと sig 付き URL が `role.iconUrl` 列に永続化される (secret ローテーションで無効化 / `varchar(512)` 超過で 500 / drop-in で TS が mk-go の `/proxy` URL を配り続ける)。他に proxy 化するのはアバターデコレーション `url` (pack 時 + 公開カタログ picker。emoji 由来 / catalog 由来の両分岐)、通知 `icon` / アクセストークン `iconUrl` (利用者由来は期限付き署名。上記行)、reversi の WebSocket stream payload の avatar (REST は proxy 済みで stream だけ漏れていた。`avatarDecorations` 自体は stream に無いので対象外)、RSS/Atom/JSON feed の avatar、お知らせ SSR の og:image、ローカル system account の `meta.iconUrl` 由来 avatar。**意図的に raw のまま**にするのは広告 `imageUrl` (upstream も server/client で raw。ただし mk-go の CSP enforce では remote 画像の広告は表示されない)、admin 専用の avatar-decorations 作成/一覧/更新 (編集ダイアログが `url` を書き戻すため role と同じ理由)、`/api/meta` のブランディング URL、ActivityPub outbound。**副作用: proxy 化した先は mk-go の media proxy の MIME ポリシーを引き継ぐ** — `modeDefault` (role iconUrl / decoration url / 通知 icon / app iconUrl / お知らせ OGP) は `image/svg+xml` を 1x1 透明 PNG に差し替え、`modeAvatar` (`meta.iconUrl` 由来の system account avatar) は SVG を 404 にする。運営者が `meta.iconUrl` や role アイコンに外部 CDN の SVG を置いている構成では、この PR 以降そのアイコンが 1x1 透明 PNG / 404 になる (PR 前は生 URL でブラウザが描画していた) |
