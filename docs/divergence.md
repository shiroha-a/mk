...
> 混じりうるので、乖離を判断するときは対象の実装を現 pin (`2026.9.0-mk.16a`) で確認すること。
...
| config キー | 20 前後 | 0 | — |
| fork frontend の独自変更 | 63 tag (`2026.7.0-mk.0` ～ `2026.9.0-mk.16a`) | — | — |
...
| `2026.9.0-mk.16` | リモート絵文字の一覧を media proxy 経由にする (#2957)。カスタム絵文字管理 (beta) のリモート一覧が**1 件も表示されていなかった**。グリッドの image セルは `<img :src="cell.value">` に値をそのまま入れる実装で、渡していたのが相手サーバーの `publicUrl` だったため、`img-src 'self' data: blob:` をenforce している構成で全部ブロックされる (実測: 上位 5 ホストで約 1.1 万件、すべて外部オリジン)。**media proxy は通る** — 外部 3 ホストで実測し、いずれも 200 (allowlist が `emoji.publicUrl` を通す)。**静止画設定は `getStaticImageUrl` で包む** — `getProxiedImageUrl` の第 4 引数は `noFallback` で静止画とは無関係。また `noFallback` は渡さない (グリッドに `@error` の受け皿が無く、proxy が 403/404/500 を返すと壊れ画像になる)。**`MkDataCell` は変えない** — image セルの列は 4 つで**すべてカスタム絵文字管理**だが、`local.list` の url 列は `editable: true` で、proxy を通すと編集・保存が壊れる。呼び出し側で変換する (#2935 と同じ形)。ローカル一覧と登録画面は影響を受けない (自サーバーが配る URL で CSP に入る。**ただし `'self'` とは限らず**、object storage 構成では `cspMediaExtras` が足すオリジンで通っている)。編集ダイアログは #2903 で既に proxy 経由で、**一覧だけが取り残されていた**。**同じ型を 3 回踏んだ**ので (#2903 モーダル / #2935 審査画面 / 本件 一覧)、`TestRemoteMediaGoesThroughProxy` で新規流入を止める。**一般的な判定はできない** — `publicUrl` はローカルなら自オリジン、リモートなら相手のオリジンで静的には区別できないため、#2792 と同じ allowlist 方式にし、通す場合は理由を書く。**mk-go 固有の CSP への適応**。ただし CSP が無くても生 URL は閲覧者の IP を相手 host へ渡すので (本番で remote 絵文字の host は 432、1 ページ最大 100 行)、**還元不能ではなく優先度が低い**という位置づけ。 |
| `2026.9.0-mk.16a` | リモート絵文字一覧の proxy の通し方を直す (#2957)。**`getProxiedImageUrl` の第 4 引数を取り違えていた** — `noFallback` であって静止画設定とは無関係なのに、issue・PR・doc に「第 4 引数で静止画設定を尊重する」と書いて伝播させていた。実測では GIF が 229,610 B のまま出ており、`disableShowingAnimatedImages` を on にしていてもここだけアニメーションしていた (`getStaticImageUrl` で包むと 6,304 B の webp)。**`noFallback = true` も誤り** — グリッドの image セルは `@error` の受け皿を持たないので、proxy が 403/404/500 を返すと壊れ画像アイコンと長大な proxy URL (alt) がセルに出る。実測で 25 host 中 3 件が該当し、fallback ありなら 200 で救われる。`MkRemoteEmojiEditDialog` が同じ理由で `noFallback` を外しているのに揃えていなかった。**インポートログ側の変換は no-op だったので落とした** — `it.item` はグリッドの行で `url` には既にproxy 済みの URL が入っている (`getProxiedImageUrl` は冪等)。 |
