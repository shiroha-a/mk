...
| `specified` ノートの `Delete` / `Update(Question)` の配送先 | どちらもフォロワー + relay へ配る (`Delete` は加えてメンション先と renote / reply したリモート利用者へも) | **宛先とメンション先だけに送る**。宛先には直接届くので届く相手は減らない。アンケートは投票のたびに `Update` が出るので、可視性を見ないと本文が全フォロワーへ渡る |
