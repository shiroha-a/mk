...
| 1:1 配送で個別 inbox を持たない相手 | `core/federation/deliver_service.go` | **upstream より広く送る**。1:1 (direct recipe) の配送先はどちらも**個別 inbox を優先**するが、`inbox` が無い行について upstream (`ApDeliverManagerService.execute`) は `if (recipe.to.inbox === null) continue;` で配送を skip する。mk-go は `sharedInbox` があればそちらへ倒す (送れるなら送る方が利用者の意図に近い)。そのときだけ `IsSharedInbox` が立つので、410 Gone が host 単位の gone 判定 (`MarkGoneSuspended`) へ届きうる — 個別 inbox を持つ相手ではこの経路に入らない |
