...
> 混じりうるので、乖離を判断するときは対象の実装を現 pin (`2026.9.0-mk.23`) で確認すること。
...
| config キー | 20 前後 | 0 | — |
| fork frontend の独自変更 | 89 tag (`2026.7.0-mk.0` ～ `2026.9.0-mk.23`) | — | — |
...
**現在の pin は `2026.9.0-mk.23` (`c9340e71`)。** tag 列は「その変更が最初に入った世代」で、
...
| AP 経由の chat room 招待 | `createRoomInvitation` に block 判定なし (`// TODO: cehck block` と書かれている) | **block を見る**。1-on-1 の受信は upstream も block を見るので、同じ chat 機能の中で片側だけ無いのは非対称 |
| AP 経由の chat room 招待の定員 | membership のみ数える | **未消化の招待も数える**。membership だけだと、誰も accept しない room 1 つで招待行と通知を無制限に作れる |
| chat room の `Group.id` の host | 検証なし | **送信元の actor と突き合わせる**。管理していない host の room URI を名乗ってローカルの id 空間に room を作れる (`chat_room` に host 列が無いので、攻撃者が**自分の host で**任意 id を先取りする形は残る) |
| chat message の `content` | MFM 原文をそのまま入れる | **`mfm.ToHTML` を通し、`source` と `_misskey_content` に原文を併記する**。`mediaType` を出していないので AS2 既定の text/html として解釈され、生の本文はそのまま HTML になる。受信側も `source` を優先する (往復で装飾・色・引用の改行が落ちるため) |
...
| `specified` ノートの `Delete` / `Update(Question)` の配送先 | フォロワーへも配る | **宛先とメンション先だけに送る**。宛先には直接届くので届く相手は減らない。アンケートは投票のたびに `Update` が出るので、可視性を見ないと本文が全フォロワーへ渡る |
| nodeinfo discovery の `links[].href` | 検証なし | **取得元の host に縛り、redirect 先も縛る**。href の検証だけだと 302 一回で任意の host へ抜けられる。返る JSON は content-type も見ずに instance 行へ書き戻るため |
...
| JSON-LD の衝突キー | 後から来たものが勝つ | **document ごと拒否する**。`actor` と `as:actor` のように同じ canonical へ畳まれるキーが複数あると、Go の map は勝者が走査順で決まるので、認可の判断と本処理が別々に正規化した結果がずれる |
