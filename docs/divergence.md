...
> 混じりうるので、乖離を判断するときは対象の実装を現 pin (`2026.9.0-mk.39`) で確認すること。
...
| config キー | 20 前後 | 0 | — |
| fork frontend の独自変更 | 115 tag (`2026.7.0-mk.0` ～ `2026.9.0-mk.39`) | — | — |
...
**現在の pin は `2026.9.0-mk.39` (`609931cf`)。** tag 列は「その変更が最初に入った世代」で、
...
| URL プレビュー画像の proxy 署名 | `/proxy` に認可が無いので署名の概念が無い | **期限付きにする (#3037)**。`/url` は未認証で任意の URL を渡せて、その OGP 画像と favicon は**渡したページに書いてあった URL** = 攻撃者が自由に決められる値。無期限の署名を出すと、media proxy の allowlist (mk-go 独自の硬化) を恒久的に迂回できる。`<unix 秒>.<hex>` の形で 7 日 — プレビュー応答自身が 1 日キャッシュされるので、それより短いとキャッシュに残ったプレビューの画像だけが壊れる。**発行そのものは止まらない** (攻撃者はいつでも叩き直せる)。止まるのは「発行した URL を貼って配る」形。**#1529 で対象を拡張した** — アクセストークンの `iconUrl` (`i/apps`) と app 通知の `icon` (`notifications/create` が保存し、**読み出し時に**発行) も同じ期限付きにする。`notifications/create` は認証済み利用者が任意の `icon` を入れられるので、自分宛て通知を作って読み戻すだけで任意 URL に対する期限付き署名を 1 つ発行できる (発行できること自体は `/url` と同じで、止まるのは配る形)。**exp は half-TTL バケット (3.5 日) に丸める** — 読み出しのたびに別の署名を出すと、通知一覧を毎回取り直すブラウザ / CDN のキャッシュが毎回ミスするため。有効期間は (3.5 日, 7 日] で、URL は 3.5 日に 1 度だけ変わる。管理者が設定する画像 (ロールのアイコン / お知らせ / チャンネルのバナー) への proxy 適用は無期限のまま (#1529 でロール `iconUrl` を公開経路だけ proxy 化) |
| media proxy の pack 時適用範囲 | `DriveFileEntityService.getPublicUrl` などが remote の drive file を proxy する一方、**ロール / アバターデコレーション / 通知 icon / feed / SSR OGP / system account は raw** を返す | **閲覧者のブラウザが remote origin を直接取りに行く経路を塞ぐ (#1529)**。ロール `iconUrl` — proxy 化するのは公開 `roles/*`・roleAssigned 通知・`/api/i` の `rolePayload`・`admin/show-user` の `roles[]`・`admin/roles/list` (#3130 review で追加。消費者 (`MkRolePreview` / `rolesCache` / `MkRoleSelectDialog` / `admin-user.vue` / `emoji-edit-dialog.vue` / `avatar-decoration-edit-dialog.vue`) はどれも書き戻さない)。**raw のまま**にするのは `admin/roles/show` とその応答を経由する create/update だけ — 同梱 frontend の編集フォーム (`pages/admin/roles.edit.vue`) が読んだ値をそのまま `admin/roles/update` へ書き戻すため、proxy 済み URL を返すと sig 付き URL が `role.iconUrl` 列に永続化される (secret ローテーションで無効化 / `varchar(512)` 超過で 500 / drop-in で TS が mk-go の `/proxy` URL を配り続ける)。他に proxy 化するのはアバターデコレーション `url` (pack 時 + 公開カタログ picker。emoji 由来 / catalog 由来の両分岐)、通知 `icon` / アクセストークン `iconUrl` (利用者由来は期限付き署名。上記行)、reversi の WebSocket stream payload の avatar (REST は proxy 済みで stream だけ漏れていた。`avatarDecorations` は stream にも載る (`userLiteMap` が `u.AvatarDecorations` を出す) が、保存値の jsonb をそのまま出しており `url` を含まないため対象外 — 将来 `userLiteMap` に url 解決を足すときはここも見ること)、RSS/Atom/JSON feed の avatar、お知らせ SSR の og:image、ローカル system account の `meta.iconUrl` 由来 avatar。**意図的に raw のまま**にするのは広告 `imageUrl` (upstream も server/client で raw。ただし mk-go の CSP enforce では remote 画像の広告は表示されない)、admin 専用の avatar-decorations 作成/一覧/更新 (編集ダイアログが `url` を書き戻すため role と同じ理由)、`/api/meta` のブランディング URL、ActivityPub outbound。**副作用: proxy 化した先は mk-go の media proxy の MIME ポリシーを引き継ぐ** — `modeDefault` (role iconUrl / decoration url / 通知 icon / app iconUrl / お知らせ OGP) は `image/svg+xml` を 1x1 透明 PNG に差し替え、`modeAvatar` (= `ProxyAvatarURLString`。`meta.iconUrl` 由来の system account avatar、feed の avatar、reversi stream の avatar) は SVG を 404 にする。REST 側の avatar (`entity.PackUserLite`) も同じ `modeAvatar` なので、この PR で新しく破壊されるものではない。運営者が `meta.iconUrl` や role アイコンに外部 CDN の SVG を置いている構成では、この PR 以降そのアイコンが 1x1 透明 PNG / 404 になる (PR 前は生 URL でブラウザが描画していた) |
