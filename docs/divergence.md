...
> 混じりうるので、乖離を判断するときは対象の実装を現 pin (`2026.9.0-mk.21a`) で確認すること。
...
|---|---|---|---|
| API endpoint | GET variant 23 + alias 4 + 分割アップロード 4 + 承認制 7 + 絵文字の申請 9 + exact assignment lookup 2 + admin 観測 5 | chat 15 | **0** |
...
| config キー | 20 前後 | 0 | — |
| fork frontend の独自変更 | 81 tag (`2026.7.0-mk.0` ～ `2026.9.0-mk.21a`) | — | — |
...
**現在の pin は `2026.9.0-mk.21a` (`8cc87d81`)。** tag 列は「その変更が最初に入った世代」で、
