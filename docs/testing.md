...
| Playwright e2e | mk-go と Misskey TS の両 backend で API/frontend 統合互換を検証 | Docker Compose 全部 | `tests/playwright/` 配下 (#744、294 spec ファイル。PR ごとに mk-go、upstream 追従時に TS backend) |
...
- 範囲: 294 spec ファイル (upstream 290 = ui 194 / api 96、mkgo 4) / 40 directory (spec を直接含むもの。`find ... -printf '%h\n' | sort -u | wc -l`)
