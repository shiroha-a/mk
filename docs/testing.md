...
| Playwright e2e | mk-go と Misskey TS の両 backend で API/frontend 統合互換を検証 | Docker Compose 全部 | `tests/playwright/` 配下 (#744、297 spec ファイル。PR ごとに mk-go、upstream 追従時に TS backend) |
...
- 範囲: 297 spec ファイル (upstream 290 = ui 194 / api 96、mkgo 7) / 40 directory (spec を直接含むもの。`find ... -printf '%h\n' | sort -u | wc -l`)
...
- spec は原則 **backend-agnostic** (= URL 切替だけで両 backend で動く)、spec 失敗 = drop-in 互換 regression として issue 化。例外は `specs/mkgo/` の 7 件 (mk-go 独自機能を見るので公式 image では通らない)。`make playwright-ts-test` が `specs/upstream` に絞ることで除外している
