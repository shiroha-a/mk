...
対象バージョン: **Misskey 2026.9.0** (mk-go 1.3.0)
最終更新: 2026-09-07
...
- **Playwright e2e**: 297 spec ファイルを PR ごとに実行。Misskey TS backend に対しては upstream 追従時に実行し、spec が mk-go の挙動に引きずられていないかを検証する
...
Go側のマイグレーション (000001〜) はTS版テーブルに対して原則追加のみだが、例外が 14 件ある ([migration-from-ts.md](migration-from-ts.md#破壊的なマイグレーション))。TS版のマイグレーションで作成される全テーブルは維持される。

**mk-go 固有のテーブル (upstream に対応するものが無い) は 12 件:**

> [divergence.md](divergence.md) §2-1 は同じものを **15** と数えている。差は 3 件で、
...
mk-go の migration が作るテーブルは 115。上記 12 件と golang-migrate 台帳の
