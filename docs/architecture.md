...
| playwright | 293 spec を mk-go backend で（PR ごと、4 シャード）。TS backend は手動 | — |
...
マイグレーション（`migration/`、golang-migrate、現在 82 本）:

- TS Misskey の既存テーブルへは原則**追加のみ**。例外が 10 件あり、うち 9 件は mk-go が自分で作ったものの除去・初期化か upstream 追随 ([TS版からの移行](migration-from-ts.md#破壊的なマイグレーション))。Go 固有の追加列・テーブルは `IF NOT EXISTS`。
