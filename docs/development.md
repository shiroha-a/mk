...
| `make frontend-check` | 同梱フロントエンドの型チェック (`vue-tsc --noEmit`) と、**submodule のソースを読むゲート**。ビルド成果物を作らないので安全。ゲートを `make gates` に入れないのは、あちらが submodule 無しで回る前提で、混ぜると checkout していない環境で skip され「検査していないのに緑」になるため (#2892)。CI の同名 job はこれに加えて `make plugins-all` と統合バイナリの build を別 step で走らせる |
...
| `frontend-check` | CI | fork frontend の型 (`vue-tsc --noEmit`) + submodule のソースを読むゲート + `make plugins-all` と統合バイナリの build。job 全体を手元で再現するには `make frontend-check` に加えて `make plugins-all && go build ./cmd/misskey` も要る |
