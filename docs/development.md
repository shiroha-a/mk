...
| `make fmt` | `gofmt -s -w .` |
| `make lint` | `go vet ./...` |
| `make golangci-lint` | `errcheck` / `govet` / `ineffassign` / `staticcheck`。`go vet` だけでは見えない層を埋める。設定は `.golangci.yml`。**既定の打ち切り (同一メッセージ 3 件 / linter 50 件) を外してある** — 切り詰めるだけなので赤が緑になることはないが、直すたびに隠れていた分が出てきて「全部直してから有効化する」が成立しない。`unused` / `ST1003` / `ST1012` / `SA1019` はすべて有効 (2026-09-22 に全件処理して 0 件。SA1019 は 9 件を移行し 1 件は `//nolint`、`unused` は 20 件を削除)。除外は `test/e2e_federation` のパッケージ名 (`.golangci.yml` の rule) と `echo` の `LoggerWithConfig` (`//nolint`) の 2 つ。`QF*` (28) と `S1016` は恒久的に無効 |
