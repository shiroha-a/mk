...
| `make iprecord-check` | 利用者の IP を記録する call site が allowlist の外に増えていないか (#3135)。**#3105 の関連アカウント検索は `user_ip` の観測だけを見る**ので、失敗したサインインの IP がそこに入ると第三者が他人の関連候補を作れる (対象アカウントの ID で自分の IP から失敗を繰り返すだけでよい)。守りたいのは「どこからも呼ばれていない」という構造的な性質で、**endpoint ごとの振る舞いテストは叩いた経路しか見ない** — `/api/signin` だけを叩くテストは、同梱フロントが実際に使う `SigninFlow` や `signin-with-passkey` に記録を足す変異を素通りさせた (実測)。2 引数の `.Record(` を AST で拾い、許すものには理由を書かせる (IP と無関係な `deliveryhealth` の配送成否も 2 引数なので、名前だけでは分けられない)。**call site の列挙だけでは足りない** — パスキーの記録を `fail()` より前へ動かしても場所は変わらないので、順序も別に固定する (passwordless 無効の利用者に対して署名検証を通した時点で記録すると、本人でない誰かが試した IP が入りうる) |
