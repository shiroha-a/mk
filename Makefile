...
frontend-check: ## fork の frontend を型チェックし、submodule 依存のゲートを回す
	...
	MK_FRONTEND_GATES_REQUIRE_SUBMODULE=1 go test ./internal/server/ \
		-run 'TestCreditImageOriginsCoverAboutMisskey|TestMkGoRolePolicyKeysAreListedInFrontend|TestReactionLongPressIsWired|TestReactableRemoteReactionIsWired|TestMkGoUpdatedDialogIsWired|TestEmojiApplicationIsWired|TestEveryPluginSlotHasAMountPoint|TestAutoLoadingComponentsShowRateLimit' -count=1
