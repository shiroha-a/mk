...
- Feat: `clean` cron に未使用 antenna の自動 deactivate を追加し、その前提として antenna の read/update 時に `lastUsedAt` を bump するようにした (issue #1604, #1563 follow-up)。本家 `antennas/notes` は notes 取得のたびに `isActive=true`+`lastUsedAt=now` を bump する (使用中 antenna の再活性化を含む) ので、`core/antenna.Service.Notes` で best-effort に同等の bump を行う (mk-go は `OnNoteCreated` が毎回 `ListAllActive` を読むため本家の `antennaUpdated` 内部イベント相当の cache 無効化は不要)。`antennas/update` も `lastUsedAt=now` を常時 bump し、`isActive` を明示指定しない編集では本家同様 `isActive=true` へ復帰させる (deactivate 済み antenna が編集で再有効化される)。`isActive` を明示指定した場合はそれを尊重する (mk-go の `isActive` param 拡張)。`show` は本家同様 bump しない。`clean` cron (#1563, `0 0 * * *`) に `AntennaRepository.DeactivateUnusedSince` を使うサブタスクを追加し、`lastUsedAt < now-deactivateAntennaThreshold` かつ active な antenna を `isActive=false` にする (本家の盲目的 update と違い `isActive=true` を条件に加え、正味の無効化件数のみ書き込む)。`deactivateAntennaThreshold<=0` ではサブタスクを無効化 (本家 `> 0` ガード相当)。

- Feat: チャンネルミュートに有効期限 (`expiresAt`) を追加し、`checkExpiredMutings` cron で期限切れを掃除するようにした (issue #1603)。本家同様 `channels/mute/create` が `expiresAt` (epoch-ms, nullable = 無期限) を受理し、過去日時は `EXPIRES_AT_IS_PAST` (id `42b32236-...`) で弾く。`channel_muting` に `expiresAt` カラムを追加 (migration 000058)。read 経路 (`ChannelMutingRepository.ListByUser` / `Exists` = タイムライン filter・channel-show の isMuting・ミュート一覧) は期限切れを「ミュートされていない」として除外し、`checkExpiredMutings` cron (#1563, `*/5`) が user-mute と並べて期限切れ channel mute 行も物理削除する。`channel_muting` は `UNIQUE("userId","channelId")` を持つため、期限切れ (未prune) 行が残ったままの再muteで制約違反 500 にならないよう `Create` を upsert (ON CONFLICT で `expiresAt` 更新) にした。チェック順序は本家 create.ts に揃え alreadyMuting → `expiresAt`-past の順とし (既ミュート時の過去 expiresAt は 400 ではなく 204)、`expiresAt=0` は本家同様 falsy として無期限扱いにする。`expiresAt` には index を張る (migration 000058)。

- Fix: `/api/i/notifications-grouped` が `/i/notifications` と同じ非グループ化レスポンスを返していた問題と、`/i/update` が `alsoKnownAs` を無視していた問題を修正 (issue #1546 HIGH)。(1) 本家 `i/notifications-grouped` は連続する同種通知 (同一 note への reaction / 同一 target への renote) を `reaction:grouped` / `renote:grouped` に畳み込む (`i/notifications-grouped.ts`)。共有の fetch/cursor/type-filter/`CanSeeNote`(#1444) gate を `collectNotifications` に抽出し Show/Grouped で共用、Grouped は grouping 後に limit で slice。**両経路とも pack 後に `notehide.HideNotificationNotes`(#1570) を適用し、通知 note の depth-2 embed 可視性 IDOR を塞ぐ** (回帰テストで non-follower に followers embed が blank されることを検証)。(2) `i/update` が `alsoKnownAs` (acct/URI) を受理し、ローカル解決した canonical actor URI を csv で保存する (本家 error: `YOUR_ACCOUNT_MOVED`/`FORBIDDEN_TO_SET_YOURSELF`/`NO_SUCH_USER`/`URI_NULL`、maxItems 10)。`model.User.AlsoKnownAs` は既存のため migration 不要。
...
- Feat/Fix: 本家 systemQueue の定期 processor 4 件を mk-go の cron scheduler に追加・整合 (issue #1563)。(1) `checkExpiredMutings` (cron `*/5`): 期限切れ user muting 行を能動 prune する (read filter で既に隠れているが DB に残り続けていた)。channel-mute の erase は mk-go に expiresAt 列が無いため別 issue。(2) `checkModeratorsActivity` (cron `30 * * * *`): モデレーター全員が 7 日 (lastActiveDate 基準) 非アクティブだと登録を招待制へ自動切替 (`disableRegistration=true`) し、残 2 日以内は 6h ごとに警告メール / 各モデレーター宛 announcement / SystemWebhook (`inactiveModeratorsWarning` / `inactiveModeratorsInvitationOnlyChanged`) を送る。`role.Service.GetModerators` を新設。安全側 divergence として lastActiveDate を持つモデレーターが 0 人のときは登録を無効化しない (mk-go の lastActiveDate は認証 API request 時のみ更新されるため、計測不能を「不在」と誤判定しない)。(3) `clean` (cron `0 0 * * *`): user_ip の 90 日 prune / 期限切れ role_assignment の物理削除 / 放置 (未開始かつ ~10 分超) reversi game の削除を行う (antenna 自動 deactivate は mk-go が lastUsedAt を read 時に bump しないため faithful port が全 antenna を無効化してしまう。別 issue)。(4) `cleanRemoteNotes`: 旧 6h `time.Ticker` を本家同様 daily cron (`0 4 * * *`) に移行 (processor は既存、enable gate は processor 側)。注: `bakeBufferedReactions` の 30s ticker (本家 daily) は耐久性で勝るため維持、relationship の follow/block/unblock batch job 化は機能同等のため別 issue。
...
- Fix(security): admin REST API に OAuth access-token の scope (`meta.kind`) 強制を導入し、あわせて `/api/admin/queue/{show-job,jobs}` のレスポンスに本家の `processedBy` フィールドを追加 (issue #1552)。(1) 本家 `ApiCallService` は app (access) token が endpoint の `meta.kind` を permission 配列に持たないとき `PERMISSION_DENIED` (kind:permission, id `1370e5b7-...`, 403) を返すが、mk-go は admin REST で role gate (`RequireModerator`) のみで token scope を一切検査していなかった (= moderator が認可した app token が、その app に与えていない admin 操作を実行できた)。auth middleware に token scope 伝播 (native login token=full access / app token=permission 配列) を追加し、`RequireScope(kind)` middleware で強制する。本 PR は横断インフラを入れ #1552 対象の `admin/queue/*`・`admin/relays/*`・`admin/federation/*` 19 route に各 endpoint の本家 `meta.kind` (queue の `stats` のみ上流の癖で `read:admin:emoji`) を適用する。残り admin/全 kind-bearing endpoint への展開は follow-up。(2) `admin/queue/{show-job,jobs}` の QueueJob に本家 optional `processedBy` (処理 worker 名 = BullMQ `job.processedBy`) を mkq の `job.ProcessedBy` から出力 (非空時のみ)。注: `admin/queue/show-job-logs` の空配列 (mkq は per-task ログ非保持、上流も大半の job で空) と `admin/queue/queues` の queue 名集合 (#1393 の意図的決定で実行 queue のみ列挙) は parity 監査済みの no-action。
