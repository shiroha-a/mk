# Changelog

記法は Misskey 本家の `CHANGELOG.md` に準拠する (`General` / `Client` / `Server` の区分、`Feat` / `Enhance` / `Fix` の接頭辞)。
本家の新バージョンを取り込んだ分は「Misskey 202X.Y.Z に追従」の 1 行にまとめ、以降は mk-go 独自の変更のみを記載する。
`Client` は mk-go が同梱するフロントエンド (`third_party/misskey` fork) の変更を指す。

## 1.4.0

### Note

- **Misskey 2026.9.1 に追従した。** 本家のセキュリティ修正のほか、リモートノートの自動削除が一歩も進まなくなる問題 (本家 #17957 相当) とチャートの値が列の範囲を超えて記録ごと落ちる問題も同じ形で直してある
- **カスタム絵文字の登録申請を追加した。** 一般の利用者が画像やリモートの絵文字を申請し、モデレーターが審査する。ロール別の期間上限・審査待ちの上限・過去の申請の表示・枠の手動リセットを持つ。mk-go 独自機能
- **IP アドレスから関連アカウントを調べられるようにした。** IP からの検索とユーザー単位の関連候補、照会の監査記録を持つ。既定では管理者だけが使え、モデレーターに開くかはロールポリシー `canSearchIpHistory` で決める
- **画像の撮影情報 (位置情報を含む) が他人に見える URL で配られていた問題を修正した。** 投稿の添付・アイコン・バナーが原本を指しており、PNG / WebP / TIFF はメタデータの検出からも漏れていた。あわせて動画のサムネイルが 1 枚も作られていなかった問題も直した
- **legacy の asynq driver を削除した。** mkq が唯一のジョブキュー driver になり、mkq 1.2.0 (BullMQ 6 互換) へ更新した。定期ジョブの完了・失敗の記録が溜まり続ける問題と、使われなくなった定期ジョブが失敗を積み続ける問題も直した
- 連合・メディアプロキシ・ドライブ・権限判定まわりの硬化と、セキュリティ上の問題の修正を多数含む
- **運営者の作業が要るもの**:
  - **1.4.0 へ上げる前に同梱バッチ `backfill-remote-host` を流すこと。** 非正規化のまま保存されたホスト名を読むための互換経路を撤去したので、流していないとその行が DB から引けなくなる (手順は `docs/deployment.md`)
  - 既に設定されているアイコン / バナーは自動では直らない。同梱バッチ `backfill-avatar-public-url` で公開用の URL へ寄せ直す (手順は `docs/deployment.md`)
  - `jobQueueDriver: asynq` を明示している設定は**起動エラーになる**。旧ビルドで asynq の未処理ジョブを捌ききってから、`mkq` にするか行を消す (新ビルドからは asynq のジョブを読めない)
  - マイグレーションが 14 本ある (`000084`〜`000097`)。`note` への index 2 本は `CONCURRENTLY` で作るので書き込みは止めないが、大きな DB では時間がかかる。`000084` / `000085` / `000094` は既存の値を書き換えるので、Misskey 本家へ戻す場合は `docs/migration-from-ts.md` を先に読むこと
  - `admin/show-user` のログイン IP と `admin/drive/show-file` のアップロード元 IP / ヘッダは、`canSearchIpHistory` を持たないモデレーターには伏せるようになった。モデレーターに見せたい場合はロールで付与する
  - 同梱の nginx 参照設定を変更した (アクセスログからクエリ文字列を落とす / `/metrics` を塞ぐ / Origin を記録する)。参照設定を写して使っている場合は差分を取り込むこと。error_log にはトークンが残りうるので、設定内の注意書きを確認すること
  - 定期ジョブ (maintenance キュー) に溜まった**失敗**の記録は自動では消えない。コントロールパネルのジョブキューから消す (完了の記録は定期ジョブが完了するたびに 7 日より古いものから少しずつ消える)
  - フロントエンドを更新したので、再ビルドのうえ mk-go を再起動すること (`make uds-update` / `make docker-update`)
  - ソースからビルドする場合は Go 1.27 が要る。プラグインを組み込んでいる場合は `make plugins` で `go.work` を作り直す (古いままだとビルドが落ちる)

### General

- Misskey 2026.9.1 に追従
- Feat: カスタム絵文字の登録申請。リモート絵文字の取り込みの申請、ロール別の期間上限と審査待ちの上限、関連する過去の申請の表示、申請枠の手動リセット、受付時の審査者への通知
- Feat: IP アドレスからローカルアカウントを検索する機能と、ユーザー単位の関連アカウント候補。照会は監査記録に残る
- Feat: 通報をモデレーターの通知欄に残し、対処状況を反映するように
- Feat: ロール単位で通知の種類を受け取らないようにできるポリシー
- Feat: カスタム絵文字をアバターデコレーションとして使えるように
- Feat: リモートのリアクションに、ローカルの同名絵文字で相乗りできるように
- Feat: インスタンス全体のドライブ使用量と内訳を管理画面で見られるように
- Feat: 配送の健全性を管理画面から見られるように
- Feat: 利用者名の最小文字数を設定できるように
- Feat: 運営者のプラグイン構成で image をビルドする reusable workflow を追加
- Feat: `make pull` / `make uds-update` / `make docker-update` で本番の更新を一括化し、フロントエンドの配信 entry が生きているかを検証するように
- Fix: `Dockerfile.bundled` が `plugins/` に置いたプラグインを組み込んでいなかった問題を修正
- Fix: `.dockerignore` が設定ファイルと `drive-files` をビルドコンテキストから除外していなかった問題を修正
- Fix: フロントエンドのビルドが submodule の宣言と違う Node / pnpm の版で行われていた問題を修正
- Enhance: Go 1.27.1 / mkq 1.2.0 に更新

### Client

- Feat: カスタム絵文字の登録申請ページと審査画面。絵文字一覧とインスタンスメニューからも申請できる。画像のドラッグ&ドロップに対応
- Feat: リモート絵文字を右クリック / 長押しからインポート (権限が無い人は申請) できるように。そのままでは使えない名前の絵文字も名前を直して取り込める
- Feat: IP アドレスからの検索・関連アカウント候補・照会の監査記録の画面
- Feat: ユーザーのモデレーション画面に絵文字申請のタブと、プラグインの描画スロットを追加
- Feat: 通報の通知と、ロール編集画面の通知 opt-out の設定
- Feat: 管理画面のファイル一覧に使用量のタブ、連合ページに配送の健全性のタブを追加
- Feat: `/about-mkgo` を追加し、ソースコードの案内をそこへ集約。エントランスの GitHub リボンもそちらへ向ける
- Feat: プロフィールのアイコンを押すと拡大表示するように
- Fix: mk-go を更新しても更新ダイアログが出なかった問題を修正 (mk-go の版で判定する)
- Fix: エントランスの「他のサーバーを探す」を削除 (mk-go は行き先の一覧に載らず機能しないため)
- Fix: モバイルでバックグラウンドから復帰したとき、ストリーミングが無言で止まったままになる問題を修正
- Fix: 再接続したとき、切断していた間の通知とノートを取りこぼしていた問題を修正
- Fix: プロフィールページの pull to refresh が何も再取得していなかった問題を修正
- Fix: レート制限を受けても無限スクロールが再試行を続け、制限が解けなくなる問題を修正
- Fix: 「アニメーション画像を静止画で表示」の設定でアバターや mention のアイコンが壊れる問題を修正
- Fix: カスタム絵文字管理 (beta) のリモート一覧が 1 件も表示されなかった問題を修正
- Fix: iOS でリアクションを長押ししてもメニューが出ない問題を修正
- Fix: 分割アップロードのロールポリシーが編集画面から設定できなかった問題を修正
- Fix: 申請フォームが有効な captcha のうち 1 つのトークンしか送っていなかった問題を修正
- Fix: モバイル幅でモデレーションノートの追加ボタンの位置がずれる問題を修正

### Server

- Feat: トランザクションメールを受信者の言語 (日本語 / 英語) で送るように
- Feat: リモート actor の凍結 (`toot:suspended`) を取り込むように。モデレーターが解除した判断はリモートに巻き戻させない
- Feat: 削除したノートを renote / reply したリモートユーザーにも `Delete` を配送するように
- Feat: `mediaProxyConcurrency` でメディアプロキシの画像処理の同時実行数を設定できるように。過負荷時は 503 を返す
- Feat: 連合先の情報ページ (`/instance-info/:host`) を noindex に
- Enhance: フォロー一覧の一括取得にレート制限を設け、越境のブラウザに CORS を出さないように
- Fix: 定期ジョブの完了・失敗の記録が無期限に溜まっていた問題を修正 (7 日で消えるように)
- Fix: 専用キューへ移した旧い定期ジョブが残り、失敗を積み続け、一部のプラグインのジョブが二重に実行されていた問題を修正 (起動時に撤去する)
- Fix: 一時停止したキューの worker をオートスケーラが増やしていた問題を修正
- Fix: コントロールパネルのジョブ一覧が古い順で 30 件しか返さず、失敗したジョブが見えなかった問題を修正
- Fix: リモートノートの自動削除が同じ範囲を繰り返し見て進まなくなる問題と、配下のツリーの扱いを修正
- Fix: チャートの値が列の範囲を超えると同じ行の他の記録まで落ちていた問題を修正
- Fix: 予約投稿が一時的な失敗で再試行されずに失敗していた問題を修正
- Fix: inbox の処理が DB 障害を成功として扱い、activity を取りこぼしていた問題を修正
- Fix: 1 対 1 の配送が共有 inbox へ送られていた問題を修正
- Fix: 投稿の添付・アイコン・バナーで、他人に見せる URL が撮影情報付きの原本を指していた問題を修正。PNG / WebP / TIFF のメタデータも検出するように
- Fix: 動画のサムネイルが 1 枚も作られていなかった問題を修正
- Fix: アニメーション画像が公開用の画像やメディアプロキシで静止画に潰される問題を修正
- Fix: インターレース PNG が真っ黒で配られる問題と、プッシュ通知のバッジ画像が本家と違う問題を修正
- Fix: メディアプロキシが取得失敗・遮断・過負荷を区別せず、一時的な失敗を長期キャッシュさせていた問題を修正
- Fix: 「アニメーション画像を静止画で表示」の設定が絵文字に効いていなかった問題を修正
- Fix: リモートファイルのキャッシュ削除が行ごと消し、過去の投稿の添付が消えていた問題を修正
- Fix: 管理者が登録・差し替えたカスタム絵文字が、操作者の drive ファイルの削除で表示できなくなる問題を修正
- Fix: カスタム絵文字の操作で、列に入らない値を渡すと 500 になったり既存の絵文字を失う問題を修正
- Fix: パラメータに特定の文字を含めると 500 になる経路を塞いだ
- Fix: ブロックしても保留中のフォローリクエストが残る問題を修正
- Fix: サイレンスしたホストからのフォローが承認なしで通っていた問題を修正
- Fix: `blockedHosts` のノートが一部の一覧 (ユーザーのノート・チャンネル・リストなど 6 経路) に残っていた問題を修正
- Fix: チャットルームを配送元のホストごとに区別するように
- Fix: 管理画面で変えた `ugcVisibilityForVisitor` / `enableRemoteNotesCleaning` / `proxyRemoteFiles` / `enableIpLogging` が再起動するまで反映されなかった問題を修正
- Fix: ロールの剥奪が他のプロセスに最大 5 分反映されなかった問題を修正
- Fix: 複数プロセス構成で投票終了の通知が台数ぶん送られていた問題を修正
- Fix: IP の記録がサインイン時にしか行われていなかった問題を修正 (本家と同じく認証済みリクエストごとに記録する)
- Fix: `feedbackUrl` / `repositoryUrl` の既定値が入らず、改変版のソースの案内が空になっていた問題を修正
- Fix: `/about-misskey` の謝辞の画像が CSP で表示されなかった問題を修正
- Fix: `logging.sql.disableQueryTruncation` が効いておらず、SQL ログにバインド値が出ていた問題を修正
- Fix: `migrate` の全段 down が最後に必ず失敗していた問題を修正
- Fix: 2FA を解除してもパスキーでのパスワード無しログインが残っていた問題を修正 (該当行はマイグレーションで直す)
- Fix: nodeinfo・フィード・WebFinger など、未認証で届く経路の負荷と可視性の判定を修正
- Fix: 監査で見つかったセキュリティ上の問題を多数修正 (認可・連合・画像処理・リクエストの上限・ログへの資格情報の出力など)

## 1.3.0

### Note

- **Misskey 2026.9.0 に追従した。** あわせてカスタム絵文字の ZIP インポートが管理者限定になった (本家の破壊的変更。`canManageCustomEmojis` を持つモデレーターは使えなくなる)
- **承認制の新規登録を追加した。** 申請 → 審査 → 登録の流れを持ち、申請フォームの項目は運営者が定義できる。mk-go 独自機能で、Misskey 本家へ戻しても未知のテーブルが残るだけで壊れない
- **連合の受信経路をまとめて硬化した。** LD-Signature の検証が常に失敗して受信 activity を捨てていた不具合、相手が送ってきた長い値で取り込みごと落ちる経路、AP の型ゆらぎで特定の実装と一切連合できなくなる不具合、ピン留めの取り込みで受信処理が永久に止まる不具合など
- **ジョブキューの負荷と詰まりに対処した。** アイドル時の Redis コマンドが毎秒 774 から実質ゼロになり、handler から戻らなくなった worker を検出・回収するようになった
- プラグイン機構を拡張した (プラグイン間の連合・専用ジョブキュー・実効ロールポリシーの提供)。外部からの指摘を含むセキュリティ上の問題にも一通り対応した
- **運営者の作業が要るもの**: チャートの集計不具合を直したためグラフの値が変わる (従来が過小。既存データの遡及修正はしない)。相手インスタンスのホスト名の punycode 正規化は、既存行については同梱バッチ (`backfill-remote-host`) を手動で流す必要がある (手順は `docs/deployment.md`)

### General

- Misskey 2026.9.0 に追従
- Feat: 承認制の新規登録。申請フォームの項目は管理画面から定義でき、承認・却下は専用 bot が DM で知らせる
- Feat: プラグイン間の連合。同じプラグインを入れた mk-go 同士が専用経路でデータをやりとりできる
- Feat: プラグインに専用ジョブキューと `plugin/peercache` を開放
- Feat: プラグインが実効ロールポリシーに値を提供できるように
- Enhance: 同梱プラグインに `trustlevel` を追加 (既定では無効)
- Fix: 配布する compose 3 つがコンテナログのローテーションを設定しておらず、放置するとディスクが埋まる状態だったのを修正。**既存の環境にはコンテナを作り直すまで反映されない**
- Enhance: 依存ライブラリの既知脆弱性を解消
- Enhance: 起動時に配線し忘れの依存 64 件を検査し、欠けていたら起動を止めるように

### Client

- Feat: 起動時のスプラッシュを mk-go 独自のものに
- Feat: 承認制の登録の申請ページ・審査画面・申請フォームの編集 UI
- Feat: プラグインの描画スロットを連合ページとインスタンス情報ページに追加
- Feat: 通報画面を 5W1H の定型フォームにする。カテゴリ・該当 URL・発生日時・詳細・補足を 1 つの本文に組み立てて送る
- Fix: ジョブキューの job 詳細で、再試行が架空の時刻で並んでいた問題を修正
- Fix: WebSocket 再接続時に未読通知の件数をサーバー値へ揃えるように
- Fix: 承認制を切るとき、アカウント作成を閉じるかどうかを確認するように
- Fix: 起動に失敗したときのリロードボタンが CSP 下で反応しない問題を修正

### Server

- Feat: Webhook の本文に HMAC-SHA256 の署名ヘッダを付けるように
- Feat: 処理済みの ActivityPub activity の再投函を落とすように
- Feat: `roles/assignment-show` / `admin/roles/assignment-show` を追加
- Feat: CherryPick 由来の Argon2id パスワードを受理し、成功時に bcrypt へ段階移行するように
- Feat: 持ち主のいないリモート添付を掃除する日次ジョブ
- Feat: `bcryptCost` でパスワードハッシュの強度を設定できるように
- Feat: `crossOriginOpenerPolicy` を設定で選べるように (既定 off)
- Feat: 詰まり検出とハンドラ実行期限を設定で調整できるように
- Feat: captcha を 1 つも設定していない構成向けに、申請の入口へ署名付きフォームトークンを要求する仕組みを追加 (captcha の代替ではない)
- Fix: アイドルのジョブキューが Redis を絶えず叩き続けていた問題を修正。あわせて再起動を短縮
- Fix: handler から戻らなくなった worker がキューの処理能力を無言で削っていた問題を修正
- Enhance: `/api/meta` の応答をキャッシュ、reactions のデコードを削減、投稿後フックの同時実行数を制限
- Fix: リモートユーザーのプロフィールの場所・誕生日・追加情報が取り込まれず空だった問題を修正
- Fix: リモートユーザーのピン留め投稿が、観測を始める前の分は取り込まれない問題を修正
- Fix: リモートの投稿がアンテナに載らない問題を修正 (リレー経由を除く)
- Fix: リモートノートの `url` (HTML の permalink) が応答から落ちていた問題を修正
- Fix: 相手インスタンスのホスト名が非正規化のまま保存され、同じホストが別扱いになる問題を修正 (既存行は同梱バッチ `backfill-remote-host` で書き換える)
- Enhance: 受信した activity を捨てたときに相手をログに残すように (これまで送信元が分からなかった)
- Enhance: go-redis の内部ログを構造化ログへ流すように
- Fix: DB 障害を「対象が存在しない」という 4xx に潰していた 100 箇所超を 500 に分離。接続断が監視で 5xx として立つように
- Enhance: セキュリティヘッダを整備 (`X-Content-Type-Options` / `Permissions-Policy` / CSP の hash 方式化 / `/embed/`)
- Fix: リンクのプレビューとクローラ向け meta が本家と揃っていなかったのを修正
- Fix: LD-Signature の検証が常に失敗し、受信 activity が破棄されていた問題を修正
- Fix: AP ドキュメントの型のゆらぎを読めず、相手インスタンスと連合できなくなる問題を修正
- Fix: リモートから来た長い値が DB の列に収まらず、取り込みごと落ちていた問題を修正
- Fix: ピン留め投稿の取り込みが自己デッドロックし、受信処理の worker が永久に止まる問題を修正
- Fix: ピン留めされている投稿が inbox に直接届くと、メンション・返信の通知が出ない問題を修正
- Fix: リアクションとその取り消しが、作者が反応者をフォローしている場合に二重配送されていた問題を修正
- Fix: `Delete(Note)` とモデレーション活動が二重配送されていた問題を修正
- Fix: 再配送された `Create(Note)` で通知が二重に出る問題を修正
- Fix: リモートの添付が黙って落ちる問題を修正
- Fix: IDN ホストのリモートユーザーを投稿中のメンションから開くとエラーになる問題を修正
- Fix: リモートの投稿やリノートがリロードすると消える問題を修正
- Fix: アンテナに解決できない ID が溜まり、ページが空で返る問題を修正
- Fix: `sinceId` だけを指定したページングが「最新の N 件」を返していた問題を修正
- Fix: フォロー / フォロワー一覧が 1 ページ目で止まる問題を修正
- Fix: 通知まわりの不具合をまとめて修正 (500・添付の欠落・既読位置の飛び・バッジ・畳み込み)
- Fix: アプリが `notifications/create` で作った通知がリアルタイムでは出ない問題を修正
- Fix: Web Push の不具合を修正 (本文が空・payload 超過・チャットの新着)
- Fix: 凍結したユーザーの投稿がリアルタイム配信では流れ続ける問題を修正
- Fix: リノート元を削除するとリノート側の `renoteId` まで NULL になる問題を修正
- Fix: 運用実績のある Misskey 本家の DB を引き継ぐとマイグレーションが止まる問題を修正
- Fix: チャートの集計の不具合をまとめて修正。**この修正でグラフの値が変わる**
- Fix: ストリーミングのチャンネル単位の権限チェックが効いていなかった問題を修正 (1.1.1 の記述の訂正)
- Fix: チャットの認可漏れを修正 (ルーム操作・メッセージの参照と既読・所有権の譲渡・退出の連合配信・レート制限)
- Fix: 2FA のバックアップコードがパスワードの打ち間違えで消費される問題を修正
- Fix: パスキーを登録している利用者がパスワード検証の輻輳でログインできなくなる問題を修正
- Fix: パスワードリセットの経路だけ bcrypt の強度が下がっていた問題を修正
- Fix: ネイティブのアクセストークンを本家と同じ 62 文字集合から採るように
- Fix: 利用者キャッシュが古い行を返す経路を塞いだ
- Fix: コントロールパネルのジョブキューで job 詳細が保存値を返していなかった問題を修正
- Fix: `/api/meta` のデフォルトテーマが JSON5 のまま返される問題を修正
- Fix: メディアプロキシで一部の画像が表示できない問題を修正
- Fix: ロールポリシーの数値に小数を設定すると上限そのものが消える問題を修正
- Fix: ロールポリシーの極端な数値がオーバーフローする経路を飽和演算で塞いだ
- Fix: `signup` の応答が実効ポリシーではなく素の既定値を返していた問題を修正
- Fix: `meta.enableFanoutTimelineDbFallback` が読み取り経路で参照されていなかった問題を修正
- Fix: 承認制の登録まわりの不具合を修正 (期限切れからの確認メール・複数アカウント作成・招待枠の消費・ゲートの取り残し)
- Fix: `invite/delete` にモデレーターの迂回が無かったのを本家に揃えた
- Fix: 相手インスタンスの情報取得を本家に揃えた
- Fix: 管理画面の `@system` ファイル一覧にリモート由来のファイルが混ざる問題を修正
- Fix: スプラッシュの CSS / JS がキャッシュに固定される問題を修正
- Fix: 絵文字の配信で一時的な 500 応答にキャッシュ指示が付く問題を修正
- Fix: `config.url` に大文字が混ざる構成でリモートユーザーがピン留め一覧から消える問題を修正
- Fix: `-config-dump` がレート制限の実効値を worker 数倍で誤表示していた問題を修正
- Fix: 参照 nginx 構成で `enablePprof` 有効時に `/debug` が外から読める状態だったのを塞いだ
- Fix: 外部からの指摘を含むセキュリティ上の問題を複数修正。あわせて HSTS を実際に出すように

## 1.2.1

### Note

- 1.2.0 の pg_bigm 入り PostgreSQL イメージが、最新の `postgres:18-alpine` を pull する環境 (新規構築など) でビルドできない問題の修正のみを含むパッチリリース。

### General

- Fix: `deploy/postgres-bigm/` イメージが最新の `postgres:18-alpine` ベースでビルドできない問題を修正。base の Alpine 更新で `clang19` パッケージが消えたため、JIT 用 bitcode の生成 (`with_llvm=no`) をやめて clang/llvm への依存自体を無くした。pg_bigm の検索性能には影響しない

## 1.2.0

### Note

- **プラグイン機構を追加した。** 運営者が自分のインスタンスにバックエンド・フロントエンドの両方へ独自機能をビルド時に組み込める。公開パッケージ `plugin/` と `plugin-api.ts` が「壊さないと約束する範囲」で、サンドボックスではない (信頼できる作者のものだけを組み込む前提)。ドキュメントは `docs/plugins/`
- **アカウント移行の引き継ぎを本家同等にした。** 移行の検知、ローカルフォロワー・ブロック・ミュート・ロール・リストの引き継ぎ、アンテナへの移行先追記、24時間後の旧アカウントフォロー解除まで
- **PostgreSQL を 18 に統一し、pg_bigm による日本語部分一致検索の高速化に対応した。** あわせて検索クエリの LIKE メタ文字が素通しになっていた不具合も修正した
- 運用ツールを拡充した: データ整合性チェック (`-fsck`)、連合セルフ診断 (`-doctor`)、実効設定のダンプ (`-config-dump`)、配送・受信の健全性テレメトリ、プロセス統計のダッシュボード表示

### General

- Feat: プラグイン機構。`plugins/` に置いた独立 Go module をビルド時に組み込む。プラグインごとの PostgreSQL schema・独自 API (`/api/plugin/<name>/`)・定期ジョブ・設定と有効/無効トグル・独自ページとナビ項目・管理画面・プロフィール/設定画面への描画・バイナリ応答 (画像プロキシ) に対応。テスト補助 (`plugin/plugintest`)・開発ループ (`make plugin-dev`)・同梱サンプル (`plugins/status/`、既定無効) 付き。公開面は golden で drift gate する
- Feat: Web とジョブ処理のプロセス分離。upstream と同じ `MK_ONLY_SERVER` / `MK_ONLY_QUEUE` で役割を分けられる
- Enhance: PostgreSQL を 18 に統一 (compose 全構成 / CI / testcontainers)。`postgres:18` イメージの data layout 変更 (旧マウント先のままだと新規デプロイのデータが消える) に対応済み。既存 16 環境の移行手順は `docs/deployment.md`
- Enhance: pg_bigm 対応。検索クエリを upstream と同じ `lower(text) LIKE` 形にし、拡張入りイメージ (`deploy/postgres-bigm/`) を同梱。拡張と GIN インデックスを作るだけで検索がインデックスを使う (provider は `sqlLike` のまま)
- Enhance: DB を使うテストをパッケージ専用の PostgreSQL schema に隔離し、CI shard 間の干渉を解消
- Enhance: Playwright の UI 検証 spec を大幅拡充 (管理画面・設定画面・チャット・MiAuth・ギャラリー等 30 ページ超)
- Enhance: 依存と Go 本体の既知脆弱性を解消 (Go 1.26.6 / `golang.org/x/image` v0.45.0)

### Client

- Feat: ダッシュボードに mk-go のプロセス統計 (CPU・メモリ・GC・goroutine 等) を表示
- Feat: コントロールパネルにサーバープラグインの一覧ページを追加。構成・有効/無効・設定キー・残存データが確認できる
- Feat: 連合サーバー一覧に相手の署名方式 (Ed25519 対応状況) のラベルを表示
- Feat: プラグインの frontend (独自ページ・管理画面・スロット描画・Misskey コンポーネント再公開) を受け入れる

### Server

- Feat: データ整合性チェッカー `-fsck`。非正規化カウンタのずれと孤児行を検査し、`-fix` でカウンタを修正できる
- Feat: 連合セルフ診断 `-doctor` / `admin/self-check`。公開 URL 経由で WebFinger・nodeinfo・actor・DB・Redis・TLS 期限を一括検査する
- Feat: 実効設定のダンプ `-config-dump`。worker 数やレート上限など「設定ファイルを読むだけでは分からない実効値」を出す
- Feat: 配送先ホストの健全性テレメトリ (`admin/federation/delivery-health`) と受信側の集計 (`admin/federation/inbox-health`)
- Feat: プロセス統計 API `admin/server-metrics`
- Feat: 相手サーバーの署名方式 (RSA / Ed25519) を観測して記録し、`federation/instances` 応答に `signatureCapability` を追加
- Feat: relationship (follow / unfollow / block / unblock) を専用キューに分離 (`relationshipJob*` 設定)。あわせてフォロー関係の変更をストリーミング接続へ live で反映するように
- Feat: リバーシのランダムマッチが成立するように (ローカルユーザー同士)
- Feat: 新しい端末からのログインを upstream と同じ通知メールで知らせるように
- Feat: frontend HTML の Content-Security-Policy (既定 off、`report-only` / `enforce` を設定で選択)。object storage・外部メディアプロキシ・captcha 業者の origin は構成に応じて自動許可する。`Referrer-Policy` とアセット系の CSP も upstream に揃えた
- Fix: **検索クエリの `%` / `_` がワイルドカードとして解釈されていた問題を修正** (ノート検索・チャンネル検索。upstream はエスケープ済み)
- Fix: リモートの投稿・ユーザーを他サーバーが mk-go の URL 経由で照会すると 404 になり解決が失敗する問題を修正 (原本 URI へリダイレクト)。あわせて広告済みの activity id (`/notes/:id/activity`) を解決可能にした
- Fix: localOnly (連合しない) ノートが ActivityPub で取得できてしまう問題を修正
- Fix: Announce activity に `published` が無い・Create 内に余分な `@context` が残る shape を upstream に揃えた
- Fix: CSP の enforce 運用でリモート添付動画・URL プレビューの埋め込みプレイヤー・通知音が遮断されていた問題を修正
- Fix: アンテナのページ送りでノートが欠ける問題と、タイムライン閲覧後もアンテナの未読が消えない問題を修正
- Fix: リレー経由の投稿で、引用先が「削除されたノート」になる問題と、複数リレー購読時に同じ投稿が重複する問題を修正
- Fix: 期限付きミュートの自然失効が、接続中のストリーミングに反映されない問題を修正
- Fix: 移行済みアカウントのフォロー解除でフォローカウントが二重に減る問題を修正
- Fix: アバター・絵文字のリダイレクトでリモート URL をそのまま返していた経路を修正

## 1.1.2

### Note

- mk-go で開設したインスタンスから Misskey 本家へ戻す経路を実際に検証したところ、**送信側の連合が全滅する不具合**が見つかったため修正した。mk-go が発行していた RSA 秘密鍵の形式を本家が読めず、受信は動くので片方向だけ静かに壊れる。1.1.1 以前で mk-go が作成したユーザーの鍵は起動時に自動で変換される。
- 外部のセキュリティ監査で指摘された項目に一通り対応した。あわせて依存ライブラリと Go 本体の既知脆弱性を解消し、以降は CI で継続的に検出する。

### General

- Enhance: 依存ライブラリと Go 本体の既知脆弱性を CI で検出するように。到達可能なものだけを対象にするので、無視リストを育てずに運用できる
- Enhance: mk-go で作成したデータベースを Misskey 本家に引き渡せるかを CI で検証するように。「mk-go で始めても本家に戻れる」ことを毎回確かめる

### Server

- Feat: 投稿・ユーザータイムライン・クリップの埋め込み (`/embed/*`) を実装。これまでは埋め込もうとすると iframe の中に通常のアプリ画面が表示されていた
- Enhance: HTTP サーバーに読み込みと接続保持の上限時間を設定。低速な接続を大量に張られても資源を占有されにくくなった
- Enhance: クリックジャッキング対策のヘッダ (`X-Frame-Options`) を付与するように。埋め込み用の経路は対象外
- Fix: **mk-go が発行する RSA 秘密鍵の形式が Misskey 本家と異なり、本家に切り替えると連合の送信が全て失敗する問題を修正**。既存の鍵は起動時に変換する
- Fix: 未認証のリクエストでも巨大なファイルアップロードの解析が始まり、ディスクとメモリを消費できた問題を修正
- Fix: 依存ライブラリと Go 本体の既知脆弱性 11 件を解消 (画像デコードの異常終了・過大なメモリ確保、文字列正規化の無限ループなど)
- Fix: アクセスログとエラー報告 (Sentry) に認証トークンが平文で残る問題を修正
- Fix: メディアプロキシの署名鍵が未設定のとき、公開情報から誰でも計算できる値を使っていた問題を修正。未設定時は起動時に乱数を生成して保存する

## 1.1.1

### Note

- Misskey 本家の backend e2e (1245 テスト) を mk-go に向けて回せるようにし、そこで検出した互換性の差分をまとめて解消した。以降は PR ごとに CI で実行する。

### General

- Enhance: Misskey 本家の backend e2e をテスト本体に手を入れずに mk-go へ向けて実行する基盤を追加。意図的な差異は根拠付きで一覧化し、解消したら気付けるようにしている
- Enhance: `docker pull` して起動するだけの構成を、compose と設定のひな形だけを置いた `docker` ブランチとして配布するように。ソース一式を clone する必要がなくなった

### Server

- Feat: ユーザーフィード (`/@user.rss` / `.atom` / `.json`) を実装。これまでフィードリーダーには HTML を返す壊れたフィードとして見えていた
- Feat: 投稿・ユーザー・クリップ・ページ・ギャラリーの各 URL に OGP 用の meta タグをサーバー側で出すように。リンクを貼ったときのプレビューが機能するようになった
- Feat: `robots.txt` / `opensearch.xml` / `/api.json` / twemoji-badge を実装
- Enhance: アップロード時にファイルの拡張子を実際の形式に合わせて補正するように
- Fix: 宛先指定 (ダイレクト) の投稿が、本文でメンションしただけの相手からも読めていた問題を修正
- Fix: `read:account` 権限を持たないアプリのトークンで WebSocket に接続できていた問題を修正
- Fix: ストリーミングのチャンネル購読で権限チェックが不足していた問題を修正
  - **訂正**: この時点ではチャンネル単位の判定が実際には働いていなかった。接続時の
    `read:account` 検査 (上記) は有効だったが、接続にトークンのスコープが載って
    おらず、`read:chat` などチャンネルごとの要求は素通ししていた。実際に効くように
    なったのは後述の修正から
- Fix: ダイレクト投稿が宛先のホームタイムラインとリストのタイムラインに流れない問題を修正
- Fix: フォロワー限定投稿への返信が、投稿者をフォローしていない利用者のタイムラインに流れる問題を修正
- Fix: ロールの数値設定が整数に丸められ、1MB 未満のファイルサイズ上限などが無効になっていた問題を修正
- Fix: アンテナのユーザー指定が `@user@host` 形式を扱えず、指定しても除外してもアンテナが機能しない問題を修正
- Fix: アンテナのセンシティブチャンネル除外が判定に使われていなかった問題を修正
- Fix: ファンアウトタイムラインを無効にするとタイムラインが更新されなくなる問題を修正
- Fix: OAuth で認可コードを再利用したとき、発行済みトークンが最大 30 秒間有効なままだった問題を修正
- Fix: AVIF 画像から Web 公開用の WebP が生成されず、AVIF を表示できない環境で画像が見えない問題を修正
- Fix: API のエラー応答の HTTP ステータス・エラー ID・`WWW-Authenticate` ヘッダを本家に揃える
- Fix: `limit` に範囲外の値を渡したときに 400 を返すように
- Fix: `/api/i` や `/api/signup` の応答の形が本家と異なっていた問題を修正
- Fix: ミュート・ブロックの判定がリノート先まで辿れておらず、タイムラインに漏れる問題を修正
- Fix: リバーシで、相手が承諾していない招待に対して対局成立として応答していた問題を修正
- Fix: `meta` の行が失われた場合に自動で作り直すように
- Fix: 依存ライブラリの既知の脆弱性を解消 (drop-in e2e 用モックの `cryptography`)

## 1.1.0

### Client

- Feat: リレーの設定画面に「リレー投稿の一時保存」と「リレー由来ユーザーの整理」を追加
- Feat: 大きなファイルを自動的に分割してアップロードするように
- Fix: 管理画面のジョブキューで、実在するキューが一覧に出ず、存在しないキューが常時ゼロ表示になっていた問題を修正

### Server

- Feat: リレー経由でしか届かない投稿と投稿者をデータベースに保存せず、一定時間で破棄するように (既定では無効)。反応があった時点で通常の投稿として保存する
- Feat: リレー由来の投稿者のうち、投稿もリアクションもフォロー関係も残っていないものを定期削除する機能を追加 (既定では無効)
- Feat: 大きなファイルの分割アップロード (`drive/files/create-chunked/*`) を実装 (既定では無効)
- Feat: オブジェクトストレージ上のファイル削除をジョブキュー化。大量削除で API の応答が待たされなくなり、ストレージの不調時に再試行するように
- Feat: アセットを同梱したコンテナイメージを追加し、`docker pull` してそのまま起動できるように
- Enhance: 添付ファイルの形式判定を Misskey 本家と同等にし、`.mov` / `.heic` など 13 形式が「不明なファイル」になる問題を解消
- Fix: オブジェクトストレージを設定してもローカルに保存され続け、`/files` が 404 になる問題を修正
- Fix: リモート投稿の定期削除が、クリップ・ピン留め・お気に入り・リアクションの付いた投稿まで削除していた問題を修正
- Fix: ノートのメンション数の上限がロールの設定を参照していなかった問題を修正
- Fix: 一部の定期処理が連合配送用のキューに積まれ、配送用のワーカーを消費していた問題を修正

## 1.0.0

### Note

- Misskey 2026.6.0 / 2026.7.0 に追従。
- 本バージョンをもって drop-in 互換 (同じ DB / Redis / フロントエンドを共有したままバックエンドだけを差し替えられる状態) をベースラインとして固定する。以降、同梱フロントエンドは mk-go 独自の変更を含む。

### General

- Feat: Misskey 本家との仕様差分カタログ (`docs/divergence.md`) を追加し、意図的に本家と異なる挙動を一覧化
- Enhance: mk-go と Misskey 本家に同じリクエストを投げてレスポンスを値レベルで突き合わせる差分比較 e2e を追加
- Enhance: Playwright による e2e (370 spec) を PR ごとに実行するように。Misskey TS バックエンドに対しては upstream 追従時に実行し、spec が mk-go の挙動に引きずられていないかを検証する
- Enhance: drop-in 互換の e2e も PR ごとに実行するように (いずれもマージはブロックしない)
- Enhance: DB スキーマ・マイグレーション記録・インデックス命名の drift を CI で検出するゲートを追加
- Enhance: API 互換性マトリクスを Misskey 2026.7.0 基準で再生成
- Enhance: `make help` で全ターゲットを一覧できるように。あわせて `make check` (整形・静的解析・テスト) や `make update` (更新手順) などの常用コマンドを追加
- Enhance: セットアップ手順とアップデート手順をドキュメントに整備
- Fix: 依存ライブラリの既知の脆弱性を解消 (`golang.org/x/image`・`x/crypto`・`x/net`)

### Client

- Feat: チャットとリバーシで、リモートユーザーを相手に選べるように (mk-go は連合に対応しているが、本家フロントエンドはローカル限定として UI 側で無効化していた)
- Feat: バージョン情報で mk-go の実装バージョンを表示するように
- Feat: 管理画面のジョブキューに、ワーカー数・オートスケールの状況・ディスパッチ待ちと処理時間の統計・スケール履歴を表示するように
- Fix: 管理画面で VAPID 鍵を自動生成したあと、生成された鍵が画面に反映されない問題を修正

### Server

- Feat: OAuth 2.0 / IndieAuth プロバイダ (`/oauth/authorize`・`/oauth/decision`・`/oauth/token`) を実装
- Feat: ActivityPub のコレクションエンドポイント (outbox・followers・following・featured) を実装
- Feat: 未実装だった通知タイプ (投稿通知・ログイン・トークン作成・テスト通知・ロール付与・チャットルーム招待) を実装
- Feat: 注目の投稿 (`/api/notes/featured`・`/api/users/featured-notes`) を、リアクションとリノートの時間窓ランキングで算出するように
- Feat: Mastodon 互換の `GET /api/v1/instance/peers` を実装
- Feat: `POST /api/notes` (公開投稿の一覧取得) を実装
- Feat: ローカルのブロック・プロフィール更新・ピン留めを ActivityPub で配信するように
- Feat: リレーへの配送に LD-Signature (RsaSignature2017) を付与するように
- Feat: 実績 (achievement) のサーバー側付与基盤を追加
- Feat: 定期メンテナンス処理を cron として追加 (期限切れミュートの掃除、モデレーター全員が非アクティブな場合の招待制への自動切替、各種クリーンアップ、リモート投稿の削除)
- Feat: フォロー / ブロックの一括処理をジョブキュー化
- Feat: チャンネルミュートに期限を設定できるように
- Feat: 長期間使われていないアンテナを自動で無効化するように
- Feat: ジョブキューのワーカー数・オートスケール状況・遅延の統計を `/api/admin/queue` から取得できるように (mk-go 独自 API)
- Enhance: API のレスポンス形状・エラーコード・パラメータ検証を Misskey 本家に揃える全面監査を実施し、全 API 領域で 300 件以上の差分を解消
- Enhance: 日時を返すフィールドを本家と同じミリ秒付き ISO-8601 形式に統一
- Enhance: 一覧系エンドポイントが返す埋め込みユーザーに、フォロー / ブロック / ミュート等の関係を付けるように
- Enhance: 検索・タイムライン・アンテナで、凍結済みユーザー・ブロック中のホスト・ミュート対象を除外するように
- Enhance: 配送に失敗し続けるインスタンスを自動でサスペンドするように
- Enhance: deliver / inbox ジョブのリトライ間隔を本家と同じ指数バックオフにし、既定の試行回数を deliver=12 / inbox=8 に (従来の no-retry 挙動が必要な場合は `deliverJobMaxAttempts` / `inboxJobMaxAttempts` に 1 を指定)
- Enhance: inbox の署名検証でリモート公開鍵のパース結果をキャッシュし、受信処理を高速化
- Enhance: タイムライン取得の N+1 を削減 (関係解決のバッチ化、リアクション取得の絞り込み)
- Enhance: `note.mentions` / `note.fileIds` / `clip_favorite` / `drive_file` の各カラムにインデックスを追加
- Fix: リノート数の集計条件が本家と異なり、自分の投稿を自分でリノートした場合や bot によるリノートも加算され、逆に引用リノートが加算されない問題を修正
- Fix: `/api/antennas/create` と `/api/admin/announcements/create` が本家の必須パラメータを検証せず、不完全なリクエストを受理する問題を修正
- Fix: `/api/admin/show-user` が本家のレスポンスに無いフィールドを多数返していた問題を修正
- Fix: `/api/meta` が管理者向けの設定値 (アプリアイコン URL・シングルユーザーモード) を公開エンドポイントで返していた問題を修正
- Fix: ユーザーの `updatedAt` が API リクエストのたびに更新され、本家の「最終投稿日時」と意味が異なっていた問題を修正 (「アクティブなユーザー」の絞り込みや更新日時順の並び替えに影響)
- Fix: 引用元・返信先として埋め込まれた投稿が閲覧者の公開範囲を無視して本文ごと返っており、フォロワー限定・ダイレクトの投稿が引用や返信を介して漏れる問題を修正
- Fix: 通知・Web Push・Webhook・ストリーミングに埋め込まれる投稿にも、受信者ごとの公開範囲チェックを適用 (2 段目の引用 / 返信まで)
- Fix: `/api/notes/show` が非可視の投稿の本文を返す問題を修正
- Fix: ドライブ・クリップ・ページ・Play・ギャラリー・リスト・アンテナ・ロール・チャンネルの各一覧で、公開範囲や所有者の検証が漏れていた問題を横断的に修正
- Fix: レガシーな `/api/signin` で 2 要素認証を迂回できる問題を修正
- Fix: ドライブファイルの配信でアップロード時の MIME をそのまま返しており、細工したファイルでスクリプトが実行されうる問題を修正 (オブジェクトストレージからの直接配信も同様)
- Fix: リモートのメディア URL が API レスポンスに生のまま含まれ、閲覧者の IP アドレスが外部サーバーへ漏れる問題を修正 (メディアプロキシ経由の URL へ書き換え)
- Fix: ActivityPub inbox で Digest ヘッダによる本文の完全性検証と、署名者のホストと `actor` の一致検証を行うように (なりすまし対策)
- Fix: 取得した ActivityPub オブジェクトの id ホストを取得元ホストに束縛し、他ホストのオブジェクトを詐称できないように
- Fix: HTTP Signature の `keyId` を actor のホストに束縛し、鍵の取り違えを防ぐように
- Fix: SSRF ガードの遮断範囲を、IPv6 の未指定アドレスや NAT64 / 6to4 等の特殊用途レンジまで拡張
- Fix: API と inbox に認証前のリクエストボディ上限を追加
- Fix: 管理 API に OAuth アクセストークンのスコープ検証を導入 (モデレーターが認可したアプリが、与えていない管理操作を実行できた)
- Fix: `/api/admin/roles/users`・`/api/admin/accounts/find-by-email`・`/api/app/show`・`/api/my/apps` からメールアドレスやアプリシークレットが漏れていた問題を修正
- Fix: ストリーミングで、匿名接続が認証必須チャンネルを購読できる問題と、1 接続あたりのチャンネル数に上限が無い問題を修正
- Fix: チャンネル・ハッシュタグ・ロールタイムライン・アンテナの WebSocket チャンネルにリアルタイム投稿が一切届かない問題を修正
- Fix: アンテナの WebSocket チャンネルで所有者の検証が無く、他人のアンテナを購読できた問題を修正
- Fix: リストのタイムラインで、メンバーごとの「返信を含める」設定やメンバーの追加・削除が反映されない問題を修正
- Fix: カスタム絵文字とアナウンスの作成・更新・削除を全接続へ配信するように
- Fix: 通知ストリームにミュート / ブロック / インスタンスミュートのフィルタを適用
- Fix: 連合先からの引用投稿で引用元が表示されず本文のみになる問題を修正
- Fix: 受信した投稿の公開範囲の導出が activity の `to` / `cc` を見ておらず、公開範囲を誤判定する問題を修正
- Fix: 同じリモート投稿が重複して保存され、引用元のリノート数が二重に増える問題を修正
- Fix: 送受信する ActivityPub のオブジェクト形状と検証を本家に揃える (Person の `url` とアイコンのフォールバック、添付ファイルのサイズ、Undo / Flag の必須フィールド、受信 Follow / Update / Announce / Reject の扱いなど)
- Fix: ブロックが原因でフォローを受け入れられない場合に Reject を返すように
- Fix: サイレンス中のインスタンスからの公開投稿をホーム扱いに降格するように
- Fix: 連合を無効にしている場合に ActivityPub の各エンドポイントと inbox を 403 で塞ぐように
- Fix: nodeinfo / WebFinger / host-meta の応答内容を本家に揃える
- Fix: ホストの比較を punycode 正規化してから行うようにし、大文字混じり / IDN のホストで照合に失敗する問題を修正
- Fix: サスペンド / ブロックしたインスタンスへの配送を停止するように
- Fix: `/api/i/delete-account` がアカウントを削除しても関連データが残り、連合先に Delete を配信していなかった問題を修正
- Fix: 管理画面の設定 (`/api/admin/meta`) が保存済みの値でなく既定値を返し、保存しても巻き戻る問題を修正
- Fix: 管理画面の CAPTCHA 設定がプロバイダ別の構造を返さず、設定を保存できない問題を修正
- Fix: `/api/admin/server-info`・`/api/admin/queue`・`/api/admin/abuse-user-reports`・`/api/admin/show-user`・`/api/admin/emoji`・`/api/admin/roles` のレスポンス形状と権限チェックを本家に揃える
- Fix: モデレーターが管理者の情報を閲覧できる問題と、一覧エンドポイント経由で管理者専用フィールドを取得できる問題を修正
- Fix: 下書き・ギャラリー・ページ・Play・クリップで、添付ファイルが解決されない・`isLiked` が欠ける・更新パラメータが無視される問題を修正
- Fix: ロール・カスタム絵文字・アバターデコレーションのレスポンスが内部モデルのままで、必要なフィールドを欠いていた問題を修正
- Fix: チャットのルーム参加に招待を必須化し、メンバー以外がルームの発言やメンバー一覧を読める問題を修正
- Fix: チャットの入力検証・リアクション・既読・ページネーション・権限を本家に揃える
- Fix: `/api/users/show` が凍結済みユーザーを非モデレーターにも返す問題を修正
- Fix: `/api/users/notes` の `withReplies` の既定値が本家と逆だった問題を修正
- Fix: `/api/i/update` が `alsoKnownAs` やフォロー / フォロワー一覧の公開範囲を無視する問題を修正
- Fix: `/api/i/notifications-grouped` がグループ化されていない結果を返す問題を修正
- Fix: `/api/charts/*` の `offset` の解釈が本家と異なり、集計窓の境界がずれる問題を修正
- Fix: リアクションで、投稿の受け入れ設定 (いいねのみ等)・センシティブな絵文字・ロール制限・メディアサイレンスが無視される問題を修正
- Fix: 検索 (`/api/notes/search`・`/api/users/search`・`/api/notes/search-by-tag`) の条件と並び順を本家に揃える
- Fix: スレッドミュートが保存されず機能していなかった問題を修正
- Fix: `/api/notes/polls/recommendation`・`/api/notes/clips` が常に空配列を返すスタブだった問題を実装で解消
- Fix: モデレーター / 管理者が他人の投稿を削除できない問題を修正
- Fix: ロールのキャッシュが期限切れの割り当てを延命し、期限後もロールが有効なままになる問題を修正
- Fix: 複数プロセス構成でインスタンス設定の変更が他プロセスに伝播しない問題を修正
- Fix: Misskey TS が作成した DB に対してマイグレーションが途中で停止する問題を修正 (2026.5.0 以降の TS 製 DB からの切替が完走しなかった)
- Fix: mk-go のマイグレーション記録が TypeORM の形式と食い違い、Misskey TS へ戻したときにマイグレーションが再実行される問題を修正
- Fix: mk-go でのみ作成されるカラムに依存していた箇所を外し、Misskey TS 製の DB でも動作するように
- Fix: 同じ定義のインデックスが mk-go 名と Misskey TS 名で二重に作成される問題を修正
- Fix: `useObjectStorage` を有効にしたインスタンスで、S3 移行前に保存されたローカルファイルへのアクセスが 404 になる問題を修正
- Fix: Misskey TS から移行した直後に、既に終了していたアンケートの終了通知が一斉に発火する問題を修正

## 0.9.2

### General

- Feat: Misskey 本家と mk-go の API 互換性マトリクスを自動生成するツールを追加
- Enhance: API 応答の shape drift を CI で検出するゲートを導入

### Server

- Feat: グループチャットの連合に対応 (招待の送受信と Accept / Reject、ルームメッセージの送受信、退出の同期)
- Feat: リモートアカウントの自己削除 (Delete) を処理し、tombstone 化とカスケード削除を行うように
- Feat: `/api/fetch-external-resources`・`/api/drive/files/upload-from-url`・`/api/notifications/create`・カスタム絵文字の zip エクスポート・`/api/hashtags/users` を実装
- Feat: `/miauth/:session/check` に対応し、サードパーティクライアントからのログインを可能に
- Feat: 管理画面のジョブキューダッシュボードを整備 (キューごとのメモリ / 接続数 / 稼働時間、completed / failed ジョブの一覧と作成・処理時刻、mk-go が扱わないキューの非表示、ユーザー詳細のサインイン履歴 / ロール割り当て表示)
- Enhance: 各 API のレスポンス形状を Misskey 本家に整合
- Enhance: エラー ID・HTTP ステータス・ページネーション上限・権限 / secure の扱いを Misskey 本家に合わせて整合
- Enhance: ハッシュタグの抽出を MFM パーサベースに変更
- Enhance: タイムライン取得を高速化 (note hydration の batch 化、添付ファイルの folder / owner 解決の batch 化、デフォルトロールポリシーの共有キャッシュ化)
- Enhance: 1 ページ目のタイムライン応答を viewer ごとに短時間キャッシュするオプションを追加 (既定で無効)
- Enhance: ジョブキューの挙動を改善 (completed / failed の保持設定、Retry / Scheduled の分離、重複 Like の冪等化、配列でラップされた inbound activity の許容、キューごとの既定並列度の調整)
- Fix: サードパーティクライアント (Miria など) で、プロフィール・リアクション・リスト等のタブがレスポンス形状の不一致でクラッシュする問題を修正
- Fix: データエクスポート完了通知の `exportedEntity` が不正な値になり、通知一覧の取得が失敗する問題を修正
- Fix: 投稿通知を設定したユーザーへの返信がタイムラインから除外される問題を修正
- Fix: 実績の獲得 (achievementEarned) 通知が発火しない問題を修正
- Fix: プロフィール更新時にタグが空だと更新全体が失敗する問題を修正
- Fix: 連合インスタンス一覧の blocked / silenced フィルタと並び順を Misskey 本家に合わせて修正
- Fix: フォロー / アンフォローがストリームのフォロー関係に即時反映されない問題を修正
- Fix: `/api/users/reactions` の公開範囲フィルタと、リモートユーザーでのエラーを修正
- Fix: `/api/users/lists/show` で非公開のリストが他者に露出する問題を修正

## 0.9.1 以前

### Note

- リリース単位ではなく Phase 単位で開発していた期間。個別の変更は GitHub の issue / PR に記録されているため、ここでは主な成果のみをまとめる。
- Misskey 2026.5.4 に追従。

### General

- Feat: Misskey TS インスタンスとの drop-in 互換を検証する e2e 基盤を整備し (pytest / Cypress / Playwright)、nightly CI で実行するように
- Feat: 本家との差分を追跡するドキュメント運用を開始 (`docs/update/`)

### Server

- Feat: HTTP サーバー・DB / Redis 接続・設定ローダー・マイグレーション等の基盤を構築
- Feat: ユーザー・投稿・タイムライン・ドライブ・フォロー・リアクション・通知を実装
- Feat: WebSocket ストリーミングの全チャンネルを実装
- Feat: ActivityPub の送受信を実装 (Inbox・Deliver・Renderer・Resolver・HTTP Signature)
- Feat: ジョブキュー・検索 (Meilisearch / SQL フォールバック)・チャート・管理 API を実装
- Feat: TOTP による 2 要素認証・パスワードリセット・MiAuth・アプリ API を実装
- Feat: HTTP Signature の Ed25519 (FEP-521a Multikey) による署名・検証に対応 (Misskey 本家に先行する mk-go 独自実装、既存の RSA 鍵と併用)
- Feat: 受信する ActivityPub アクティビティの LD-Signature (RsaSignature2017) 検証を実装
- Feat: リモートユーザーの投稿数 / フォロー数 / フォロワー数を連合元から取得して表示するように (mk-go 独自)
- Enhance: ジョブキューのドライバを mkq に移行し、既定ドライバに
- Enhance: inbox の署名検証をワーカー側に移し、受信スループットを Misskey TS 比 2.6-2.8 倍に
- Enhance: 一覧系エンドポイントのページネーションを整理し、`sinceDate` / `untilDate` を全 61 ハンドラで受け付けるように
- Fix: Playwright / drop-in e2e で検出した Misskey 本家との差分 40 件以上を解消
- Fix: 遅延配送されたリモート投稿の投稿日時が受信時刻になる問題を修正
- Fix: カスタム絵文字のアニメーションがメディアプロキシで静止画化される問題を修正
- Fix: URL プレビューで Shift_JIS / EUC-JP / ISO-2022-JP のページが文字化けする問題を修正
- Fix: リモートユーザーのアクティビティタブでヒートマップが空になり、投稿数の折れ線が負方向にのみ動く問題を修正
