...
- mk-go で開設したインスタンスから Misskey 本家へ戻す経路を実際に検証したところ、**送信側の連合が全滅する不具合**が見つかったため修正した。mk-go が発行していた RSA 秘密鍵の形式を本家が読めず、受信は動くので片方向だけ静かに壊れる。1.1.1 以前で mk-go が作成したユーザーの鍵は起動時に自動で変換される。
- 外部のセキュリティ監査で指摘された項目に一通り対応した。あわせて依存ライブラリと Go 本体の既知脆弱性を解消し、以降は CI で継続的に検出する。

### General

- Enhance: 依存ライブラリと Go 本体の既知脆弱性を CI で検出するように。到達可能なものだけを対象にするので、無視リストを育てずに運用できる
- Enhance: mk-go で作成したデータベースを Misskey 本家に引き渡せるかを CI で検証するように。「mk-go で始めても本家に戻れる」ことを毎回確かめる

### Server

- Feat: 投稿・ユーザータイムライン・クリップの埋め込み (`/embed/*`) を実装。これまでは埋め込もうとすると iframe の中に通常のアプリ画面が表示されていた
- Enhance: HTTP サーバーに読み込みと接続保持の上限時間を設定。低速な接続を大量に張られても資源を占有されにくくなった
- Enhance: クリックジャッキング対策のヘッダ (`X-Frame-Options`) を付与するように。埋め込み用の経路は対象外
- Fix: **mk-go が発行する RSA 秘密鍵の形式が Misskey 本家と異なり、本家に切り替えると連合の送信が全て失敗する問題を修正**。既存の鍵は起動時に変換する
- Fix: 未認証のリクエストでも巨大なファイルアップロードの解析が始まり、ディスクとメモリを消費できた問題を修正
- Fix: 依存ライブラリと Go 本体の既知脆弱性 11 件を解消 (画像デコードの異常終了・過大なメモリ確保、文字列正規化の無限ループなど)
- Fix: アクセスログとエラー報告 (Sentry) に認証トークンが平文で残る問題を修正
- Fix: メディアプロキシの署名鍵が未設定のとき、公開情報から誰でも計算できる値を使っていた問題を修正。未設定時は起動時に乱数を生成して保存する

## 1.1.1

### Note

- Misskey 本家の backend e2e (1245 テスト) を mk-go に向けて回せるようにし、そこで検出した互換性の差分をまとめて解消した。以降は PR ごとに CI で実行する。

### General

- Enhance: Misskey 本家の backend e2e をテスト本体に手を入れずに mk-go へ向けて実行する基盤を追加。意図的な差異は根拠付きで一覧化し、解消したら気付けるようにしている
- Enhance: `docker pull` して起動するだけの構成を、compose と設定のひな形だけを置いた `docker` ブランチとして配布するように。ソース一式を clone する必要がなくなった

### Server

- Feat: ユーザーフィード (`/@user.rss` / `.atom` / `.json`) を実装。これまでフィードリーダーには HTML を返す壊れたフィードとして見えていた
- Feat: 投稿・ユーザー・クリップ・ページ・ギャラリーの各 URL に OGP 用の meta タグをサーバー側で出すように。リンクを貼ったときのプレビューが機能するようになった
- Feat: `robots.txt` / `opensearch.xml` / `/api.json` / twemoji-badge を実装
- Enhance: アップロード時にファイルの拡張子を実際の形式に合わせて補正するように
- Fix: 宛先指定 (ダイレクト) の投稿が、本文でメンションしただけの相手からも読めていた問題を修正
- Fix: `read:account` 権限を持たないアプリのトークンで WebSocket に接続できていた問題を修正
- Fix: ストリーミングのチャンネル購読で権限チェックが不足していた問題を修正
  - **訂正**: この時点ではチャンネル単位の判定が実際には働いていなかった。接続時の
    `read:account` 検査 (上記) は有効だったが、接続にトークンのスコープが載って
    おらず、`read:chat` などチャンネルごとの要求は素通ししていた。実際に効くように
    なったのは後述の修正から
- Fix: ダイレクト投稿が宛先のホームタイムラインとリストのタイムラインに流れない問題を修正
- Fix: フォロワー限定投稿への返信が、投稿者をフォローしていない利用者のタイムラインに流れる問題を修正
- Fix: ロールの数値設定が整数に丸められ、1MB 未満のファイルサイズ上限などが無効になっていた問題を修正
- Fix: アンテナのユーザー指定が `@user@host` 形式を扱えず、指定しても除外してもアンテナが機能しない問題を修正
- Fix: アンテナのセンシティブチャンネル除外が判定に使われていなかった問題を修正
- Fix: ファンアウトタイムラインを無効にするとタイムラインが更新されなくなる問題を修正
- Fix: OAuth で認可コードを再利用したとき、発行済みトークンが最大 30 秒間有効なままだった問題を修正
- Fix: AVIF 画像から Web 公開用の WebP が生成されず、AVIF を表示できない環境で画像が見えない問題を修正
- Fix: API のエラー応答の HTTP ステータス・エラー ID・`WWW-Authenticate` ヘッダを本家に揃える
- Fix: `limit` に範囲外の値を渡したときに 400 を返すように
- Fix: `/api/i` や `/api/signup` の応答の形が本家と異なっていた問題を修正
- Fix: ミュート・ブロックの判定がリノート先まで辿れておらず、タイムラインに漏れる問題を修正
- Fix: リバーシで、相手が承諾していない招待に対して対局成立として応答していた問題を修正
- Fix: `meta` の行が失われた場合に自動で作り直すように
- Fix: 依存ライブラリの既知の脆弱性を解消 (drop-in e2e 用モックの `cryptography`)
