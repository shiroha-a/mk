...
- **`Check bundled plugins are disabled by default` step** で、tracked な
  ...
  required job なので、コンパイル以外の理由でも `build` が赤くなる。
...
- **2026-08-24**: Section 8 の `build` ジョブに `Check bundled plugins are disabled by default` step を追記 (#2701)。同梱サンプルは #2495 で既定無効にする方針にしたが、trustlevel は #2586 で `disabled: true` 付きで同梱したあと **#2585 の実測を採るために意図的に外され、実測が終わっても戻っていなかった**。起きたのは「新しく同梱したものに既定を付け忘れた」ではなく「**検証のために一時的に外して戻し忘れた**」なので、gate はそちらを主対象にしてある。判定は 2 段 — tracked な `plugins/*/mk-plugin.yml` に `disabled: true` の行があること (列挙が空なら落とす) と、既定の `pluginbuild` が生成物を作らないこと (grep は行の形しか見ないので、行としては通るのに parser が読まない書き方を実際の parser で塞ぐ)。**この job は submodule を取らない**ので、frontend を持つプラグインが有効になっていると pluginbuild が submodule 不在で落ちる。原因と無関係な方向を指さないよう、exit code より先に生成物の有無を見る。
