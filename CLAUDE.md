...
- **2026-05-18**: iceshrimp / 一部 Pleroma fork など `icon` / `image` を **array 形式** (multi-resolution) で expose する remote instance からの actor fetch で avatar / banner URL が抽出されず、TL の `@mention` 横にアイコンが表示されない drop-in regression を fix (PR #1110)。原因: ActivityStreams 2.0 spec (https://www.w3.org/TR/activitystreams-vocabulary/#dfn-icon) は `icon` / `image` の値として **単一 object または array** どちらも許容するが、旧 mk-go の `activitypub.Image` は単一 object 形式のみ JSON unmarshal 可能で、array 形式の actor は Image 取得を silent fail していた (= avatar/banner field が空のまま user 行作成 → identicon fallback → mention 横にアイコン無し)。upstream Misskey TS `ApPersonService.resolveAvatarAndBanner` は `Array.isArray(img) → find(item => item.url)` で吸収する semantics なので、mk-go も `Image.UnmarshalJSON` で同等の両形式吸収 (array は url-non-empty な最初の要素を採用、空 array / null / 全 url 空は zero value で safely fallback) を実装。`Person.Icon` / `Person.Image` (banner) 両方が同 `*Image` 型なので 1 箇所の修正で actor / note attachment / channel banner 等の全 `Image` field に effect。test 7 件 (helper unit 6 / Person 経路 1 = iceshrimp 風 actor JSON で avatar/banner URL 正しく抽出される end-to-end guard) で regression を guard。drop-in 互換: schema 変更なし、エラーコード追加なし、行動変化は array 形式の actor を fetch したときのみ顕在化 (= 旧来 silent fail → 正しく avatar/banner 取得される方向のみ)。
- **2026-05-17**: 公開ロール (role.isPublic=true) が user profile に表示されない drop-in regression を fix (PR #1109、closes #1103)。旧 mk-go の `entity.PackUserLite` / `entity.PackUserDetailed` は `badgeRoles` / `roles` field を **空配列 hard-code** していたため、admin が公開ロールを assign しても (a) UserDetailed 経路の `roles` 配列が空のまま frontend がロール一覧を描画できない、(b) UserLite 経路の `badgeRoles` 配列が空のままバッジが表示されない、という 2 重の漏れがあった。upstream Misskey TS `UserEntityService.pack` の挙動 (`roles.filter(role => role.isPublic).sort((a,b) => b.displayOrder - a.displayOrder).map(...)`) と **field shape を byte-for-byte 互換**に揃え、`internal/entity/user_roles.go` に `UserRolesLookup` interface + `packPublicRoles` / `packBadgeRoles` helper を新設 (挙動レベルでは下記 scope 外 2 件のみ upstream 未追従)。`internal/core/role/user_roles_lookup.go` に `UserRolesLookupAdapter` を追加し router.go で `entity.SetUserRolesLookup(corerole.NewUserRolesLookup(roleService))` を wire (`SetCanChatLookup` #988 と同パターン、entity → core 逆依存を回避)。`badgeRoles` は upstream の保守的 default に揃えて **local user (`u.Host == nil`) のみ** に限定 (= remote user は meta.showRoleBadgesOfRemoteUsers gate が必要、meta lookup pattern 追加は follow-up)、moderator が private badge を view する経路 (`iAmModerator` branch) も viewer context が PackUserLite に通っていないため follow-up。`roleService.GetUserRoles` は per-user TTL cache (#761) 経由なので hot path コスト無し。test 10 件 (helper unit / nil-safe / 報告経路 end-to-end + adapter 4 ケース) で regression を guard。drop-in 互換: schema 変更なし、エラーコード追加なし、行動変化は admin が事前に公開ロールを assign していた場合のみ顕在化 (= 「設定したのに表示されない」期待挙動が実現される方向の差分のみ)。
- **2026-05-17**: admin endpoint 5 経路の silent enum fallback / 設定漏れ drift を一括 sweep (PR #1108)。PR #1102 の RolesUpdate.target で見つけたのと同種の「silent corruption / drop-in regression」パターンを admin 配下で網羅的に塞ぐ。(1) `ResolveAbuseReport`: `resolvedAs` を `"accept"` hard-code していたため admin が reject 判定を選んでも記録されない bug を fix、upstream 同様 `resolvedAs` (enum: accept/reject/null) を accept + enum 検証で silent corruption 防止。(2) `AbuseReportNotificationRecipientUpdate`: `method` を enum 検証なしで素通ししていた + correlation check が Update では走らない drift があり、admin が method=webhook に切替時 systemWebhookId 未設定で silent に矛盾状態を作れた → method enum reject + correlation check 追加。(3) `AnnouncementsAdminCreate`: `icon` / `display` を enum 検証なし → 不正値 reject。(4) `AnnouncementsAdminUpdate`: `imageUrl` / `icon` / `display` / `forExistingUsers` / `silence` / `needConfirmationToRead` の 6 field が request struct に欠落 → admin UI で変えても DB に書かれない drop-in regression (RolesUpdate と同種) → 全 field 追加 + icon/display enum 検証。(5) `UpdateMeta`: `sensitiveMediaDetection` / `sensitiveMediaDetectionSensitivity` / `federation` / `ugcVisibilityForVisitor` の enum 検証なし、特に federation を typo すると federation 全停止する drastic 事故の余地があった → 入口で validate して 400 reject。drop-in 互換: schema 変更なし、行動変化は不正値 reject + 旧来漏れていた update が反映される方向のみ。`MockAbuseReportRepository` / `MockMetaRepository` / `MockAnnouncementRepository` も同種の drift を持っていたため拡張 (test 経路でも regression catch 可能に)。test 14 件追加で各 sweep 経路を guard。
- **2026-05-17**: sensitive-words 配線の runtime perf を 2 経路で改善 (PR #1107)。(1) `NoteCreateService.Create` で `matchesSensitiveWords` と `checkProhibitedWords` がそれぞれ独立に `metaRepo.Fetch()` を呼んでいたため、L1 cache cold な環境で note 作成あたり meta を 2 round-trip fetch していた → caller 側で 1 度だけ fetch して両 helper に `*model.Meta` を渡す pure 関数 shape に refactor。(2) `HashtagService.OnNoteCreated` の `fetchHiddenAndSensitive` が caller 同期 thread で sync DB call していたため、fire-and-forget 設計 (#719) なのに caller が 1 fetch 分待たされていた → filter ロジック全体を goroutine 内に移動して caller 待ち 0 に。両改善とも behavioral change 無し (test 全 pass 維持)、純粋に hot path の DB call を削減する pure perf 改善。
- **2026-05-17**: PR #1105 の sensitive word 検出を mk-go independent hardening として **CW と text の独立 check** に強化 (PR #1106)。upstream Misskey TS の `NoteCreateService.ts:469` は `isKeyWordIncluded(data.cw ?? data.text ?? '', sensitiveWords)` で JS nullish coalescing semantics に従い CW が non-nil なら CW のみを check する仕様で、admin が「公開すべきでない」と設定したセンシティブワードを **CW を被せるだけで bypass できる** known な穴があった (例: CW=`"作品レビュー"` を付ければ text=`"重大なネタバレ"` でも public のまま federation 配送される)。本 PR で `matchesSensitiveWords` を CW / text 独立に `keyword.IsKeyWordIncluded` を呼ぶ shape に書き直し、いずれか片方でも match すれば home に降格する strict 仕様に切り替えた。regex フィルタ (`/pattern/flags`) の `^/$` boundary も field 単位で正しく解釈される (concat 案だと改行越境 match が発生して挙動が予測しにくくなる) ので、独立 check 設計は regex の正しい semantics 維持にも資する。drop-in 互換: schema 変更なし、API shape 変更なし、行動変化は CW 設定済 + text 側に sensitive word のケースで public → home に降格するようになる方向のみ (= upstream で漏れていた検出が補強される)。Ed25519 (#1067) / TOTP replay (#1100) / bcrypt 72-byte (#1075) と同系列の **upstream に存在しない mk-go independent hardening**、upstream への提案は `shiroha-a/misskey-ts` fork 経由で中長期検討。
- **2026-05-17**: `meta.sensitiveWords` / `meta.hiddenTags` 設定が runtime で効いていない drop-in regression を fix (PR #1105)。旧 mk-go は model に column を持ち admin API で受け取れていたが、note 作成経路 (`NoteCreateService.Create`) と hashtag mention 記録経路 (`HashtagService.OnNoteCreated`) で **設定値を一切参照していなかった** ため、admin がセンシティブワードを設定しても (a) 該当 word を含む public note が public timeline / federation 経由で広く伝播してしまう、(b) sensitive hashtag が featured / trends ランキングに出続ける、という二重の compliance / safety hole が空いていた (テストユーザー報告経路)。upstream Misskey TS は `UtilityService.isKeyWordIncluded` ヘルパで両経路を gate しているのと同様に、mk-go も `internal/misc/keyword.IsKeyWordIncluded` を新設し、(1) note create で text/CW が match した public note を home に降格 (channel 内は対象外、`silencingProvider` と同列のロジック)、(2) hashtag service で match した tag を `RecordMention` skip、の 2 経路に配線。helper は upstream と同じく `/regex/flags` パターン (Go regexp、JS-only flag `g/y/u/d` は no-op で吸収) と space 区切り AND match (= `"hello world"` で「hello AND world」) の両 syntax を完全実装し、不正 regex はキャッチして false を返す fail-safe。`HashtagService` には新規に `SetMetaRepo` setter を追加し router.go で wire。drop-in 互換: schema 変更なし、エラーコード追加なし、行動変化は admin が事前にワードを設定していた場合のみ顕在化 (= 旧来「設定したのに効かない」期待挙動が実現される方向の差分のみ)。upstream への提案不要 (= upstream regression fix と同方向)。
- **2026-05-17**: `admin/roles/{create,update}` で `policies` / `color` / `iconUrl` / `target` / `condFormula` / `canEditMembersByModerator` / `preserveAssignmentOnMoveAccount` field が DB に書かれない drop-in regression を fix (PR #1102)。upstream Misskey TS は 14-15 field を accept するが旧 mk-go は 5 field しか pipe しておらず、admin UI で「パブリック投稿を許可 (canPublicNote)」等の policy を変えても **DB に書かれず** 反映されない現象になっていた (= テストユーザー報告経路)。`RolesCreate` は request struct で受け取りつつ `CreateOptions` の 6 field 限定で /dev/null に流していた (=「将来 migration で col 追加するまで payload acceptance のみ」のコメント残骸、実際は migration 済で col 全部存在)。`RolesUpdate` は request struct 自体に 10 field 欠落。両 handler を upstream paramDef と同 15 field に揃え、JSON object (`policies` / `condFormula`) は `json.Marshal` で `datatypes.JSON` に変換して persist。同時に `Service.UpdateFields` の cache 戦略を `invalidateRolesListCache` → `InvalidateAllRoleCaches` に強化し、policy 変更が user の `userRoleCache` (per-user, 5min TTL) に対しても即時反映されるよう修正 (旧版は最大 5min stale)。`MockRoleRepository.UpdateFields` も同 drift を持っていたため拡張。test 4 件 (Create/Update の全 field persist + nullable color クリア + userRoleCache flush) で regression を guard。drop-in 互換: schema 変更なし / error code 据え置き、frontend は元から全 field を送っているため payload 変更不要。
- **2026-05-17**: TOTP gate が漏れていた sensitive endpoint 6 件 (`i/change-password` / `i/delete-account` / `i/update-email` / `i/2fa/register` / `i/2fa/unregister` / `i/2fa/remove-key`) に upstream Misskey TS 互換の 2FA gate を追加。旧 mk-go ではこれらが password だけで通っていたため、**password 漏洩 = 2FA 完全 bypass** で password 変更 / account 削除 / email 乗っ取り (→ password reset → account takeover) / 既存 2FA 無効化 / passkey 削除 / 既存 secret 上書き再登録 がすべて可能だった (drop-in regression、認証強度の大幅 degrade)。修正は `verify2FAToken(ctx, profile, token)` helper を 6 endpoint に追加 (= `if profile.TwoFactorEnabled && !verify` → 403 INVALID_TOKEN) する形で行い、PR #1100 で導入した `ValidateWithReplay` を介すため RFC 6238 §5.2 の replay 保護も同時に効く。request struct に `Token string` フィールドを追加した以外は backward compatible (2FA 未有効 user には影響なし、frontend は元から token を送っていたので drop-in)。upstream への提案不要 (mk-go が drop-in 互換のため upstream に合わせる方向の fix)。
- **2026-05-17**: TOTP code replay 脆弱性を fix (PR #1100)。`twofactor.Validate` が pquerna/otp の `totp.Validate` 素呼びで accept window (デフォルト ±1 step = 約 90 秒) 内に同一 6 桁コードを何度でも使い回せた (RFC 6238 §5.2 違反: "The verifier MUST NOT accept the second attempt of the OTP after the successful validation has been issued for the first OTP")。upstream Misskey TS の `UserAuthService.twoFactorAuthenticate` も同じ穴を持ったままで、本 fix は bcrypt 72-byte (#1075) / Ed25519 (#1067) と同系列の **mk-go 独自 hardening**。実装: `internal/core/twofactor/replay.go` に `ReplayGuard` interface + `RedisReplayGuard` + `ValidateWithReplay` を新設し、Redis SETNX で `mk:2fa:totp:used:{userID}:{code}` を TTL 120 秒で記録 (SETNX atomicity で concurrent 同コード送信でも片方しか accept しない)。signin TOTP / `i/2fa/done` / `verify2FAToken` (TwoFARegisterKey / TwoFAKeyDone) の 3 経路を一括差し替え、router.go で signin と `/api/i` 両方に同じ guard インスタンスを inject (= replay slot がエンドポイント越しに一貫)。drop-in 互換は厳格に維持: schema 変更なし / error code は既存 `INVALID_TOKEN` (UUID `cdf1235b-...`) を流用 → frontend / client は何も変更不要、replay と invalid を同 shape で返すので enumeration leak も無し。guard nil + Redis 障害は **fail-open + `slog.Warn`** (operator が Redis 障害で自分の環境から締め出される事故を防ぐ + degraded 状態を user-scoped で検知可能)。backup code は元から `ConsumeBackupCode` で single-use なので scope 外。既知の UX エッジケース: `i/2fa/done` 成功直後 ~30 秒以内に同じコードで signin を試みると permanent secret に同 value で昇格するため Redis slot に hit して 403 → 次の TOTP step (30 秒後) で解消。upstream Misskey TS への同 fix 提案は `shiroha-a/misskey-ts` fork 経由で中長期検討。
- **2026-05-16**: ed25519 follow-up #1083 (PR #1086) で Python ベースの Fedibird-like ActivityPub mock + `run-fedibird-test.sh` orchestrator + `test_fedibird_ed25519.py` (3 経路) を追加。mock は cryptography + Flask で軽量実装し、actor JSON で FEP-521a Multikey (Ed25519) と RSA を併出 + inbox の Ed25519/RSA HTTP Signature verify + `/_test/deliver` 経由で任意 inbox への signed POST を支援する。e2e で mk-A の resolver upsert / inbound Ed25519 verify / outbound capability-gated deliver の 3 経路が実 federation で動くことを担保。`make dropin-fedibird-test` で local 実行可能、nightly workflow への組み込みは別 PR で追加予定。
- **2026-05-15**: Ed25519 サポート (FEP-521a Multikey) を 6 PR で完成 (#1067 tracker / PRs #1074 #1078 #1079 #1080 #1081 #1084 + follow-up #1085 SHOULD)。mk-go 独自の先行実装として、HTTP Signature の Ed25519 sign / verify / capability-gated outbound deliver / lazy backfill / drop-in test を一連で導入。**Misskey TS 2026.5.1 は Ed25519 を採用しておらず**、本 feature は upstream に存在しない mk-go 独自拡張。`shiroha-a/misskey-ts` fork で同等の verify 対応を入れる PR は中長期検討。drop-in 互換は厳格に維持: 既存 `user_keypair` / `user_publickey` テーブルは untouched、Ed25519 鍵は別テーブル (`user_keypair_extra` / `user_publickey_extra`) で管理し、TS は新規 table を認識せず無視する。outbound sign は **配送先 actor が assertionMethod を expose しているときのみ** Ed25519 で sign し (= TS / Pleroma 等の RSA only サーバーへの誤配送を回避)、4xx 失敗時は host 単位の degrade flag (Redis EX=5min) で RSA 自動 fallback。stale key 削除 (= remote actor の key rotation 時に古い keyId を purge) も実装済で、compromised key による rogue verify を防ぐ。新規 error code / log message convention drift は無し (= upstream に提案できる形を維持)。
- **2026-05-15**: bcrypt 72 byte 上限超過で空 hash が DB に書かれて user が永久 login 不能になる bug を fix (#1075 → PR #1077)。Go の `golang.org/x/crypto/bcrypt.GenerateFromPassword` は 73 byte 以上で `ErrPasswordTooLong` を返すが、Node の bcrypt は silent truncation するため upstream Misskey TS では問題化していなかった (= Go ⇄ Node の bcrypt 実装挙動差)。mk-go の 3 経路 (`Signup` / `CreatePending` / `i/change-password`) で error を `_` で握り潰しており silent breakage を起こしていたため、`signup.ErrPasswordTooLong` sentinel を新規 export して handler 層で 400 に変換。新規 error code **`PASSWORD_TOO_LONG`** は upstream には存在しない **mk-go 独自 drift** (Fastify-style は code のみ / Misskey misc-style は UUID `f2e62243-b80a-43e4-9b17-329b422deded`)。`admin/password_reset.go` の `issueTemporaryPassword` は 16 byte hex 固定生成のため bcrypt 上限を超えず scope 外。drop-in 互換: 既存 72 byte 以下 user は影響なし、73 byte 以上で login 不能だった user は元から壊れていたため再登録で復旧可能。upstream に `PASSWORD_TOO_LONG` を提案する PR は中長期検討 (shiroha-a/misskey-ts fork ルート)。
- **2026-05-13**: admin/emoji/* (14 endpoint) と admin/avatar-decorations/* (4 endpoint) の middleware を policy gate に統一 (#1025 → PR #1037)。旧来 mk-go は `RequireAdmin` (4 件) と `RequireModerator` (13 件) で inconsistent に gate していたが、upstream Misskey TS `ApiCallService.ts:399-407` は `requiredRolePolicy: canManageCustomEmojis` / `canManageAvatarDecorations` のみで gate (`requireModerator/Admin` flag 無し) する semantics なので、mk-go も `middleware.RequireRolePolicy(roleService, corerole.PolicyCanManage*)` に統一。**production 影響**: 旧来 moderator が emoji 管理に使われていた場合、moderator role の policy で当該 key を `true` に override しないと管理不可になる (= upstream 仕様準拠で、drop-in 経路の TS frontend は元から同前提で動く)。admin role 持ちは `HasRolePolicy` 内で常に true を返す → 自動 bypass の挙動は維持。403 UUID は `c3d38592-...` (旧 RequireAdmin/Moderator) → `7f86f06f-...` (upstream `requiredRolePolicy` 違反 UUID) に変わるが、`code: "ROLE_PERMISSION_DENIED"` は両方とも同じ。
- **2026-05-13**: role policy enforcement 基盤を Misskey TS 互換に再実装 (#1020 → PR #1023)。`RolePolicyChecker` interface + `RequireRolePolicy(checker, key)` middleware を新設し、channels/create の handler-internal gate (#1012) を middleware に昇格 + 新規 10 endpoint (`/api/notes/search` `canSearchNotes` / `/api/users/search` 系 `canSearchUsers` / `/api/notes/translate` `canUseTranslator` / `/api/invite/create` `canInvite` / `/api/i/import-*` `canImport*` 5 種) に配線。同時に `core/role` の policy 計算を upstream 互換に書き直し: (a) `target=conditional` の 17 formula type (and/or/not/roleAssignedTo/isLocal/isRemote/isSuspended/isLocked/isBot/isCat/isExplorable/created Less・MoreThan/followers・following・notes Less・MoreThanOrEq) を `EvalCond` で評価し、`GetUserRoles` で manual + matched conditional の union を返す (= 全 user 適用の "ベースロール" 相当が conditional role 経由で効くようになる)。(b) `meta.policies` を `DefaultPolicies()` に base override として merge し、admin UI 設定の全 user 適用 default が反映されるよう修正 (数値型は `coerceToBaseType` で base の型に丸めて consumer 側 type assert が安全に動くようにする)。(c) `GetUserPolicies` を priority 2→1→0 cascade + 型別 aggregator (bool OR / int max / chatAvailability ranked / slice keep base) に置き換え。fail-closed (未知 type / nil user / parse error は false) + fail-soft (meta fetch error / role list error は base policies に fallback)。残 policy (条件付き gate / admin / TL availability / canHideAds / drive upload / limit 系 ~13 件) は sub-issue #1024-#1029 で順次対応。
