...
- **4 シャード並列** (`--shard=i/4`)。`fail-fast: false` で 1 つが落ちても
  他は完走する。
- **1 スタックあたりは直列でしか回せない。** 297 spec ファイル中 178 が共有の
  ...
  サインインせず root の token で API を叩く (`root.json` を読むのが 210 で、その差分)。
...
- **2026-09-12**: `make gates` に `secretfield-check` を追加。`make help` の target は 131 → 132。**モデルをそのまま JSON 化する経路があるので、`json:"-"` が唯一の防波堤になっているフィールドがある。** 実測で `internal/model` の該当タグを外しても `make gates` も全テストも緑のままだった。**名前だけでは判定できない** — `Meta` の captcha secret は `admin/meta` が管理画面へ返すし、drive の `accessKey` は URL の構成要素で秘密ではない。そこで #2792 と同じ **allowlist に理由を書かせる**方式にした。
  ...
  **静的なタグ検査だけでは足りない。** `MarshalJSON` を自前で実装した型で抜けるので、代表的な型を実際に `json.Marshal` して秘密が出ないことも見る。逆に**新しい秘密フィールドを `json:"-"` 無しで足す**形は静的検査でしか捕まらない (JSON 側は**そこに書いた型だけ**しか見ない)。
  **敵対的レビューで 3 つの穴が実測された。** (a) 正規表現が `Pass` を見ておらず **`Meta.SmtpPass`** (SMTP パスワード) が検出集合にすら入っていなかった。`Meta.*` を 11 件並べてあるので「Meta の秘密は列挙済み」と読めるのに 1 件漏れていた。(b) `Code` も見ておらず、**`SignupApplication.ClaimCodeHash`** — モデル側が「平文では持たない。DB が漏れた時点で全申請が乗っ取れる」と書いて `json:"-"` で守っているフィールド — のタグを外しても緑だった (**ゲートが存在する理由そのものの失敗形**)。`UserPending.Code` / `UserProfile.EmailVerifyCode` / `RegistrationTicket.Code` も同じクラスで serialize されたままだった。(c) `fld.Tag == nil` を skip していたため、**タグを書き忘れた新規フィールドが原理的に見えなかった** — `encoding/json` はタグの無い exported フィールドを Go の名前でそのまま出すので、これは最頻のミスにあたる。`Pass` / `Code` の追加は実測で**偽陽性ゼロ**、タグ無しを拾う変更も**現状の偽陽性ゼロ** (タグ無しの model フィールドは filter 構造体など 113 件あるが、どれもパターンに一致しない)。
  **変異検証は 22 形すべてを検出した** (数え方: モデル側の `json:"-"` 剥がし 13 + タグ削除 3 + ゲート側 6)。加えて 2 段変異で「`Pass` を外すと `Meta.SmtpPass` の漏れを検出できない」「`Code` を外すと `ClaimCodeHash` の漏れを検出できない」ことを確認し、(a)(b) の修正が必要だったことを実証した。**「N 形中 M 形」と書くなら変異集合そのものを書く** — 前稿は「20 形中 19 形」と書いたが、そこに (a)(b)(c) は入っておらず「他に穴は無い」と読める書き方だった。
  **allowlist の理由は「出してよい理由」ではなく実態を書く。** 初版は `Meta` に「moderator 以上にしか届かない」と書いていたが、実際の配線は `RequireAdmin` で、しかも **`admin/meta` は `map[string]any` を手で組んで返すのでこのタグは使われていない**。モデルを直接 JSON 化しているのは `internal/core/ephemeral/store.go` の 2 箇所 (`model.Note` / `model.User` を Redis へ) だけで、API のレスポンスは map か `internal/entity` を通る。つまりタグは今すぐ漏れる経路ではなく、**将来モデルが直接 marshal される経路に乗ったときの最後の防波堤**。`AccessToken.Token` の「本人の app token 一覧に返る」も誤りで、`i/apps` は token 値を返さない (返すのは auth/session/userkey と miauth)。
